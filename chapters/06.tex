...
\CAO{} auf Basis des \REM{} unter Verwendung des Taggers\is{Tagger} von
...
\posscite{schmid2019} \isi{Tagger} hat insgesamt 60.022 Wortformen im \CAO{}
...
editorische Abkürzung \fw{S.} für `Siegel' 3.321-mal fälschlich als
Adjektivform gekennzeichnet. Von den verbleibenden 56.701 Wortformen wurden
31.805 mit 95-prozentiger Sicherheit als Adjektiv gewertet. Der Anteil an
attributiven Adjektiven und Partizipien im ganzen \isi{Korpus} beträgt damit
ungefähr zwischen 1,5 und 3\,\%; sie finden in den Urkunden\is{Urkunde} also
eher selten Verwendung. Auffällig häufig kommen diese Wortformen als Attribute
von Präpositionalobjekten in Rechtsformeln vor \autocites[\ref{ex:adjprepobj};
vgl.][30]{becker2016}.
...
Urkunden\is{Urkunde} aufweisen und dabei die verschiedenen
oberdeutschen\il{Oberdeutsch} Dialekt\-räume abdecken, auch in Hinsicht auf die
auszuwertenden \KC{}-Handschriften (vgl.~\figref{fig:adjstpr_orte}).
...
betrifft, für die jeweils nur wenige Urkunden\is{Urkunde} zur Verfügung stehen.
Mögliche schreibsprachliche Unterschiede zwischen Stadt und Umland sowie die
Lage Augsburgs im Übergangsgebiet vom Schwäbischen\il{Schwäbisch} zum
...
Durchsicht und Klassifizierung der Belege erwies sich dieser \isi{Vorbehalt}
als unproblematisch.
...
automatische Wortartzuordnung des Taggers\is{Tagger} bei Adjektiven nicht
...
\isi{Tagger} von \citet{schmid2019} mit mindestens 95-prozentiger Sicherheit
...
Urkunden\is{Urkunde} des \CAO{} fällt auf, dass unflektiertes \norm{mīn} `mein'
...
enthalten \REF{ex:adjzueru}. Diese stammen allerdings nicht aus Zürich selbst,
sondern aus dem ca.\ 40~km in südöstlicher Richtung liegenden Hohenrain
(Kt.~Luzern).
...
Die Belege der Stichprobe zu Nürnberg zeigen ebenfalls keine
Auffälligkeiten in der Verteilung\is{Distribution!morphologische}. Allein die
relativ geringe Belegmenge führt dazu, dass für den Akk.\ Pl.\ M. sowie für den
Nom./Akk.\ Pl.\ F.\ keine Belege gefunden werden konnten. Da \norm{-e} im Akk.\
Sg.\ F.\ und im Nom.\ Pl.\ M. ansonsten aber erwartungsgemäß \norm{-iu}
(geschrieben ebenfalls \lit{iu}) im Nom.\ Sg.\ F.\ und Nom./Akk.\ Pl.\ N.\
gegenübersteht, kann mit etwas Vorsicht davon ausgegangen werden, dass der
Regelfall auch hier gilt.
...
Stichprobe zu jeder im Rahmen dieser Teiluntersuchung ausgewerteten
...
jeder vierte Beleg ausgewertet. Hierzu wurden Kasus, \isi{Genus} und
\isi{Numerus} sowie der Deklinationstyp (stark/schwach/prädikativ) und die
...
Lücken. Nominalisierungen von Adjektiven wie \norm{junc unde alt} `Jung und
Alt', \norm{diu hėiligen} `die Heiligen' oder appositiv\is{Apposition}
nachgestellte Attribute wie in \norm{Sylvester der guete} `Sylvester, der gute'
...
\ex \label{ex:kcmsg}
	...
	\ex \label{ex:kcmsg_1}
		...
		\footnotetext{Das \isi{Personalpronomen} der 1.~Pers.~Pl.~Akk. lautete
			im Althochdeutschen \norm{unsih} \autocite[331--333]{braune2018}.
...
\isi{Numerus}. Da das Adjektiv an dieser Stelle \lit{gvte} `gute' lautet und
nicht \lit{gvteſ} `gutes', wurde auf Plural geschlossen. Die zugehörige Stelle
in der Edition nach A1 weicht im Wortlaut komplett ab \REF{ex:kcvbaccplne_4};
...
Soweit Parallelstellen\is{Paralleltext} zu finden waren, ist die Formulierung
mit unflektiertem \norm{grōȥ} `groß' den C-Handschriften gemein. Die A- und
B-Handschriften enthalten das Synonym \norm{michel} `groß' ebenfalls
unflektiert \REF{ex:kcc1akkpln0_2}, mit Ausnahme\is{Ausnahme} von VB, wo das
Adjektiv ausfällt.
...
\subsubsection{K}
Der Fall der recht späten alemannischen\il{Alemannisch} Handschrift K ist
ähnlich gelagert wie der von C1, da auch hier an allen relevanten Stellen des
Paradigmas die Flexionsendung fehlen kann. Die Belege in \REF{ex:kckregel}
...
\REF{ex:kcb1regel_1} präsentierten. Mit \lit{alte} `alte' liegt hier eindeutig
ein Adjektiv vor, kein Kompositum \norm{althērren} `Älteste'
...
vor. \norm{Wunder} `Wunder' ist im \isi{Numerus} nicht eindeutig\is{Ambiguität}
und der Akk.\ Sg.\ N.\ kann nominal-stark endungslos\is{Endungslosigkeit} sein
(vgl.~\tabref{tab:adjparadgmstr_nm}), sodass nur der Vergleich mit
Parallelstellen\is{Paralleltext} einen Hinweis geben kann.
