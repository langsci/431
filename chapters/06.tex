...
Zeit angesetzt wird, ist in \tabref{tab:adjparadgmstr_pr} und
...
\isi{Korpus} beträgt damit ungefähr 1,5 bis 3\,\%; sie finden in den
...
angesprochen wird. Bei der Durchsicht und Klassifizierung der Belege
erwies\is{Validierung} sich dieser \isi{Vorbehalt} als unproblematisch.
...
Hiatusvermeidung\is{Hiatus} zurückgeführt werden kann.
...
Neben zwei Belegen mit \lit{rechte} `rechtmäßige' im Akk.~Sg.~F.\ entsprechend
dem in \REF{ex:adjzuere_1} finden sich auch zwei mit \lit{rechtú} für denselben
syntaktischen Kontext wie in \REF{ex:adjzuere_3}; vergleiche dazu
\citet[270--271]{ksw2}. Bei den letzteren zwei Urkunden handelt es sich um eine
Urkunde und ihre Bestätigung \autocite[375]{caor}. Für beide ist Zürich als
Ausstellungs\-ort angegeben.
...
Mehr noch als in Regensburg manifestiert sich die Schwa-\isi{Apokope} in der
...
\ex \label{ex:adjwienakksgf_4}
	...
	\trans `rechtsgemäßer Verzicht'
		...
		als starkes Femininum mit der Bedeutung `Entsagung, Verzichtleistung'
		aus. Aus dem Text der Urkunde wird deutlich, dass es um die Zusage des
...
vor. \norm{\textins{W}under} `Wunder' ist im Numerus nicht
...
\ex	\begin{xlist}
	...
	\sn \gll Groͤzu̍ wunder {maͤnig ualt} \\
		...
		\trans `Deine Macht vollbrachte dort vielerlei große Wunder.'
			(%
				K:~1ra,20; vgl.~%
				C1:~1ra,18--19%
				Z:~1ra,21--22%
...
\ex \label{ex:groziuwunder}
	...
	\ex \label{ex:groziuwunder_2}
		...
		\gll So groͤziv zaichen tæt \\
			so groß-\textsc{acc.pl.\NeutI.st} Wunder täte \\
		\trans `der durch den Rat seiner Leute so große Wunder wirkte'
			(%
				B1:~28vb,17--18; vgl.~%
				VB:~49vb,17--18;
				\cite[271]{schroeder1895}%
