...
das von sich aus keine Personenmerkmale\is{Personenmerkmal} und Informationen
zum Kasus der \isi{Nominalphrase} (NP) enthält. Personen- und Kasusmerkmale des
Substantivs werden durch die Flexion am Adjektiv\is{Adjektiv!attributiv}
...
Kongruenz noch Kongruenz mit den Merkmalskategorien\is{Merkmale!grammatische}
\isi{Concord} (grammatische Kongruenz \fw{ad formam} innerhalb der
NP\is{Nominalphrase}) und \isi{Index} (grammatische Kongruenz \fw{ad formam}
oder \fw{ad sensum} außerhalb der NP\is{Nominalphrase};
\cite[8--17]{wechslerzlatic2003}. Die theoretische Grundlage für diese
Unterscheidung liefert ihnen die \fw{\isi{Head-driven Phrase Structure
...
enthält, konkret also grammatikalisierte Personen\-merkmale\is{Personenmerkmal}
wie Person, \isi{Numerus} und \isi{Genus}
\autocite[15--17]{wechslerzlatic2003}. Auch in der mit der HPSG\is{Head-driven
Phrase Structure Grammar} verwandten \fw{Lexical-Functional Grammar} (LFG;
...
der Konjugation von Verben und der Bindung\is{Bindung} von
Reflexivpronomen\is{Reflexivpronomen} sichtbar \REF{ex:explvbkonj}.
...
\isi{Pragmatik} die Einstellung einer Sprecherin oder eines Sprechers zu
...
z.\,B.][]{klinger2002}. Weil Kontexte, in denen cis- und heterosexuelle
Normen\is{Heteronormativität} explizit unterlaufen oder infrage gestellt
werden, im Belegmaterial nicht vorkommen, gehe ich bei der Klassifizierung
meiner Daten von cis- und heteronormativen Gegebenheiten aus.
...
bezeichneten Person abhängt und damit vom konkreten
...
Einfluss auf die \isi{Genuskongruenz} von anaphorischen\is{Anapher} Targets
haben, möchte ich im Folgenden formale\is{Merkmale!grammatische} und
semantische Geschlechtsmerkmale\is{Merkmale!semantische} in der Annotation von
Beispielen konsequent abbilden. Zu diesem Zweck erweitere ich die herkömmliche
Annotation des Genus um einen \isi{Index}, der den Sexus kodiert, wie in
...
nachdem, ob das Geschlecht des Kindes im Kontext\is{Pragmatik} bekannt ist, als
...
\lit{Rvͦdiger} `Rüdiger (\MascM)' und seine \lit{hovſfrowe} `Ehefrau (\FemF)'
illustriert, auch wenn auf formaler Ebene \norm{bėide} `beide (\textsc{m+f})'
ansonsten sowohl für Maskulina als für auch Feminina gilt.
...
Reihenfolge\is{Abfolge} der grammatischen Merkmale\is{Merkmale!grammatische}
...
Subjekt-NP\is{Nominalphrase}, die die Personenmerkmale\is{Personenmerkmal}
3.~Person und Singular aufweist. Da die Personenmerkmale von \fw{Lola} diese
...
erfordert das Verb \fw{spielen} ein Subjekt mit den
Personenmerkmalen\is{Personenmerkmal} 3.~Person und Plural in Übereinstimmung
mit dem kombinierten \isi{Index} der Subjekt-NP\is{Nominalphrase} in $g$; zwei
...
einer anderen F-Struktur\is{funktionale Struktur} als das Subjekt ($j$
gegenüber $g$). Da aber Subjekt und Objekt vom gleichen Verb \fw{putzen}
abhängen, befinden sich beide dennoch innerhalb derselben
F-Struktur\is{funktionale Struktur} $f$ (gleicher Satz).
...
trotz Restriktionen bezüglich ihrer Abfolge\is{Abfolge} grundsätzlich möglich
ist. Im Unterschied zu Adjektiven\is{Adjektiv!attributiv} können Quantoren vom
Typ \fw{alle} und \fw{beide} im Deutschen\il{Neuhochdeutsch} außerdem nicht
...
Merkmal\is{Merkmale!grammatische} (\textsc{quant}\is{Quantifizierung})
definiert, da er aufgrund des distributiven Unterschieds zu Adjektiven nicht
als Teil der Menge der Adjunkte\is{Adjunkt} (\textsc{adj}) gezählt werden
sollte.
...
höher die Wahrscheinlichkeit für semantische Kongruenz. Die
Abfolge\is{Abfolge} ist monoton: Wenn bei einem Targettyp semantische
Kongruenz möglich ist, ist dies auch bei allen Targettypen rechts von ihm
möglich. Da für das Deutsche\il{Neuhochdeutsch} manche von
\citeauthor{corbett1979}s Kategorien für den von ihm untersuchten
...
NP-intern\is{Nominalphrase} auftreten.\is{Merkmale!grammatische} Aufgrund
induktiver Kriterien ergibt sich das Schema in \figref{fig:theoagrdist}.
...
Herrn Lukas' Hand' eine adverbiale\is{Adverbial} \isi{Präpositionalphrase} vor,
in der \lit{Lukas} seinerseits ein Genitivattribut\is{Attribut} zu \lit{hant}
...
\isi{Controller} bezieht, indem die \isi{Controller} im Kontext\is{Pragmatik}
