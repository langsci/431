...
\term{Floating Quantifiers} -- Quantoren, die getrennt von ihrer Bezugs-NP
...
\phantomsection
\label{phsec:hebrqf}
Die bisher referierten Aufsätze diskutieren Floating Quantifiers im Rahmen von
...
syntaktischer \isi{Ambiguität} führt. In den hier gesammelten Daten tritt das
Problem vor allem bei den zahlreichen Belegen für \norm{si bėide} \wdef{sie
beide} auf, wenn \norm{si} \wdef{sie (\textsc{pl})} im \isi{Mittelfeld} steht
(vgl.~\figref{fig:sibeideambig}). \isi{Kontaktstellung} ist also nicht gleich
Kontaktstellung, da auch dann ein gefloateter Quantor vorliegen kann.
...
wie in \figref{fig:sibeideambig} links dargestellt.
