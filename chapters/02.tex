...
\centering
	\begin{tabular}[t]{l @{} l c}
		...
		%
		& \colorbox{gray}{\textcolor{white}{-\textsc{nom.m.sg}}}% .\Stp
...
NP mit dem Kopf \fw{Lola}, die das \textsc{subjekt} des Satzes darstellt,
und einer \fw{conjunctional phrase} (CP), die das \textsc{subjekt}
...
%
	...
	% 	`und': boolesches \q{und} (beide Konjunkte beziehen sich jeweils
...
\textsc{condord}-Merkmal jedes der beiden Konjunkte übereinstimmt, damit Kongruenz
...
Französischen kongruiert der gefloatete Quantor (\fw{kol} `alle') dort
...
% \cref{ex:detbeidemhd} zitierten Stellen zurück; auch im \REM{} finden sich
...
% 		\trans `Da sagten die Leute über die beiden Eigentume'
% 		\end{taggedline}
...
% 		\trans `Danach verliehen die Pfleger der beiden Häuser'
% 		\end{taggedline}
...
% 		\trans `Da erschienen ihm Petrus und Paulus, die beiden heiligen
% 			Boten'
% 		\end{taggedline}
...
% 		\trans `Als die beiden vorgenannten Herren da zum Gerichtstermin
% 			kamen'
% 		\end{taggedline}
...
% 		\trans `da erschienen ihm die Herren, die beiden Gottesboten.'
% 		\end{taggedline}
% 	\end{xlist}
\end{exe}
...
% und auch im \REM{} fanden sich keine entsprechenden Vorkommen von
...
% brüeder} `meine Brüder' um eine DP mit \norm{mīn} als Kopf oder um eine NP
...
% 	\trans `So sollen sie meine beiden Brüder anmahnen'
% 	\end{taggedline}
% \end{exe}
...
% \cref{subsec:indexconcord} im Grunde das Gleiche anhand der Termini \textsc{concord}
% und \textsc{index} diskutiert. Unabhängig vom verwendeten Theorieframework weisen
...
% sinnvollsten zu sein, \norm{si bėide} `sie beide' und ähnliche
...
% Strukturmerkmalen (\textsc{concord} innerhalb der NP oder der vom Controller
...
% tendenziell nach deren semantischen Personenmerkmalen flektieren (\textsc{index}
...
% 	\textsc{index}-Merkmal innerhalb der NP schließen.}
% %
...
gemeinsam getätigt haben. Sie sagen \lit{bedi} `beide (\N.\Pl)' aus, die
...
\lit{hant} `haben (\Tpl.\Ind.\Prs)', Pronomina wie \norm{ſie} `sie
(\Pl)' und damit auch \lit{bedi} `beide' auf sie gemeinsam Bezug nehmen
können.
...
\lit{ſi beidv̍} `sie beide (\N)' dort gleichermaßen auf
...
\norm{si} `sie (\Pl)' und damit auch für \norm{bėide}
`beide' zur Verfügung.
