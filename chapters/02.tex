...
\wdef{versäumt} bildet. Damit ist \lit{\textins{B}eidiv} auch hier ein
\isi{Attribut}.
...
\fw{these} \wdef{diese} nur dann verwendet werden kann, wenn beide von ihm
...
Belebtheit macht sich nicht nur bei der \isi{Genuszuweisung}, sondern auch
...
historisch\is{Diachronie} langlebige Wort für \wdef{Frau} als formales Neutrum
...
speziellen Kontext mit \norm{bėide} \wdef{beide} liegen in der Belegsammlung zu
...
\wdef{Erbe} oder \norm{burgǟre} \wdef{Bürger} nicht auf eine
bestimmte\is{Definitheit} Person beziehen lassen, also unspezifische Referenz
vorliegt, werden sie mit \MascA\ annotiert. Im Fall von \norm{schuech}
\wdef{Schuh} als unbelebtem Substantiv\is{Substantiv} steht \MascI, bei
\norm{minne} \wdef{Liebe} als Abstraktum \FemI. Da es sich bei Körperteilen
nicht um Menschen handelt, wurden auch diese unter \isi{Vorbehalt} als
Inanimata\is{Inanimata} gewertet. So wird \norm{hant} \wdef{Hand} mit \FemI{}
...
gemeinsame \isi{Target} \fw{u-/kimevunjika} zeigt Kongruenz in der
Nominalklasse nur mit demjenigen Konjunkt, das ihm am nächsten steht (3
bzw.~7).
...
\wdef{Baum}, A-Strukturen\is{Argumentstruktur} wie
...
\wdef{alle} im Französischen\il{Französisch}. Er argumentiert, dass Quantoren,
die getrennt von ihrer Bezugs-NP rechts vom Verb stehen, keine
...
der gefloatete Quantor (\fw{kol} \wdef{alle}) dort nicht bloß, sondern
beinhaltet ein klitisches Pronomen, das mit dem Quantifizierten
...
\begin{figure}
	...
	\draw [myarrow]
		...
		([yshift=.5ex]{pic cs:beidepost2c_obj});
	\end{tikzpicture}
	\caption{Analyse des Satzfragments \norm{die jārƶīt bėide begāt} \wdef{die
		beiden Jahrestage begeht}}
...
\begin{figure}
	...
	\draw [myarrow]
		...
		([yshift=.5ex]{pic cs:beidepost2d_foc});
	\end{tikzpicture}
	\caption{Analyse des Satzframents \norm{die jārƶīt bėidiu begāt} \wdef{die
...
\lit{bedi} \wdef{beide (\textsc{n.pl})} aus, die Zahlung erhalten zu haben.%
%
...
So bezieht sich \lit{ſi beidv̍} \wdef{sie beide (\textsc{n})} dort
...
Pronomen wie \norm{si} \wdef{sie (\textsc{pl})} und damit auch für \norm{bėide}
