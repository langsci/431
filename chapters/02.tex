...
Beispiel ein Adjektiv\is{Adjektiv!attributiv} oder ein \isi{Artikel}, oder auch
ein anderer Kopf, der anaphorisch\is{Anapher} von diesem Substantiv abhängt,
...
die \isi{Kongruenzrelation} lakonisch als \textquote{essentially a matter of
...
\isi{Kongruenzrelation} zueinander stehen. Die \isi{Kongruenzrelation} geht vom
Controller aus, nach \citeauthor{corbett2006} \q{empfängt} das Target die
Information zu dessen grammatischen Merkmalen sozusagen.
...
Kasus des Substantivs als Suffix \fw{-er} am Adjektiv\is{Adjektiv!attributiv}
\fw{grün}, das von sich aus keine Personenmerkmale und Informationen zum Kasus
...
Personenbezeichnungen sind Informationen zum \isi{Genus} und Numerus
tendenziell an semantische Eigenschaften der bezeichneten Person
gebunden;\is{Denotation} bei Dingen und Abstrakta ergeben sie sich anhand
...
\fw{Mutter} hat damit \emph{feminin} als Wert des grammatischen Merkmals
\emph{\isi{Genus}}. Doch dass etwa \fw{Baum} maskulin und \fw{Liebe} feminin
ist, hat kein Korrelat in der außersprachlichen Realität, sondern ist eine
Konvention des Deutschen\il{Neuhochdeutsch}.%
%
...
Kongruenz mit den Merkmalskategorien \isi{Concord} (grammatische Kongruenz
\fw{ad formam} innerhalb der NP) und \isi{Index} (grammatische Kongruenz
...
es nach \citet[54]{corbett2006} vier Domänen mit wachsender Lokalität und
...
vorgenannten Ziele' das Akkusativobjekt zu \lit{verſitzzet} `versäumt' ist. Das
Target steht in einer anaphorischen Beziehung\is{Anapher} zum Controller.
...
Phrase Structure Grammar} Teil des Kopfmerkmals ist
\autocite[17]{wechslerzlatic2003}. Das Merkmal \emph{Concord} existiert ebenso
in der LFG \autocite[189--192]{bresnanetal2016} und enthält in beiden
...
NPs weitgehend deckungsgleich mit denen des \isi{Index} sind:%
%
...
was in der HPSG\is{Head-driven Phrase Structure Grammar} mit dem \fw{\isi{Head
Feature Principle}} begründet wird. Dieses besagt im Grunde, dass ein
...
Gegensatz zum \isi{Index} das Merkmal \emph{Concord} lokal auf die NP
...
dass \citet[76--78]{kingdalrymple2004} Concord im Gegensatz zum \isi{Index} als
...
lediglich ein einzelnes Substantiv determiniert und stattdessen die
\isi{Konjunktion} auf höherer syntaktischer Ebene stattfindet
...
biologische\is{Geschlecht!biologisches} Lesart. Vielmehr ist damit diejenige
Geschlechter\-rolle\is{Geschlechterrolle} gemeint, die in der Wortbedeutung
...
Zusammenhang ist das historisch\is{Diachronie} langlebige Wort für `Frau' als
formales Neutrum mit weiblicher \isi{Denotation} hervorzuheben: Im Alt- und
...
referenziellen Lesart mit Bezug auf einen bestimmten\is{Definitheit} Mann
...
stereotypen Geschlechterrollen\is{Geschlechterrolle} andererseits
...
die Koordination von \lit{ader} `Ader' und \lit{lit} `Glied' mit \lit{baidiv}
...
Neben formalen und semantischen Merkmalen kann auch die \isi{Distanz} zwischen
einem \isi{Controller} und seinem Target einen Einfluss auf die Wahl der
...
grammatische Merkmale derselben Kategorie (Genus\is{Genusmerkmal}, Numerus) pro
\isi{Controller} vorliegen. Die Frage in diesem Fall ist, wie ein Target, das
...
nächsten Konjunkt zu kongruieren \autocites[\fw{closest conjunct
agreement}\is{Closest Conjunct Agreement};
...
\begin{tikzpicture}[
	...
	\node (clbl) [lbl, above=.5ex of C] {controller\isi{Controller}};
...
der Kombination der semantischen Personenmerkmale die neutrale Form
...
vorhanden, sondern lediglich der Fall mit \norm{bėide} als Modifikator eines
Personalpronomens, das sich seinerseits auf zwei \isi{Controller} in dieser
...
Conjunct Agreement\is{Closest Conjunct Agreement} wie in \REF{ex:cca} auftritt
und es sich bei \norm{bėidiu} um eine Form des Nom.~Sg.~F.\ handelt, die
ebenfalls auf \norm{-iu} endet.
...
\isi{Konstituentenstruktur} und ist damit bestens geeignet, um Kongruenz als
...
Hauptbestandteil des Formalismus der LFG ist die \is{funktionale Struktur}
...
Attribute\is{Attribut} in der linken Spalte dienen Funktionen oder Merkmale wie
...
\textit{und}, Wortformen wie `Baum', A-Strukturen\is{Argumentstruktur} wie
\astruct{schreiben}{\ups{subj}, \ups{obj}} sowie F-Strukturen\is{funktionale
Struktur} oder Mengen von F-Struk\-turen\is{funktionale Struktur} sein. Jedem
Attribut\is{Attribut} sind dabei ein oder mehrere eindeutige Werte zugewiesen;
...
C-Struktur\is{Konstituentenstruktur} und der
F-Struktur\is{funktionale Struktur} des Satzes \fw{Lola rennt} dargestellt. Der
Satz betsteht aus einer NP mit dem Kopf \fw{Lola}, die das Subjekt
...
(flektierte) Verb \fw{rennt} zum Kopf hat. Ein Pfeil nach unten (↓) bezeichnet
...
Theorieframeworks gezeigt. Die Annotation\is{Annotation} der koordinierten NPs
folgt \citet{peterson2004}. In \figref{fig:lfgcoord_1} erfordert das Verb
\fw{spielen} ein Subjekt mit den Personenmerkmalen 3.~Person und Plural in
Übereinstimmung mit dem kombinierten \isi{Index} der Subjekt-NP in $g$; zwei
...
\isi{Concord}-Merkmal jedes einzelnen der beiden Konjunkte übereinstimmt, damit
Kongruenz hergestellt werden kann und ein kohärenter\is{Kohärenz}, grammatisch
wohlgeformter Ausdruck entsteht.%
%
...
unterzubringen, weder durch Koordination \REF{ex:beidedet_3} noch durch Reihung
\REF{ex:beidedet_4}, wie es etwa bei Adjektiven\is{Adjektiv!attributiv} trotz
Restriktionen bezüglich ihrer Abfolge grundsätzlich möglich ist. Im Unterschied
zu Adjektiven\is{Adjektiv!attributiv} können Quantoren vom Typ \fw{alle} und
\fw{beide} im Deutschen\il{Neuhochdeutsch} außerdem nicht prädikativ gebraucht
werden, wie aus \REF{ex:beidepred_2} deutlich wird (vgl.~auch \cite[181,
...
Kategorie ist \xhead{Q} wie \xhead{D} aus Sicht der LFG ein \isi{Ko-Kopf} von
...
\figref{fig:nomstack}, sondern fungiert als eigenständiger Kopf eines
...
\isi{Distanzstellung} in (b) und (c) sind jeweils markiert, insofern der
...
mit \isi{Distanzstellung} behandeln. Der Unterschied wird in
...
auf \citet{shlonsky1991} für das \ili{Ivrit}, siehe \REF{ex:hebrqf} und
...
\citeauthor{spector2009}s Analyse durch \isi{Koindizierung} verbunden, werden
aber nicht in der F-Struktur\is{funktionale Struktur} miteinander vereinigt
...
%
	\footnote{\isi{Artikel} und Quantor sind im ursprünglichen Beispiel nicht
		...
		Lesart in \figref{fig:beidepost_2dist} mit pronominal verwendetem
...
(\textsc{top}) werden hier die Rollen Objekt (\textsc{obj}) und \isi{Fokus}
...
%
Eine eingehendere Diskussion der Konstruktion müsste\isi{Desiderat} auf die
...
attributiv\is{Attribut} nachgestellt}, also wie in \figref{fig:sibeideambig}
rechts dargestellt.
...
\end{forest}
\caption{Zwei Varianten von oberflächlicher Kontaktstellung\is{Ambiguität}}
...
Korrelation zwischen der \is{Distribution!syntaktische} von Kongruenz zeigenden
Wortarten, deren syntaktischer Lokalität sowie den anwendbaren
Kongruenz\-merkmalen.
...
auch in \isi{Concord}-Merkmalen, da sie NP-intern auftreten. Aufgrund
...
adverbiale\is{Adverbial} Präpositionalphrase vor, in der \lit{Lukas}
seinerseits ein Genitivattribut\is{Attribut} zu \lit{hant} `Hand' darstellt.
Trotz allem sind beide Personen damit im Diskurs verankert\is{Diskursanker},
sodass im weiteren Verlauf Verben wie \lit{hant} `haben
(\textsc{3pl.ind.prs})', Pronomina wie \norm{ſie} `sie (\textsc{pl})' und damit
auch \lit{bedi} `beide' auf sie gemeinsam Bezug nehmen können.
