...
Portmanteau-Morphem zusätzlich zum Kasus des Substantivs als Suffix \fw{-er} am
Adjektiv \fw{grün}, das von sich aus keine Personenmerkmale und Informationen
...
daher \q{verschoben}. Das Adjektiv ist damit das Target der
...
\textsc{genus} \autocite[15--17]{wechslerzlatic2003}. Auch in der mit der HPSG
verwandten \fw{Lexical-Functional Grammar} (LFG;
\cites{kaplanbresnan1982}{bresnan2001}{bresnanetal2016}) existiert ein solches
Merkmal \autocite[189--190]{bresnanetal2016}. Das \textsc{index}-Merkmal bildet
...
kongruiert wird \autocite[74--76]{kingdalrymple2004}. \fw{Jan} und \fw{Markus} in
\REF{ex:coordidx} haben zwar jeder für sich einen Singular-\textsc{index}, die
...
zu produzieren. Das Beispiel in \REF{ex:engartdiscong} illustriert dies,
insofern \fw{these} `diese' nur dann verwendet werden kann, wenn beide von ihm
determinierten Konjunkte jeweils im Plural stehen \REF{ex:engartdiscong_1}
...
Kollektiva und Abstrakta diente, wie in \tabref{tab:pie_h2} gezeigt
\autocites%
	[73--74, 77]{ringe2017}%
...
\norm{wīp}. Auch im modernen Deutschen findet sich \fw{Weib} noch mindestens
...
Ein im Deutschen langlebiges Epikoinon ist \fw{Kind},
...
des Genus um einen Index, der den Sexus kodiert, wie in \tabref{tab:gendsex}
angegeben.
...
\REF{ex:cca} aus dem Swahili. Dort treten \fw{kiti} `Stuhl' und \fw{mguu
...
Sprachperiode wird in den oberdeutschen\il{Oberdeutsch} Schreibdialekte in
...
hier ausgewerteten Belege steht daher, wie in \REF{ex:gendres} illustriert,
aufgrund der Kombination der semantischen Personenmerkmale die neutrale Form
...
Informationen strukturiert präsentieren, vergleiche \REF{ex:avm}. Als
...
%
Dies wird in \REF{ex:lolamorphlex} verdeutlicht, wo die Lexikoneinträge für
...
\figref{fig:lfgcoord_1} erfordert das Verb \fw{spielen} ein Subjekt mit den
...
in \sectref{sec:ctrltarg} vorgestellt wurden, lassen sich in der Terminologie der
...
\sectref{sec:ctrltarg}, \sectref{sec:lfgkongr})[181--184]{ksw2}[772]{woellstein2022}.
Daneben ist bei \fw{beide} im Deutschen zu beachten, dass es zusammen mit einem
Definit- oder Possessivartikel auftreten kann \REF{ex:beidedet_2}, anders als
...
\REF{ex:beidepred_2} deutlich wird (vgl.~auch \cite[181,
Anm.~1]{merchant1996}).
...
die Quantorenphase (QP) mit \fw{beide} im Standarddeutschen gewöhnlich zwischen
DP und NP ein, wie in \figref{fig:nomstack} dargestellt (vgl.~auch \cite[44--45
mit Anm.~30]{lyons1999}). Als funktionale nominale Kategorie ist \xhead{Q} wie
...
Positionen des gestrandeten Quantors vor. Für das Englische\il{Englisch}
\fw{all} `alle' mit Bezug auf das Subjekt gibt \citet{merchant1996} grob die
Konstituentenstruktur in \figref{fig:qfgg} an (die Pfeile wurden zur
Verdeutlichung der postulierten Transformation hinzugefügt).
...
\REF{fig:hebrqf}. Anders als im Deutschen oder im Französischen\il{Französisch}
kongruiert der gefloatete Quantor (\fw{kol} `alle') dort nicht bloß, sondern
...
\ili{Ivrit} könnten insofern für die Analyse des Deutschen relevant sein, als
\citet[179]{merchant1996} eine Ähnlichkeit zwischen dem hebräischen\il{Ivrit}
\fw{kol} `alle' und dem deutschen \fw{alle} bezüglich des Auftretens von
Flexion in Abhängigkeit der Stellungsvariante vermerkt: Die Kongruenzendung
fehlt, wenn der Quantor einer definiten NP vorangeht: \fw{all die X} aber
\fw{alle X}. Der Fall von \fw{die beiden X} und \fw{beide X} würde eine
separate Diskussion benötigen. Nachfolgend möchte ich einige vorläufige
...
die Rollen \textsc{obj} und \textsc{foc} für \norm{bėidiu} und die dazugehörige
...
im Mittelfeld steht \REF{ex:sibeideambig}. Kontaktstellung ist also nicht
...
zulassen.%
%
	\footnote{Dies sind: Deutsch, Englisch\il{Englisch},
	Französisch\il{Französisch}, Lateinisch\il{Lateinisch},
	Russisch\il{Russisch}, Spanisch \autocite[214--215]{corbett1979}. Die
	Stichprobe ist durch die Beschränkung auf große europäische Sprachen
	typologisch nicht repräsentativ. \citet[218]{corbett2006} listet weitere
	Sprachen auf -- auch nicht-indoeuropäische --, für die seitdem
...
Analyse von Daten, wie sie in \chapref{ch:caoanalyse}, \chapref{ch:kcanalyse} vorgenommen
wird.
...
%
	\footnote{In der ausgewerteten Stichprobe tritt \lit{be(i)di} wie an der in
		...
		\sectref{sec:caoalemschwa})[vgl.][41]{paul2007}. Die Grafie der
