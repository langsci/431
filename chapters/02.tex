...
Substantivs als Suffix \fw{-er} am Adjektiv\is{Kongruenz!attributive}
\fw{grün}, das von sich aus keine Personenmerkmale\is{Personenmerkmal} und
Informationen zum Kasus der \isi{Nominalphrase} (NP) enthält. Personen- und
...
\isi{Concord} (grammatische Kongruenz \fw{ad formam}\is{Kongruenz!formale}
innerhalb der NP\is{Nominalphrase}) und \isi{Index} (grammatische Kongruenz
\fw{ad formam} oder \fw{ad sensum} außerhalb der NP\is{Nominalphrase};
...
\citet{wechslerzlatic2003} also alle vier Termini. Pragmatische\is{Pragmatik}
und semantische Kongruenz\is{Kongruenz!semantische} überschneiden sich mit dem
Begriff \fw{constructio ad sensum}, da sie sich beide aus der Semantik speisen.
...
zwischen Lexical Hybrids\is{Lexical Hybrid} im engeren Sinn und
Epikoina\is{Epikoina}.
...
semantischer Kongruenz\is{Kongruenz!semantische} steht
...
unterschiedlich hohe Affinität zur semantischen
Kongruenz\is{Kongruenz!semantische} aufweisen, sich also auch die syntaktische
\isi{Domäne} von \isi{Controller} und Target auf die Wahl der Kongruenzform
auswirkt \autocites%
...
formal\is{Kongruenz!formale} in den Kategorien Kasus (\textsc{case}),
\isi{Genus} (\textsc{gend}) und \isi{Numerus} (\textsc{num}) über ein
...
(indo-)europäischer Sprachen, die sowohl formale\is{Kongruenz!formale} als auch
semantische Kongruenz\is{Kongruenz!semantische} zulassen.%
%
	\footnote{Dies sind: 
...
innerhalb der NP\is{Nominalphrase} \isi{Concord} das entscheidende
