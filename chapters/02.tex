...
\norm{wīp} `Frau' als \NeutF\ annotiert, \norm{kint} `Kind' je nachdem, ob das
Geschlecht des Kindes im Kontext\is{Pragmatik} bekannt ist, als \NeutM, \NeutF\
oder \NeutX. Wenn sich im Kontext eines Beispiels \norm{ėrbe} `Erbe' oder
\norm{burgǟre} `Bürger' nicht auf eine bestimmte\is{Definitheit} Person
