...
%
Hervorzuheben ist dabei der Begriff der \isi{Kovarianz}. Informationen über den
\isi{Kopf} einer Phrase zeigen sich an einem anderen Wort, das sich auf diesen
\isi{Kopf} bezieht. Dies kann im Fall eines Substantivs ein Modifizierer des
...
\isi{Kopf}, der anaphorisch\is{Anapher} von diesem Substantiv abhängt, wie ein
Pronomen. Kongruenz zeigt sich daneben auch in der Übereinstimmung des Verbs in
Person und \isi{Numerus} mit seinem Subjekt. \citet[20]{corbett2006} fasst die
\isi{Kongruenzrelation} lakonisch als \textquote{essentially a matter of
\q{displaced} information} zusammen. Die unterschiedlichen Grammatiktheorien
...
grammatischen Merkmalen\is{Merkmale!grammatische} übereinstimmende Instanzen,
die in einer \isi{Kongruenzrelation} zueinander stehen. Die
\isi{Kongruenzrelation} geht vom Controller aus, nach \citeauthor{corbett2006}
\q{empfängt} das Target die Information zu dessen grammatischen
Merkmalen\is{Merkmale!grammatische} sozusagen.
...
Merkmale\is{Merkmale!grammatische} trägt: maskulin, Singular. Ferner sei
angenommen, dass es im Nominativ steht. Die Merkmale \fw{maskulin} und
\fw{Singular} sind dem Lexikoneintrag\is{Lexikon} des Wortes inhärent; sie
...
Substantivs als Suffix \fw{-er} am Adjektiv\is{Adjektiv!attributiv} \fw{grün},
das von sich aus keine Personenmerkmale und Informationen zum Kasus der
\isi{Nominalphrase} (NP) enthält. Personen- und Kasusmerkmale des Substantivs
werden durch die Flexion am Adjektiv\is{Adjektiv!attributiv} widergespiegelt
und erscheinen daher \q{verschoben}. Das Adjektiv\is{Adjektiv!attributiv} ist
damit das Target der Kongruenz\-relation\is{Kongruenzrelation}, während das
Substantiv den Controller darstellt.
...
\isi{Genus} und \isi{Numerus} tendenziell an semantische Eigenschaften der
bezeichneten Person gebunden;\is{Denotation} bei Dingen und Abstrakta ergeben
sie sich anhand formaler Kriterien \autocites[vgl.][2--4,
125--132]{corbett2006}{koepckezubin2017}. So bezeichnet
\fw{Mutter} eine \textquote{Frau, die ein oder mehrere Kinder geboren hat},
oder allgemeiner eine \blockcquote[s.\,v.~\fw{Mutter}]{duden-online}{Frau, die
in der Rolle einer Mutter ein oder mehrere Kinder versorgt, erzieht}.
\fw{Mutter} hat damit \emph{feminin} als Wert des grammatischen
Merkmals\is{Merkmale!grammatische} \emph{\isi{Genus}}. Doch dass etwa \fw{Baum}
maskulin und \fw{Liebe} feminin ist, hat kein Korrelat in der außersprachlichen
Realität, sondern ist eine \isi{Konvention} des Deutschen\il{Neuhochdeutsch}.%
%
	\footnote{Nichtsdestoweniger ist die \isi{Genuszuweisung} bei
		...
		starken Einfluss auf die \isi{Genuszuweisung} haben. Zum Beispiel
...
Kongruenz mit den Merkmalskategorien\is{Merkmale!grammatische} \isi{Concord}
(grammatische Kongruenz \fw{ad formam} innerhalb der NP\is{Nominalphrase}) und
\isi{Index} (grammatische Kongruenz \fw{ad formam} oder \fw{ad sensum}
außerhalb der NP\is{Nominalphrase}; \cite[8--17]{wechslerzlatic2003}. Die
theoretische Grundlage für diese Unterscheidung liefert ihnen die
\fw{\isi{Head-driven Phrase Structure Grammar}} (HPSG; \cite{pollardsag1994}).
Im Gegensatz zu der Anmerkung von \citet[164]{fleischer2012}, dass \q{formal}
und \q{semantisch} in der Forschung synonym zu \q{grammatisch} und
...
\begin{tikzpicture}[baseline=(grm.base), shorten >= 4pt, shorten <= 4pt]
	...
	\node [draw, rectangle, align=center, gray] (idx) at (3,-2) {
		\textsc{index} \\
		\footnotesize (anaphorische\is{Anapher}\\
		\footnotesize Bindung\is{Bindung})
	};
...
es nach \citet[54]{corbett2006} vier Domänen mit wachsender \isi{Lokalität} und
abnehmender Kanonizität:\is{Kongruenzrelation!nicht-kanonische}

\begin{enumerate}[noitemsep]
	\item innerhalb der NP\is{Nominalphrase};
...
Target einen \isi{Modifikator} seines Controllers \lit{wingarten} `Weingarten'
in derselben NP\is{Nominalphrase} beziehungsweise in derselben
\isi{Nominalgruppe} dar. Das Target steht attributiv\is{Attribut} zum
Controller.
...
Der Controller \lit{zil} `Ziele, Fristen' in \REF{ex:beidedomains_2} bildet den
\isi{Kopf} der in sich abgeschlossenen Genitiv-NP\is{Nominalphrase} \lit{der
vor genanten zil} `der vorgenannten Ziele'. \textins{\lit{B}}\lit{eidiv}
`beide' als darauf bezogenes Target steht außerhalb davon als \isi{Kopf} der
darüber liegenden NP, aber dennoch im gleichen Satzteil wie \lit{zil} `Ziele,
Fristen', da die komplexe NP \lit{der vor genanten zil / ainez / oder beidiv}
`eines oder beide der vorgenannten Ziele' das Akkusativobjekt zu
\lit{verſitzzet} `versäumt' ist. Das Target steht in einer anaphorischen
Beziehung\is{Anapher} zum Controller.
...
die Kongruenzbeziehung\is{Kongruenzrelation} anaphorisch\is{Anapher}.
...
\norm{bėide}-Target bezieht sich ebenfalls anaphorisch\is{Anapher} auf seinen
Controller.
...
Das Merkmal \emph{Index} ist in der HPSG\is{Head-driven Phrase Structure
Grammar} seinerseits Teil des Merkmals \emph{Content} eines lexikalischen
Zeichens, das sich aus der Semantik speisende Informationen über dessen Denotat
enthält, konkret also grammatikalisierte Personen\-merkmale wie Person,
\isi{Numerus} und \isi{Genus} \autocite[15--17]{wechslerzlatic2003}. Auch in
der mit der HPSG\is{Head-driven Phrase Structure Grammar} verwandten
...
Basis für anaphorische\is{Anapher} Referenz, indem es ein Individuum oder ein
Ding als Instanz des Bezeichneten im Diskurs verankert\is{Diskursanker}
\autocite[10--11]{wechslerzlatic2003}. Während eine große Nähe zwischen Index
und Semantik konstatiert wird, ist diese nicht absolut, da auch das generische
Pronomen\is{generischer Gebrauch} \fw{man} oder das expletive Subjektspronomen
\fw{es} einen Index besitzen, obwohl sie sich nicht auf eine bestimmte
semantische Größe beziehen \autocite[11--13]{wechslerzlatic2003}. Dies wird bei
der Konjugation von Verben und der Bindung\is{Bindung} von Reflexivpronomen
sichtbar \REF{ex:explvbkonj}.
...
koordinierten\isi{Koordination} nominalen Strukturen ein nicht-distributives
Merkmal darstellt, indem die Kombination zweier Substantive einen neuen Index
erhält, mit dem kongruiert wird \autocite[74--76]{kingdalrymple2004}. \fw{Jan}
und \fw{Markus} in \REF{ex:coordidx} haben zwar jeder für sich einen
Singular-Index, die Gruppe \fw{Jan und Markus} hat aber einen Plural-Index, was
sich in der Kongruenz zwischen Subjekt und Verb zeigt.
...
Das Gegenstück zum \isi{Index} bildet Concord, das in der HPSG\is{Head-driven
Phrase Structure Grammar} Teil des Kopfmerkmals\is{Kopf} ist
\autocite[17]{wechslerzlatic2003}. Das Merkmal \emph{Concord} existiert ebenso
in der LFG\is{Lexical-functional Grammar} \autocite[189--192]{bresnanetal2016}
und enthält in beiden Theoriesystemen syntaktisch relevante Informationen über
den nominalen Phrasenkopf\is{Kopf}, die im Deutschen\il{Neuhochdeutsch} für
einzelne, unkoordinierte\isi{Koordination} NPs weitgehend deckungsgleich mit
denen des \isi{Index} sind:%
%
...
%
Zu den Merkmalen \isi{Numerus} und \isi{Genus} tritt das strukturelle Merkmal
Kasus, dafür spielt Person hier keine Rolle. Kongruenz über das Merkmal Concord
herrscht typischerweise zwischen einem nominalen \isi{Kopf} und seinen
Modifizierern, was in der HPSG\is{Head-driven Phrase Structure Grammar} mit dem
\fw{\isi{Head Feature Principle}} begründet wird. Dieses besagt im Grunde, dass
ein Phrasenkopf\is{Kopf} seine grammatischen Informationen mit seinen Töchtern
teilt \autocites[vgl.][34]{pollardsag1994}[22]{wechslerzlatic2003}. Daher ist
im Gegensatz zum \isi{Index} das Merkmal \emph{Concord} lokal auf die
NP\is{Nominalphrase} beschränkt, während das Merkmal \emph{\isi{Index}} überall
dort vorkommt, wo anaphorische\is{Anapher} Bindung\is{Bindung} eine Rolle
spielt \parencites[14--16, 22]{wechslerzlatic2003}[189]{bresnanetal2016}.

In Bezug auf koordinierte Substantive\isi{Koordination} wie in
\REF{ex:coordidx} sei angemerkt, dass \citet[76--78]{kingdalrymple2004} Concord
im Gegensatz zum \isi{Index} als distributives Merkmal analysieren. Dies
...
Ausdruck zu produzieren. Das Beispiel in \REF{ex:engartdiscong} illustriert
dies, insofern \fw{these} `diese' nur dann verwendet werden kann, wenn beide
...
\fw{covert gender},\is{Genusmarkierung} bei dem die Klassenzugehörigkeit eines
...
allerdings durchaus Aspekte overter \isi{Genuszuweisung}, sei es durch die
...
Sinn sowie der sekundären Nutzung von Geschlecht als grammatischer Kategorie
\autocites[dazu ausführlich][]{kotthoffnuebling2018}{steriopolosteriopolo2022}.
Die Doppeldeutigkeit von \fw{gender} im Englischen\il{Englisch} als Bezeichnung
...
Sichtbarmachung von Frauen durch Movierung der maskulinen Form eines \isi{Nomen
...
mit \isi{Individualisierbarkeit} als auch in syntaktischer Hinsicht mit
Agensfähigkeit stand. Das spätere Maskulinum setzt das belebte\is{Animata}
Genus fort, das Neutrum das unbelebte\is{Inanimata}. Zur expliziten
Kennzeichnung von belebten\is{Animata} Feminina hat sich sekundär das Suffix
\fw{*-h₂} etabliert, das darüber hinaus zur Derivation von Kollektiva und
Abstrakta diente, wie in \tabref{tab:pie_h2} gezeigt
\autocites%
	[73--74, 77]{ringe2017}%
...
\q{richtigen} oder \q{falschen} Genus\is{Genusgebrauch} in Bezug auf deren
Identität betrifft \autocite[322--323]{corbett1991}.

\citet{steriopolosteriopolo2022} verorten diese affektive Funktion des
Genus\-gebrauchs\is{Genusgebrauch} im sozialen Geschlecht\is{Gender}
(\fw{social gender}) als Bindeglied zwischen
biologischem\is{Geschlecht!biologisches} und grammatischem Geschlecht. Sie
...
abweichenden\is{Ausnahme} Genus\-gebrauch\is{Genusgebrauch} zum Beispiel
despektiertlich kommuniziert wird, dass sich eine Person nicht in Einklang mit
der ihr zugemessenen \isi{Geschlechterrolle} verhält. Abgesehen von
lexikalisierten Fällen von abweichendem\is{Ausnahme} Genus wie
...
auf junge Menschen), spielt situativ abweichender\is{Ausnahme}
Genusgebrauch\is{Genusgebrauch} bei Personen im Belegmaterial keine Rolle.
...
macht sich nicht nur bei der \isi{Genuszuweisung}, sondern auch unter
morphologischen Gesichtspunkten bemerkbar, obwohl sie keine eigentliche
grammatische Kategorie des Mittelhochdeutschen\il{Mittelhochdeutsch} darstellt.
In den Worten \posscite[99]{dahl1999} ist Belebtheit, also der Unterschied
zwischen belebten\is{Animata} und unbelebten\is{Inanimata} Entitäten, in den
Grammatiken menschlicher Sprachen so allgegenwärtig, dass sie tendenziell als
...
%
	\footnote{\foreigntextcquote{english}[99]{dahl1999}{Animacy\is{Belebtheit},
		or the distinction between animate\is{Animata} and inanimate entities,
...
\caption{\isi{Belebtheitshierarchie} \autocite[72]{kotthoffnuebling2018}}
\label{fig:animhier}
\end{figure}

Belebtheit wird als Kontinuum konzipiert; die \isi{Belebtheitshierarchie}
reicht von Menschen als belebtester\is{Animata} Kategorie über Tiere zu
Gegenständen und Abstrakta als am wenigsten belebt (siehe
\figref{fig:animhier}). Die Sprecherinstanz steht dabei gewöhnlich an der
Spitze der Skala, da sie den Lokus der Wahrnehmung bildet \autocites%
	{silverstein1976}%
	[185--200]{comrie1989}%
	[203]{bossong1998}%
...
Unbelebtheit\is{Inanimata} mit formalen Kriterien der \isi{Genuszuweisung}
einhergeht (\fw{lexical gender}). Wie aus \figref{fig:animhier} deutlich wird,
gibt es der Natur von Kontinua entsprechend keine scharfe Grenze zwischen den
Polen \emph{belebt} und \emph{unbelebt}, sondern viele Übergangs- und
...
biologischem\is{Geschlecht!biologisches} Geschlecht in jedem einzelnen Fall
...
biologische\is{Geschlecht!biologisches} \isi{Lesart}. Vielmehr ist damit
diejenige Geschlechter\-rolle\is{Geschlechterrolle} gemeint, die in der
...
Geschlechterrolle\is{Geschlechterrolle}. Gerade in der älteren Literatur ist in
diesem Kontext häufig vom \q{natürlichen Geschlecht} die Rede;
\citet[67]{panther2009} spricht diesbezüglich vom
\q{konzeptuellen Genus}. Um auf das zuvor zitierte Beispiel zurückzukommen,
wird im Folgenden die Bezeichnung \fw{Mutter} entsprechend der in der
Wortbedeutung angelegten weiblichen \isi{Geschlechterrolle} als eine weibliche
Person denotierend\is{Denotation} aufgefasst
...
\fw{Lexical Hybrids}\is{Lexical Hybrid}, also Substantive, deren Genus und
...
also zum Beispiel, wenn \fw{das Mädchen} im weiteren Verlauf formal mit \fw{es}
oder semantisch mit \fw{sie} pronominal referenziert wird. \citet{klein2022}
unterscheidet darüber hinaus zwischen Lexical Hybrids\is{Lexical Hybrid} im
engeren Sinn und Epikoina\is{Epikoina}.

Bei Lexical Hybrids\is{Lexical Hybrid} im engeren Sinn liegt ein Konflikt
zwischen Genus und Sexus auf der lexikalischen Ebene vor
\autocite[145]{klein2022}. In diesem Zusammenhang ist das
historisch\is{Diachronie} langlebige Wort für `Frau' als formales Neutrum mit
weiblicher \isi{Denotation} hervorzuheben: Im Alt- und
...
sozialem oder biologischem\is{Geschlecht!biologisches} Geschlecht bezieht, oder
\fw{Mensch}, das zwar formal maskulin ist, für das aber prinzipiell dasselbe
gilt.%
%
...
Neutrum in Einklang mit dem formalen \isi{Genusmerkmal} sowie \emph{maskulin}
oder \fw{feminin} in Übereinstimmung mit dem semantischen Sexusmerkmal der
...
Maskulina\is{generischer Gebrauch}, also (vermeintlich) geschlechtsneutrale
Bezeichnungen wie \fw{Bürger} und \fw{Gast},
mittelhochdeutsch\il{Mittelhochdeutsch} \norm{burgǟre} und \norm{gast},
bewegen sich darüber hinaus im Spannungsfeld zwischen einer spezifischen,
referenziellen \isi{Lesart} mit Bezug auf einen bestimmten\is{Definitheit} Mann
einerseits und einer unspezifischen, nicht-referenziellen \isi{Lesart} mit
Bezug auf irgendein Individuum aus der benannten Gruppe und die damit
assoziierten stereotypen Geschlechterrollen\is{Geschlechterrolle} andererseits
...
Dingcharakter haben, aber Teil eines Organismus sind.\is{Ambiguität} Im
speziellen Kontext mit \norm{bėide} `beide' liegen in der Belegsammlung zu
dieser Arbeit keine Fälle von kombiniertem pronominalen Bezug auf Körperteile
vor, sondern lediglich der Fall in \REF{ex:bodyparts_attr}, bei dem \lit{baide}
`beide' attributiv\is{Attribut} \lit{hênde} `Hände' quantifiziert, sowie
mehrere Fälle mit \norm{bėide} als \isi{Konjunktion}, von denen einer
exemplarisch in \REF{ex:bodyparts_conj} zitiert wird. In diesem Beispiel wird
die \isi{Koordination} von \lit{ader} `Ader' und \lit{lit} `Glied' mit
\lit{baidiv} `beide' eingeführt, jedoch im weiteren Verlauf kein kombinierter
...
\ex \begin{xlist}
	\ex \label{ex:bodyparts_attr}
		...
		\trans `Sie wand ihre beiden Hände.'
			(%
				K:~6rb,19; vgl. abweichend\is{Ausnahme}
				A1:~4va,26--27;
				H:~5va,4;
				M:~7vb,8;
				B1:~4vb,49;
...
weniger belebten\is{Animata} Dingen abhebt, wie die Beispiele in
\REF{ex:stdgeralien} zeigen. Dieses Verhalten lässt sich als Unterschied in der
\is{Alienabilität} fassen \autocites{nichols1988}[17--18]{heine1997}. Eine
kursorische Suche im \REM{} nach dem inalienablen Typ mit
\textsc{pronomen}~(\textsc{dat}) -- \textsc{\isi{Artikel}}~(\textsc{acc}) --
\textsc{substantiv} hat keine Ergebnisse geliefert.
...
\isi{Controller} und seinem Target einen Einfluss auf die Wahl der
...
\citet{fleischer2012} geht diesem Aspekt in diachroner\is{Diachronie}
Perspektive nach. Diesen Studien ist die Erkenntnis gemein, dass Kongruenz
\fw{ad sensum} zum einen mit wachsender linearer Distanz\is{Distanz!lineare}
zwischen \isi{Controller} und Target tendenziell zunimmt, zum anderen
verschiedene Arten von Pronomina und anaphorischen Ausdrücken\is{Anapher} eine
unterschiedlich hohe Affinität zur semantischen Kongruenz aufweisen, sich also
auch die syntaktische \isi{Domäne} von
\isi{Controller} und Target auf die Wahl der Kongruenzform auswirkt \autocites%
...
\isi{Genuskongruenz} von anaphorischen\is{Anapher} Targets haben, möchte ich im
Folgenden formale\is{Merkmale!grammatische} und semantische
Geschlechtsmerkmale\is{Merkmale!semantische} in der Annotation von Beispielen
konsequent abbilden. Zu diesem Zweck erweitere ich die herkömmliche Annotation
des Genus um einen \isi{Index}, der den Sexus kodiert, wie in
...
`Erbe' oder \norm{burgǟre} `Bürger' nicht auf eine bestimmte\is{Definitheit}
Person beziehen lassen, also unspezifische Referenz vorliegt, werden sie mit
\MascA\ annotiert. Im Fall von \norm{schuech} `Schuh' als unbelebtem Substantiv
steht \MascI, bei \norm{minne} `Liebe' als Abstraktum \FemI. Da es sich bei
Körperteilen nicht um Menschen handelt, wurden auch diese unter Vorbehalt als
Inanimata\is{Inanimata} gewertet. So wird \norm{hant} `Hand' mit \FemI{}
...
`Dreifaltigkeit' kommen im ausgewerteten Material nicht vor. \norm{Got} `Gott'
wurde gemäß seiner Bezeichnung als \norm{truhtīn} `Herrscher' und \norm{hērre}
...
\isi{Koordination} von zwei Nominalen (Substantiven oder Personalpronomen)
unterschiedliche grammatische Merkmale\is{Merkmale!grammatische} derselben
Kategorie (Genus\is{Genusmerkmal}, \isi{Numerus}) pro \isi{Controller}
...
Konjunkt zu kongruieren \autocites[\fw{closest conjunct agreement}\is{Closest
Conjunct Agreement}; vgl.][179--180]{corbett1983}[168--170]{corbett2006}, wie
in dem Schema in \figref{fig:ccagraphic} gezeigt.
...
in \REF{ex:cca} aus dem Swahili\il{Swahili}. Dort treten \fw{kiti} `Stuhl' und
...
solchen Fällen die \isi{Kombinationsstrategie} angewandt, wie in
...
genusindifferent\is{Genusindifferenz} ist, muss der Unterschied zwischen den
einzelnen Controllern\is{Controller} durch die Kombination ihrer
...
der Kombination der semantischen Personenmerkmale\is{Merkmale!semantische} die
neutrale Form \norm{bėidiu} `beide (\NeutMF)', wie in \REF{ex:gendres} in Bezug
auf \lit{Rvͦdiger} `Rüdiger (\MascM)' und seine \lit{hovſfrowe} `Ehefrau
...
grammatischen Merkmale\is{Merkmale!grammatische} der Konjunkte ist im
ausgewerteten Material nicht vorhanden, sondern lediglich der Fall mit
\norm{bėide} als \isi{Modifikator} eines Personalpronomens, das sich
seinerseits auf zwei \isi{Controller} in dieser Abfolge bezieht. \citet[96,
145]{askedal1973} führt die in \REF{ex:askfmbeidiu} angeführten Stellen auf,
die sich mit den leicht anders formulierten Fällen im ausgewerteten Material
decken, sodass davon auszugehen ist, dass diese keine Sonderfälle darstellen.
Da bei beiden Abfolgen von \MascM\ und \FemF\ regelmäßig dieselbe Form
\norm{bėidiu} auftritt, ist nicht davon auszugehen, dass in \REF{ex:gendres}
und \REF{ex:askfmbeidiu} Closest Conjunct Agreement\is{Closest Conjunct
Agreement} wie in \REF{ex:cca} auftritt und es sich bei \norm{bėidiu} um eine
...
Attribut-Wert-Matrizen\is{Attribut-Wert-Matrize} dargestellt
...
Attribute\is{Attribut} in der linken Spalte dienen Funktionen oder
Merkmale\is{Merkmale!grammatische} wie Subjekt (\textsc{subj}), Objekt
...
`Baum', A-Strukturen\is{Argumentstruktur} wie \astruct{schreiben}{\ups{subj},
\ups{obj}} sowie F-Strukturen\is{funktionale Struktur} oder Mengen von
F-Struk\-turen\is{funktionale Struktur} sein. Jedem Attribut\is{Attribut} sind
...
Darüber hinaus wird eine \isi{Konstituentenstruktur} (\fw{constituent
structure}, \fw{c-struc\-ture}) angesetzt, die parallel zur
F-Struktur\is{funktionale Struktur} den Aufbau von syntaktischen Konstituenten
abbildet und eine Variante der \xbar{X}-Theorie
...
zwischen Spezifikator, \isi{Adjunkt} und \isi{Komplement} gemacht wird, weil
die Annotation\is{Annotation} disambiguiert.
...
(\textsc{subj}) des Satzes darstellt, und einer \isi{Komplementiererphrase}
(\fw{complementizer phrase}, CP), die das Subjekt dominiert und das
(flektierte) Verb \fw{rennt} zum \isi{Kopf} hat. Ein Pfeil nach unten (↓)
bezeichnet in der Annotation\is{Annotation} den jeweiligen Knoten selbst, ein
Pfeil nach oben (↑) den darüber liegenden. \q{\pass{subj}} ist daher so zu
...
Die optionalen Pfeile zwischen C-\is{Konstituentenstruktur} und
F-Struktur\is{funktionale Struktur} verdeutlichen, welcher Knoten seine
Informationen in welcher F-Struktur\is{funktionale Struktur} (hier: $f$ oder
$g$) ablegt. Die F-Struktur\is{funktionale Struktur} wird vom Verb
\fw{rennen} prädiziert (\textsc{pred}) und nimmt als Argument ein Subjekt
\ups{subj}. Dessen grammatische Eigenschaften bilden den Wert des
Attributs\is{Attribut} Subjekt (\textsc{subj}).
...
Attributen\is{Attribut} stets eindeutige und miteinander kompatible Werte
zugewiesen sein müssen, um einen kohärenten\is{Kohärenz} Ausdruck zu erzeugen
...
Lexikoneinträge\is{Lexikon} für
...
\fw{Lola} bildet als Substantiv einen \isi{Diskursanker} und definiert (=) die
angegebenen Merk\-male: 3.~Person und Singular. Die finite Verbform \fw{rennt}
bedingt (\req) ihrem Lexikoneintrag\is{Lexikon} gemäß eine
Subjekt-NP\is{Nominalphrase}, die die Personenmerkmale 3.~Person und Singular
...
Theorieframeworks gezeigt. Die Annotation\is{Annotation} der koordinierten
NPs\is{Koordination} folgt \citet{peterson2004}. In \figref{fig:lfgcoord_1}
erfordert das Verb \fw{spielen} ein Subjekt mit den Personenmerkmalen 3.~Person
und Plural in Übereinstimmung mit dem kombinierten \isi{Index} der
Subjekt-NP\is{Nominalphrase} in $g$; zwei Singulare ergeben zusammen einen
Plural.%
%
...
\isi{Numerus} mit dem \isi{Concord}-Merkmal\is{Merkmale!grammatische} jedes
einzelnen der beiden Konjunkte übereinstimmt, damit Kongruenz hergestellt
werden kann und ein kohärenter\is{Kohärenz}, grammatisch wohlgeformter Ausdruck
entsteht.%
%
	...
	der Annotation\is{Annotation} des Determinierers\is{Determinierer},
	\citet{bresnanetal2016} hingegen behandeln \emph{Spezifikator} als Merkmal.
	Beide folgen in ihren Beispielen der DP-Hypothese\is{Determiniererphrase}
...
\isi{Index}-Merkmale\is{Merkmale!semantische}.
...
Konjunkte im Singular stehen, können die unterschiedlichen
Genusmerkmale\is{Genusmerkmal} jedoch nicht vereinigt werden:
...
Dort ist \fw{braune} als adjektivisches Kongruenztarget\is{Adjektiv!attributiv}
ein \isi{Adjunkt} von \fw{Katze} und steht damit in derselben
F-Struktur\is{funktionale Struktur} $g$ wie sein \isi{Controller} (gleiche
NP\is{Nominalphrase}). Auf \fw{Katze} ist ebenfalls der Possessor \fw{Janas} in
$h$ bezogen. \fw{Janas} und \fw{Katze} befinden sich zwar in verschiedenen
F-Strukturen\is{funktionale Struktur} ($g$ und $h$), allerdings ist der
Possessor in $h$ Teil der Subjektfunktion $g$ (gleicher Satzteil). Das
Reflexivpronomen \fw{sich} in $j$ bildet das direkte Objekt von \fw{putzen}. Da
es sich bei dem Objekt um ein Reflexivpronomen handelt, ist es mit dem Subjekt
in $g$ koindiziert\is{Koindizierung} und stellt ebenfalls dessen
Kongruenztarget dar, allerdings steht \fw{sich} als Objekt in einer anderen
F-Struktur\is{funktionale Struktur} als das Subjekt ($j$ gegenüber $g$). Da
aber Subjekt und Objekt vom gleichen Verb \fw{putzen} abhängen, befinden sich
beide dennoch innerhalb derselben F-Struktur\is{funktionale Struktur} $f$
(gleicher Satz).
...
\is{Floating Quantifier|(}

Wenn im Rahmen dieser Arbeit der Terminus \textit{Quantor} verwendet wird, dann
werden darunter solche \isi{Determinierer} eines nominalen Ausdrucks
...
zwei\is{Dual} im Kontext eindeutig identifizierbaren Elementen einer Gruppe
\autocite[vgl.][307]{keenan2006} und ist damit definit\is{Definitheit}
\autocite[265--268]{lyons1999}.

Als Modifikatoren\is{Modifikator} von Substantiven (und in manchen Fällen auch
Pronomen, zum Beispiel \fw{sie beide}, \fw{alle diese}) unterliegen Quantoren
wie \norm{bėide} im Mittelhochdeutschen\il{Mittelhochdeutsch} wie auch
\fw{beide} im Neuhochdeutschen\il{Neuhochdeutsch} der
\isi{Adjektivdeklination}. Das bedeutet, sie kongruieren mit ihrem nominalen
\isi{Controller} formal in den Kategorien Kasus (\textsc{case}), \isi{Genus}
(\textsc{gend}) und \isi{Numerus} (\textsc{num}) über ein fusionales Suffix,
das diese Eigenschaften kombiniert kodiert \autocites(vgl.~auch
\sectref{sec:ctrltarg},
...
in der syntaktischen Position von \fw{beide} weitere Wörter vom gleichen Typ
unterzubringen, weder durch \isi{Koordination} \REF{ex:beidedet_3} noch durch
Reihung \REF{ex:beidedet_4}, wie es etwa bei Adjektiven\is{Adjektiv!attributiv}
trotz Restriktionen bezüglich ihrer Abfolge grundsätzlich möglich ist. Im
Unterschied zu Adjektiven\is{Adjektiv!attributiv} können Quantoren vom Typ
...
nominalen Typs repräsentieren. Anders als zum Beispiel \fw{all} und das
englische\il{Englisch} \fw{both} `beide', die ganz links in der
\isi{Nominalgruppe} stehen (\fw{all die \dots}, \fw{both the \dots}), reiht
sich die Quantorenphrase (QP) mit \fw{beide} im Standarddeutschen gewöhnlich
zwischen DP\is{Determiniererphrase} und NP\is{Nominalphrase} ein, wie in
\figref{fig:nomstack} dargestellt (vgl.~auch \cite[44--45 mit Fußnote
30]{lyons1999}). Als funktionale nominale Kategorie ist \xhead{Q} wie \xhead{D}
aus Sicht der LFG\is{Lexical-functional Grammar} ein Ko-Kopf\is{Kopf} von
\xhead{N} (\fw{cohead}; \cite[124]{bresnanetal2016}). Dies bedeutet, dass die
Information in DP und QP mit NP in derselben F-Struktur\is{funktionale
Struktur} vereinigt wird. Zusätzlich wird hier angenommen, dass der Quantor
zumindest bei seiner Verwendung innerhalb der DP der Quantor als
Merkmal\is{Merkmale!grammatische} (\textsc{quant}) definiert, da er aufgrund
des distributiven Unterschieds zu Adjektiven nicht als Teil der Menge der
Adjunkte\is{Adjunkt} (\textsc{adj}) gezählt werden sollte.
...
bestimmten\is{Definitheit} Bezugskontext benötigt. Das Paar, das der Quantor
...
(\isi{Demonstrativpronomen}) und \fw{meiner} (Possessivpronomen) in
\REF{ex:quantpron_2}, insofern auch sie definit\is{Definitheit} sind und
...
getrennt von ihrer Bezugs-NP\is{Nominalphrase} rechts vom Verb stehen, keine
Adverbien\is{Adverb} sind, wie zuvor vorgeschlagen wurde, sondern in einem
anaphorischen Verhältnis\is{Anapher} zu der NP stehen, die sie modifizieren
\autocite[428--433]{sportiche1988}. Ferner werde nicht der Quantor nach rechts
verschoben (\q{gefloatet}), sondern die Subjekt-NP aus ihrer Position hinter
dem Quantor aus der Verbphrase extrahiert, also nach vorn gezogen. Dabei werde
eine Spur (\fw{trace}; t) zurückgelassen, was die anaphorische
Relation\is{Anapher} zwischen Quantor und Quantifiziertem erkläre
...
Quantifiziertes eine Quantorenphrase mit dem Quantor als \isi{Kopf} und einer
Determiniererphrase\is{Determiniererphrase} (DP) als \isi{Komplement} bilden.
\citet{merchant1996} überträgt die Erkenntnisse von \citet{sportiche1988} und
\citet{shlonsky1991} auf das Deutsche\il{Neuhochdeutsch} und nimmt weitere
Verfeinerungen ihrer Hypothese bezüglich möglicher Positionen des gestrandeten
Quantors vor. Für das Englische\il{Englisch} \fw{all} `alle' mit Bezug auf das
Subjekt gibt \citet{merchant1996} grob die \isi{Konstituentenstruktur} in
...
Unterschied wird in \figref{fig:siebeidefhrd_1} und \ref{fig:siebeidefhrd_2}
...
\figref{fig:siebeidefhrd_2} zusätzlich die Interpretation zu, nach der beide
...
\is{Government and Binding} \autocite{chomsky1981}. Wie könnte eine Adaption in
die LFG\is{Lexical-functional Grammar}\is{Lexical-functional Grammar} aussehen?
...
dem Quantifizierten koindiziert\is{Koindizierung} ist.%
%
...
besonders markiert\is{Markiertheit}. Beide NPs, Topik (\textsc{top}) und
...
Stellungsvariante\is{Distribution!syntaktische} vermerkt: Die Kongruenzendung
fehlt, wenn der Quantor einer definiten\is{Definitheit} NP\is{Nominalphrase}
vorangeht: \fw{all die X} aber \fw{alle X}. Der Fall von \fw{die beiden X} und
\fw{beide X} würde eine separate Diskussion benötigen. Nachfolgend möchte ich
...
Adjektive\is{Adjektiv!attributiv} normalerweise links vom Substantiv stehen,
können Adjektive im Mittelhochdeutschen\il{Mittelhochdeutsch} auch rechts davon
auftreten \autocite[185--186, 237--243]{ksw2}, selten auch \isi{Determinierer}
wie Possessivartikel\is{Artikel}, \norm{dehėin} `kein' und \norm{bėide}
...
\norm{jārƶīt} `Jahrestag' (\xhead{N}) zu behandeln.%
%
	\footnote{\isi{Artikel} und Quantor sind im ursprünglichen Beispiel nicht
		...
		dagegen neutral. Neutrum in Bezug auf einen unbelebten\is{Inanimata}
		Referenten lässt sich als semantische Kongruenz interpretieren, was die
		\isi{Lesart} in \figref{fig:beidepost_2dist} mit pronominal verwendetem
		\norm{bėidiu} plausibler macht.}
...
Schema in \figref{fig:beidepost_2dist} als Arbeits\-hypothese für die
\isi{Lesart} mit gefloatetem Quantor. Parallel zu Subjekt (\textsc{subj}) und
Topik (\textsc{top}) werden hier die Rollen Objekt (\textsc{obj}) und
\isi{Fokus} (\textsc{foc}) für \norm{bėidiu} und die dazugehörige
DP\is{Determiniererphrase} angenommen. Die aufeinander bezogenen Teile ($g$ und
$h$) sind koindiziert\is{Koindizierung} ($i$), um die anaphorische\is{Anapher}
...
%
	\footnote{Die Annotationsform\is{Annotation}
		...
		Funktion $x$ als \isi{Controller} existiert, die ein
		Attribut\is{Attribut} $a$ besitzt, das den Wert $v$ für sein Target
		voraussetzt (\fw{inside-out functional uncertainty};
		\cite[66--70]{bresnanetal2016}).}
%
Eine eingehendere Diskussion der Konstruktion müsste\is{Desiderat} auf die
Bindungsrelation\is{Bindung}, etwa im Vergleich zu Reflexivpronomina, und
...
syntaktischer Ambiguität\is{Ambiguität} führt. In den hier gesammelten Daten
tritt das Problem vor allem bei den zahlreichen Belegen für \norm{si bėide}
auf, wenn \norm{si} im \isi{Mittelfeld} steht (vgl.~\figref{fig:sibeideambig}).
\isi{Kontaktstellung} ist also nicht gleich \isi{Kontaktstellung}, da auch in
\isi{Kontaktstellung} ein gefloateter Quantor vorliegen kann.
\citet[623--624]{ksw2} benennen dieses Problem ebenfalls und zählen
\blockquote{solche Fälle immer als attributiv\is{Attribut} nachgestellt}, also
wie in \figref{fig:sibeideambig} rechts dargestellt.
...
\isi{Kontaktstellung}\is{Ambiguität}}
...
%
	...
	Beschränkung\is{Beschränkung} auf große europäische Sprachen typologisch
...
Distanz}\is{Distanz!syntaktische} zwischen \isi{Controller} und Target die
...
den von ihm untersuchten Genusausgleich relevanter als andere sind (\isi{Genus}
...
\isi{Genus}-Kongruenz folgende Konkretisierung vor:

\begin{exe}
\ex \isi{Artikel}, attributives \isi{Demonstrativpronomen} \\
	\hspace*{1em} > andere attributive Relationen \\
	\hspace*{2em} > Relativpronomen \\
	\hspace*{3em} > anaphorisches \isi{Demonstrativpronomen}, Personalpronomen \\
	\hspace*{4em} > anaphorisches\is{Anapher} Possessivpronomen
	\is{Attribut}
	\is{Prädikat}
	\is{Relativpronomen}
	\is{Anapher}
	\is{Possessivpronomen}
\end{exe}

Er betont den Sonderstatus des Relativpronomens, das sowohl eine
anaphorische\is{Anapher} als auch eine attributive\is{Attribut} Funktion hat,
und merkt weiterhin an, dass das \blockcquote[194]{fleischer2012}{attributiv
verwendete Adjektiv\is{Adjektiv!attributiv} und Partizip \textelp{} im
...
schreibt \citet[216]{corbett1979}, dass attributive Kongruenz\is{Concord} auf
die einfache Phrase beschränkt sei, prädikative
Kongruenz\is{Kongruenz!prädikative} die Phrase überschreite doch auf den
Teilsatz beschränkt bleibe, während Personalpronomen nicht auf den Satz der
Instanz beschränkt sei, die die Kongruenzbeziehung\is{Kongruenzrelation}
kontrolliert.
% %
...
bei Attributen\is{Attribut} zu erwarten ist, da innerhalb der
NP\is{Nominalphrase} \isi{Concord} das entscheidende
Kongruenzmerkmal\is{Merkmale!grammatische} darstellt. Hingegen ist semantische
...
\isi{Index} an ihren Referenten gebunden sind
...
Bezeichneten. Relativpronomina seien dagegen immer anaphorisch\is{Anapher}
gebunden -- wobei sich die Autoren hier vermutlich auf den innersprachlichen
Kontext beziehen -- und kongruieren in \isi{Index}-Merkmalen, möglicherweise
auch in \isi{Concord}-Merkmalen, da sie NP-intern\is{Nominalphrase}
auftreten.\is{Merkmale!grammatische} Aufgrund induktiver Kriterien ergibt sich
das Schema in \figref{fig:theoagrdist}.
...
\mr{5}{*}[1.5ex]{\rotatebox{-90}{lokal $\to$ distal}}
	& Attribute\is{Attribut} von \xhead{N}
	& \chk
	& %
	& %
	& %
...
kombinierten \isi{Controller} nach syntaktischen Kriterien keine Konjunkte im
formalen Sinn darstellen \autocite[vgl.~auch][247--248]{askedal1973}.
Nichtsdestoweniger können zwei formal getrennte \isi{Controller} $i$ und $j$ im
Diskurs existieren, auf die sich \norm{bėide} `beide' als Target gleichermaßen
bezieht. Dies ist in der Belegstelle in \REF{ex:disjointctrl_1} der Fall, in
der es um ein Geschäft geht, das \fw{Agneſ} und \fw{Lukas} gemeinsam getätigt
haben. Sie sagen \lit{bedi} `beide (\textsc{n.pl})' aus, die Zahlung erhalten
zu haben.%
%
...
Auf formaler Ebene werden \fw{Agneſ} und \fw{Lukas} aber nicht
koordiniert.\is{Koordination} Vielmehr liegt \lit{mit hern Lukas hant} `mit
Herrn Lukas' Hand' eine adverbiale\is{Adverbial} Präpositionalphrase vor, in
der \lit{Lukas} seinerseits ein Genitivattribut\is{Attribut} zu \lit{hant}
...
`haben (\textsc{3pl.ind.prs})', Pronomina wie \norm{ſie} `sie (\textsc{pl})'
und damit auch \lit{bedi} `beide' auf sie gemeinsam Bezug nehmen können.
...
\isi{Controller} in \REF{ex:disjointctrl_2} unter formalen Kriterien noch
lockerer. So bezieht sich \lit{ſi beidv̍} `sie beide (\textsc{n})' dort
...
`sie' vertreten, doch auch hier liegt keine Konjunktion vor, da \lit{ſi} das
Subjektpronomen und Heinrich das Objekt von \lit{neme} `nahm' darstellt. Beide
\isi{Controller} werden zwar syntaktisch unabhängig voneinander im Diskurs
...
\isi{Controller} bezieht, indem die \isi{Controller} im Kontext
