...
\ex \label{ex:adjsngl}
	...
	\trans `wie gute Ritter sollten'
		(Hartmann von Aue, \tit{Iwein}: \notecite[V.~5345]{mertens2004};
...
%
	\footnote{Siehe \citeurl{cao-online}. Die Digitalisierung der
		...
		\tit{Marburger Urkundenrepositorium} in Arbeit. Im \cref{sec:urkliste}
...
\KC{} (\KC{}; vgl.~\cites{schroeder1895,nellmann1983}), der
...
%
Wenn sich diese Studie exemplarisch auf die \KC{} als Beispiel für einen
breit überlieferten, literarisch gestalteten Text und ein zum Teil editorisches
Novum beschränkt, ist zu bedenken, dass auch der \tit{Iwein} Hartmanns von Aue
und der \tit{Parzival} Wolframs von Eschenbach eine vielfältige und komplexe
Überlieferung besitzen (\cite[vgl.][s.\,v.~\textit{Hartmann von Aue:
\tit{Iwein}}, \textit{Wolfram von Eschenbach: \tit{Parzival}}]{hsc} sowie die
dort verzeichnete Literatur). Diese ebenfalls in Gänze aufzuarbeiten, stellt
ein Desiderat dar, das mit einem Anschlussprojekt an die vorliegende Arbeit
...
\ex \label{ex:modgermbeide_1}
	\langinfo%
		{Isländisch}%
		{Nordgermanisch}%
		{\cite[nach][283]{corbett1991}}\\ %[569]{wechsler2009}}\\
...
`müde' auf \fw{drengurinn} `der Junge' und \fw{telpan} `das
Mädchen' in \cref{ex:modgermbeide_1} sowie \fw{vøkur} `schön' auf
...
\textcites[244--247]{delbrueck1900}[33--41]{behaghel1928} reißen dieses Thema
an. Im Vordergrund steht dabei die Beobachtung, dass in den historischen
...
\tit{Parzival} Wolframs von Eschenbach und des \tit{Tristan} Gottfrieds von
Straßburg anhand der Edition von \citet{lachmannhartl1952} sowie
\citet{maroldschroeder1969}. \posscite{askedal1973} Belegsammlung ist mit knapp
fünfzig Stellen zum hier untersuchten \norm{bėide} allerdings nicht sehr groß.
Ein Anliegen der Belegauswertung in dieser Arbeit ist daher, unter Verwendung
moderner Methoden digital verfügbare, diplomatische Transkriptionen von
...
%
	\footnote{Belegstellen in \KC{}-Handschriften werden angegeben als:
		\textit{<Sigle>: <Blattnummer><Blattseite r/v>(<Spalte a--c>):%
		...
		sich nach \citetitle{kcdigital}, siehe unter
		\citeurl{kcdigital}\nocite{kcdigital}.%
	}
%
...
verwendete Terminologie (\cref{ch:terminologie}). Zur Einordnung in den
