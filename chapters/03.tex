...
\wdef{beide (\textsc{nom+acc.pl.m+f.st})} beziehungsweise \norm{bėidiu}
\wdef{beide (\textsc{nom+acc.pl.n.st})} in mittelhochdeutschen Quellen
...
\ex \label{ex:germbeide}
	...
	\ex \label{ex:germbeide_1}
		...
		\begin{taggedline}{%
			...
			vgl. Uppsala, Universitätsbibl., MS~DG~1, Bl. 124\tsup{r}, 1--9)%
			}
...
\cref{ex:germbeide_1,ex:germbeide_3} mit \lit{þo} \wdef{diese
(\textsc{pl.\NeutMF})} beziehungsweise \lit{baru} \wdef{nackt
(\textsc{pl.\NeutMF})} keine Formen von \wdef{beide} betroffen sind,
...
\wdef{beider (\textsc{gen.pl})} mit appositiver Konjunktion konnten in einer
...
%
	\footnotetext{Die Formen \lit{der boſten} und \lit{der besten} sind ambig
		...
		eines Substantivs wie \norm{dinc} \wdef{Ding, Sache (\textsc{n.pl})}.%
	}
