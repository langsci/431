...
\norm{bėidiu} `beide (\textsc{nom+acc.pl.n.st})' in mittelhochdeutschen Quellen
...
begründetes Phänomen darstellt, wie in \sectref{ch:diskussion} zu zeigen sein
...
hinweisende Pronomina ursprünglich im Neutr.\ Plur.\ \textins{stehen}}.
...
\citet{askedal1973} wird in \tabref{tab:askbeide} aufgeführt, insofern die Daten
...
\caption%
	[Belegverteilung bei \tit{Parzival} und \tit{Tristan} in
		\citet{askedal1973}]%
	{Belegverteilung von \norm{bėide} und \norm{bėidiu} bei Wolframs von
	 	Eschenbach\ia{Wolfram von Eschenbach} \tit{Parzival} und Gottfrieds von
	 	Straßburg\ia{Gottfried von Straßburg} \tit{Tristan} in
	 	\citet{askedal1973}}
\begin{tabular}{
	l l
	c
...
sich \q{direkt} auf den unmittelbaren Bezug von \norm{bėide} auf zwei nominale
Größen und \q{indirekt} auf den mittelbaren Bezug über ein Pronomen.
...
zwei im Kontext bereits etablierte \REF{ex:askedal73pr_1} beziehungsweise
...
als Resolutionsgenus besitzt \REF{ex:icelgendres}, zeigt er, dass grammatisch
...
zusammen (vgl.~auch \tabref{tab:ahd_stradj} zum Althochdeutschen).
...
Beispiel \REF{ex:focpart1_1} beschränkt \fw{nur} die Aussage dahingehend, dass
...
Ähnlich verhält es sich bei \REF{ex:focpart2_1}, wo \fw{entweder} explizit
...
\REF{ex:focpart1_2} und \REF{ex:focpart2_2} verdeutlichen, dass die vom Adverb
...
Referenzen]{ksw2}. Eine Recherche im \citetitle{ddd} (\tit{ReA};
...
(\ref{ex:beidejohahd_1}--\ref{ex:beidejohahd_3}) für das (Spät-)Althochdeutsche
ergeben. \citet{braune2018} und \citet{schrodt2004} bieten keine Beispiele
dies\-bezüglich.
...
In \REF{ex:beidejohahd_2}
suggeriert der Punctus (\lit{·}) als Sprechpause ebenfalls eine kataphorische
Interpretation, zumal \lit{pêide} `beide' hier ebenfalls im Genus mit den
...
\lit{béidíu} `beide' sowohl Katapher als auch Konjunktion sein kann. Ob
\lit{béidíu} in diesem Fall tatsächlich nach dem Genus der Konjunkte flektiert,
...
Auffällig an \REF{ex:beideintiahd_3} ist, dass \lit{pediu} `beide' schon mit
Präpositionalphrasen steht (\lit{in demo lihnamen} `im Körper', \lit{in demo
muôte} `im Geist'), die als solche keine Personenmerkmale definieren, mit denen
...
Kontexte wie in \REF{ex:mhdbeideunde2}, in denen Substantive koordiniert
...
\tit{Corpus der altdeutschen Originalurkunden} (\CAO) nicht gefunden werden.
