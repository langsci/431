...
\tit{Mittelhochdeutschen\il{Mittelhochdeutsch} Grammatik} von \citet{ksw3,ksw2}
...
Die Schreibweise \orth{i} für den unbetonten Nebensilbenvokal
...
findet sich \lit{-i} in der Stichprobe zu \norm{bėide} `beide' typischerweise
in Kontexten, in denen ursprünglich \norm{-iu} /yː/ vorlag. Um sicherzustellen,
dass \lit{-i} in alemannischen Urkunden tatsächlich die ebenfalls dort typische
Entrundung von \norm{-iu} darstellt und nicht regelmäßig auch an anderen
...
Modell von \citet{schmid2019} abgefragt. Wie in \sectref{subsec:cao_sample} zum
Aufbau der Stichprobe zur \isi{Adjektivdeklination} im \tit{Corpus der
altdeutschen Originalurkunden} (\CAO) beschrieben, wurden auch hier jeweils die
...
Formenbestimmung der Annotations\-algorithmus\is{Annotation} mit mindestens
...
geografische Variation der Grafie des unbetonten Vokals in der Stichprobe. Gut
zu sehen ist, dass \norm{e} und die Syn- oder \isi{Apokope} des Vokals -- wie
beispielsweise bei \lit{hern} `Herrn' und \lit{hoͤrnt} `hören
(\textsc{3pl.ind.prs})' sowie \lit{fron} `Frau (\textsc{obl.sg}), Frauen' und
\lit{vmb} `um' -- vorherrschen. Schreibungen mit anderen Vokalen kommen nur
selten vor, zum Beispiel \lit{herrun} `Herrn, Herren'
(althochdeutsch\il{Althochdeutsch}-oberdeutsch\il{Oberdeutsch} \norm{hērrun},
vgl.~\cite[283]{braune2018}) oder \norm{frowan} `Frauen'
(althochdeutsch\il{Althochdeutsch} \norm{frouwūn}). Darüber hinaus liegen noch
einige Fälle von \lit{frown} vor, bei denen der Nebensilbenvokal vermutlich
synkopiert ist, jedoch die Schreibung \orth{w} für \norm{wu} nicht strikt
ausgeschlossen werden kann \autocite[vgl.][142]{paul2007}. Ein großer
...
\caption{Geografische Verteilung\is{Distribution!geografische} der Stichprobe
	zur Schwa-Grafie (Hintergrundkarte: \cite{wiesinger1983:rede})}
...
Die Form \lit{ani} für mittelhochdeutsch\il{Mittelhochdeutsch} \norm{āne},
althochdeutsch\il{Althochdeutsch} \norm{ānu} `ohne', ist ein einziges Mal in
Konstanz belegt und mit ihrem Kontext in \REF{ex:konst_ani} aufgeführt.
...
Auch \lit{umbi} `um', das zur Präposition
mittelhochdeutsch\il{Mittelhochdeutsch} \norm{umbe},
althochdeutsch\il{Althochdeutsch} \norm{umbi}, gehört, tritt mit 4 Belegen nur
...
Belege aus Colmar wird in \REF{ex:col_umbi} zitiert.
...
\autocites(1297)[15,32--16,4]{cao4} außer dem einen Beleg für \lit{vmbi} `um'
(15,39) an allen sechs anderen Stellen die Form \lit{vmbe} steht und \lit{i}
auch sonst nicht als Grafie für Schwa dient.

Ähnlich schwach bezeugt sind Formen des Typs \lit{frowin} zu
mittelhochdeutsch\il{Mittelhochdeutsch} \norm{vrouwen} `Frau (\textsc{obl.sg}),
Frauen', die sich auf 3 Belege aus Colmarer und 2 aus Baseler Urkunden
verteilen. Wie zu erwarten, stehen diese Belege im Dat./Gen.~Sg. sowie im Nom.\
und Dat.~Pl., der im Althochdeutschen\il{Althochdeutsch} in allen Fällen
\norm{frouwūn} lautete. Ein Beispiel wird in \REF{ex:col_vrouwin} gegeben. Die
Form kommt allein in dieser Urkunde noch zwei weitere Male vor, was allen
Colmarer Belegen entspricht. Über alle Ausstellungsorte in der Stichprobe
hinweg endet der Nom.~Sg.\ des Lemmas \norm{vrouwe} `(Edel-)Frau' in 64,6\,\%
der Fälle auf \lit{-e}, in 31,5\,\% auf -Ø und in lediglich 3,9\,\% auf
\lit{-a} (vgl.~althochdeutsch\il{Althochdeutsch} \norm{frouwa}), jedoch nie auf
\norm{-i}.
...
Die Form \lit{herrin}, mittelhochdeutsch\il{Mittelhochdeutsch} \norm{hērren}
`Herrn, Herren', die zurückgeht auf althochdeutsch\il{Althochdeutsch}
\norm{hērron, -un} (Akk.~Sg.\ und im Pl.) beziehungsweise \norm{hērrin}
(Gen./Dat.~Sg; \cite[vgl.][282--283]{braune2018}), ist in der Stichprobe mit 43
Belegen vertreten, nämlich in St.~Gallen (12 Belege), Basel (11), Freiburg
i.\,Br. (5), Zürich (3), Colmar (2), Konstanz und Staufen i.\,Br. (je 2) sowie
in Guebwiller, Kl.~Olsberg, Kl.~Töss und Mulhouse (je 1). Auffällig ist, dass
...
wird. Daneben tritt noch je einmal \lit{herri} zu
mittelhochdeutsch\il{Mittelhochdeutsch} \norm{hērre},
althochdeutsch\il{Althochdeutsch} \norm{hērro} `Herr', in Colmar und Überlingen
auf, wie in \REF{ex:col_herri} gezeigt. Darüber hinaus steht in der gesamten
Stichprobe in 87,4\,\% der Fälle im Nom.~Sg.\ eine Form vom Typ \lit{her}. Der
Typ \lit{herre} kommt mit 12,5\,\% wesentlich seltener vor, während sich die
...
Typ \lit{hörint} zu mittelhochdeutsch\il{Mittelhochdeutsch} \norm{hȫrent},
althochdeutsch\il{Althochdeutsch} \norm{hōrėnt} `hören (\textsc{3pl.ind.prs})'.
...
1284)[57,9--57,35]{cao2}. Die Wortform \norm{hȫrent} kommt stereotyp in der
Promulgatio, also der Verkündigungsformel nahezu aller Urkunden im \CAO{}
vor, wie in \REF{ex:fribr_hoerint} exemplarisch angeführt.
...
Insgesamt betrachtet kommt \orth{i} in der Stichprobe zwar als Schreibweise für
einen unbetonten Nebensilbenvokal vor, liegt aber in der Häufigkeit weit hinter
Schreibweisen mit \orth{e} zurück. Auffällig ist, dass \orth{i} in der
...
unbetonter Stellung vor Konsonant: \lit{herr\emph{in}}, \lit{frow\emph{in}},
\lit{hör\emph{int}}. Darüber hinaus zeigt gerade \lit{herrin} `Herren, Herrn'
als schwaches Maskulinum, dass \orth{i} an diesen Stellen unabhängig vom
Lautwert der entsprechenden Kasusform im Althochdeutschen\il{Althochdeutsch}
steht \autocite[vgl.][282--283]{braune2018}. Umgekehrt darf also davon
ausgegangen werden, dass Formen vom Typ \lit{beidi} `beide' nicht
typischerweise \lit{-i} für Schwa enthalten, zumal in keiner der betroffenen
Urkunden (\cites(Nrn.~81, 190)[124,18--33; 205,30--45]{cao1};
...
Ausstellungsort in der Stichprobe zur Adjektivflexion\is{Adjektivdeklination}
in \sectref{sec:adjdeclcao} auf.
...
jeder genannten Signatur ein Eintrag im \tit{HSC}.\\
