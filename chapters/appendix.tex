...
(althochdeutsch-oberdeutsch \norm{hērrun}, vgl.~\cite[283]{braune2018}) oder
\norm{frowan} `Frauen' (althochdeutsch \norm{frouwūn}). Darüber hinaus liegen
noch einige Fälle von \lit{frown} vor, bei denen der Nebensilbenvokal
vermutlich synkopiert ist, jedoch die Schreibung \orth{w} für \norm{wu} nicht
...
Auch \lit{umbi} `um', das zur Präposition mittelhochdeutsch \norm{umbe},
althochdeutsch \norm{umbi}, gehört, tritt mit 4 Belegen nur selten in der
...
\norm{vrouwe} `(Edel-)Frau' in 64,6\,\% der Fälle auf \lit{-e}, in 31,5\,\% auf
-Ø und in lediglich 3,9\,\% auf \lit{-a} (vgl.~althochdeutsch \norm{frouwa}),
jedoch nie auf \norm{-i}.
...
Die Form \lit{herrin}, mittelhochdeutsch \norm{hērren} `Herrn, Herren', die
zurückgeht auf althochdeutsch \norm{hērron, -un} (Akk.~Sg.\ und im Pl.)
beziehungsweise \norm{hērrin} (Gen./Dat.~Sg;
...
\norm{hērre}, althochdeutsch \norm{hērro} `Herr', in Colmar und Überlingen auf,
wie in \REF{ex:col_herri} gezeigt. Darüber hinaus steht in der gesamten
...
Typ \lit{hörint} zu mittelhochdeutsch \norm{hȫrent}, althochdeutsch
\norm{hōrėnt} `hören (\textsc{3pl.ind.prs})'. Die Belege verteilen sich auf die
...
\norm{hȫrent} kommt stereotyp in der Promulgatio, also der Verkündigungsformel
nahezu aller Urkunden im \CAO{} vor, wie in \REF{ex:fribr_hoerint} exemplarisch
angeführt.
