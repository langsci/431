...
damit durchschnittlich 453 Wortformen vor, die
\isi{Standardabweichung} beträgt 500 Wortformen.
...
(determinierender\is{Determinierer} Quantor, Konjunktion) geschieden. Diese
Zuordnung geschah manuell in einem zweiten Schritt.
...
Zu allen Kongruenztargets wurden die \isi{Controller} erfasst -- im Fall von
Targets in unmittelbarer Abhängigkeit von pronominalen
Controllern\is{Controller} diese sowie die letzte Nennung der Referenten, auf
die sie verweisen\is{Anapher} (\q{Erstcontroller}). Alle \isi{Controller} und
Targets wurden anschließend nach ihren formalen und semantischen
...
Wortformen und die \isi{Domäne} der Kongruenzbeziehung (gleiche Phrase,
gleicher Teilsatz, anderer Satz) wurden registriert.
...
ergibt; die \isi{Standardabweichung} liegt bei 34.652 Wortformen.
...
als Teil der korrelativen Konjunktion \norm{bėide \dots\ unde} `sowohl \dots\
als auch' vor. Dass nur sehr wenige attributive\is{Attribut} Belege in der
Stichprobe enthalten sind, macht den Vergleich zu den Urkunden des \CAO{}
...
Verfügung, von denen 70 attributiv\is{Attribut} sind und lediglich sechs Teil
der korrelativen Konjunktion. Die Belegsammlung zur \KC{} lässt sich so als
Gegengewicht zu derjenigen des
\CAO{} verstehen, nicht nur, was die Textgattung betrifft. Die
\tabref{tab:beidevar} schlüsselt die Zahl der gesammelten Belege nach
...
Trotz des Vorbehalts zur zahlenmäßigen Verteilung\is{Distribution!statistische}
der Belege auf die unterschiedlichen syntaktischen
Kontexte\is{Distribution!syntaktische} ist gerade der Vergleich der
bairischen\il{Bairisch} Handschrift B1 zum Urkunden\-material interessant, da
B1 die einzige der neun untersuchten Handschriften ist, in der im Kontext des
kombinierten gemischtgeschlechtlichen Bezugs beim Quantor
...
Transkription der Textstelle wurde am Digitalisat\is{Digitalisat} der
...
Rückbezug der Konjunktion \lit{bėide} auf zwei Konjunkte nahelegen, ließen sich
im exzerpierten Material keine finden. Da keine zwei Könige explizit genannt
...
einer determinierten\is{Determinierer} NP liegen in der Stichprobe lediglich
vier vor. Davon enthält der einzige Beleg im Akkusativ \REF{ex:maccadj_1}
unflektiertes \lit{gut} `gut', gegenüber flektiertem \lit{guten} `guten' bei
Voranstellung, wie exemplarisch in \REF{ex:maccadj_2} gezeigt. Die Handschrift
A1 enthält an dieser Stelle % [31\vb, 29]
...
Nachstellung eine Tendenz zur \isi{Endungslosigkeit} vor. Dies wird bestätigt
...
\isi{Endungslosigkeit} von \lit{paid} in \REF{ex:chindpaid_1} wird vermutlich
...
Hiatuspositionen bei der Rezitation apokopiert\is{Apokope} werde und
möglicherweise hyperkorrekt dafür \orth{e} geschrieben werde
\autocite[90--91]{askedal1973}.
...
Mit Ausnahme\is{Ausnahme} der Handschrift K wurden keine \KC{}-Text\-zeugen mit
