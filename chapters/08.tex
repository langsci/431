...
%
	\footnote{Die Liste der Handschriftensiglen befindet sich im
		...
		\citeurl{kcdigital}. \KC{}-Versnummern richten sich nach der
...
\includegraphics[
	height=.75\textheight,
]{./figures/ueberlieferungszeitraeume.png}
...
\sn \gll Willehalm vnd Dietreich. \\
...
\sn \gll wurden baíde da erſlagen. \\
	...
	\begin{taggedline}{\parencite[\pno~83vb, 36--37]{kc:C1}}
		`Willehalm und Dietrich wurden beide dort erschlagen.'
	\end{taggedline}
\end{exe}
...
\ex \label{ex:dietwill} % 203
	...
	% 	\lit{Dietrich vnd Willehalm \\
	% 	Wurden zewurmtz peid erſlagen.}
	% 	\begin{taggedline}{\parencite[\pno~124va,3--4]{kc:M}}
	% 	\trans `Dietrich und Willehalm wurden in Worms beide erschlagen.'
	% 	\end{taggedline}
	...
	% 	\begin{taggedline}{\parencite[\pno~326ra,23--24]{kc:Z}}
	% 	\lit{Wilhalm vnd dietreiche \\
	% 		Wurden baide da erſlagen}
	% 	\end{taggedline}
	\end{xlist}
\end{exe}
...
% \norm{Lâterân} `Lateran' miteinander kombiniert. In beiden Fällen
% handelt es sich um Orte, also um Inanimata.

% \begin{exe}
% \ex \label{ex:romelat} % 211
% 	\begin{xlist}
% 	% \ex \begin{taggedline}{\parencite[\pno~49vb,14--16]{kc:A1}}
% 	% 	\lit{rome
% 	% 		unt lateran. \\ woͮrden im baide under%
% 	% 		tan.}
% 	% 	\end{taggedline}
% 	%	\label{ex:romelat_1}

% 	\ex \begin{taggedline}{\parencite[\pno~31vb,18--19]{kc:B1}}
% 		\lit{Rom vnd lateran \\
% 			wurden im peid vndertan}
% 		\end{taggedline}
% 		\label{ex:romelat_2}

% 	\ex \begin{taggedline}{\parencite[\pno~82va,45--46]{kc:VB}}
% 		\lit{Rome vnd lateran \\
% 			Wurden ím bede vndertan.}
% 		\end{taggedline}
% 		\label{ex:romelat_3}

% 	\ex \begin{taggedline}{\parencite[\pno~68vb,13--14]{kc:K}}
% 		\lit{Rome vnd Lateran \\
% 			Wurden ím baidú vndertan}
% 			\label{ex:romelat_4}
% 	\end{taggedline}

% 	% \ex \begin{taggedline}{\parencite[\pno~229ra,6--7]{kc:Z}}
% 	% 	\lit{Rome vnd latran \\
% 	% 		Wurden Im baide vndertan}
% 	% 	\end{taggedline}
% 	%	\label{ex:romelat_5}
% 		\trans `Rom und der Lateran wurden ihm beide untertan.'
% 	\end{xlist}
% \end{exe}
...
% 	\trans `Der [=~Nero] baute den großen Lateran.'
% 	\end{taggedline}
% \end{exe}

% Anders als im modernen Sprachgebrauch handelt es sich hier bei \lit{Latoran}
% `Lateran' um ein Femininum \autocite[\FemI;][182]{ksw2}. Diesen einen
...
% das nächste Wort \norm{undertân} `untertan' fängt mit einem Vokal an.
...
% den unterschiedlichen \KC{}-Handschriften phonologische Faktoren
...
\cref{ex:beidplsubst} diskutiert. Das Target \lit{bêde} `beide' ist hier
...
%	\begin{xlist}
%	\ex Vorangestellt: \\
%		\lit{Die baide gottes botten}
%		\begin{taggedline}{\parencites[\pno~155ra,24]{kc:Z}[vgl.][7845]{schroeder1895}}
%		\trans `die beiden Gottesboten'
%		\end{taggedline}
%		\label{ex:beidplsubst_1}

%	\ex Nachgestellt: \\
%		\lit{di gotes boten bede}
%		\begin{taggedline}{\parencites[\pno~33vb,24]{kc:A1}[vgl.][7845]{schroeder1895}}
%		\trans `die beiden Gottesboten'
%		\end{taggedline}
%		\label{ex:beidplsubst_2}

%	\ex Gefloatet (Distanzstellung): \\
%		\lit{Die lieben her geſellen. \\
%			Wonten da vber naht beide.}
%		\begin{taggedline}{\parencites[\pno~100va,24--25]{kc:VB}[vgl.][15038--15039]{schroeder1895}}
%		\trans `Die lieben Kampfgefährten wohnten da beide über Nacht.'
%		\end{taggedline}
...
\ex \label{ex:richtherriu}
	...
	\ex \gll Die rihtær ſprachen beideu {dar zuͦ} \\
			...
			dazu \\
		\begin{taggedline}{\parencites[\pno~28ra,8]{kc:B1}[vgl.~abweichend][10090]{schroeder1895}} % 1140 mit Parallelstelle in H
	...
	\sn \gll Beideu beſvnder \\
		...
		\begin{taggedline}{\parencites[\pno~31va,48--49]{kc:B1}[vgl.][11385--11386]{schroeder1895}} % 1112x
...
% 	\lit{Do chavft ſie dív chint peide}
% 	\begin{taggedline}{\parencites[\pno~8ra,9]{kc:VB}[vgl. stark abweichend][1450]{schroeder1895}} % 1158x
% 	\trans `Da kaufte sie die Kinder beide'
% 	\end{taggedline}
% 	% → -e belegt für ACC.PL.N.ST, -iu überwiegt leicht
% \end{exe}

% In \cref{ex:chintpeide} bezieht sich \lit{chint} `Kinder' auf die beiden
% Brüder \lit{Nyceta} und \lit{Aquila} \autocites[7\vb,
% 30--31]{kc:VB}[vgl.][1428--1429]{schroeder1895}, insofern ist der männliche
% semantische Bezug hier gesichert. In der Stichprobe zu VB sind für
...
% System der Handschrift VB normal verhält.
...
% \cref{ex:baideuwarn} in K \cref{ex:baideuwarn2} die an sich
% neutrale Form \lit{baidu̍} mit direktem Bezug auf \lit{babſt} `Papst'
% und \lit{ím} `ihm' (=~\lit{Karle} `Karl der Große') als
...
% 	\lit{Karle an daz gerichte ſaz \\
% 		Der babſt klegt ím daz \\
% 		Der wideme vnd der zehende gar \\
% 		Waͤren baidu̍ worden bar}
% 	\begin{taggedline}{\parencites[\pno~85vb,22--24]{kc:K}[vgl. abweichend][14382--14385]{schroeder1895}}
% 	\trans `Karl setzte sich zu Gericht. Der Papst klagte ihm, dass beide an
...
\ex \label{ex:beideuher}
	...
	\begin{taggedline}{\parencites[\pno~31rc,3]{kc:B1}[zu][11272\psqq]{schroeder1895}} % 1110
...
% 	\begin{xlist}
% 	\ex \lit{und die nag%-
% 			el die man durch ſine baide uoͮze ſlu%-
% 			ch.}
% 		\begin{taggedline}{\parencites[\pno~45va,11]{kc:A1}[10390]{schroeder1895}}
% 		\trans `und die Nägel, die man durch seine beiden Füße schlug'
% 		\end{taggedline}
...
% \lit{ſine} `seine' (\Acc.\Pl.\F.\St) der nachfolgende Quantor ebenfalls stark
...
% `beide'-Target ist in diesem Kontext also mittel\-bar.
...
\sn \gll Si nâmen di muoter mit dem sun, \\
...
\norm{kindelīn} `Kindlein' \autocite[910--932]{schroeder1895}. Das
...
% \autocites[\pno~14ra,13]{kc:VB}[\pno~9va,1]{kc:B1}[vgl.][2757]{schroeder1895}.%
% %
...
baidev verloͤrn} `dann wären wir beide verloren' \autocite[60rb,44--60va,3]{kc:C1}.
...
zur \KC{} keine Variation vor. In allen neun Fällen sind
...
\sn \gll do die hêrren pranten \textelp{} \\
...
\sn \gll da ze Kastel er si baide begraif \\
		...
		\trans `Als die Herren brandschatzten \textelp{' Dort in Kastel ergriff er sie beide}
		\end{taggedline}
\end{exe}
...
\begin{table}
\centering
\caption{Kombinationen von \norm{sie/siu/die/diu bėide/-iu} in der \KC{}}
\begin{tabular}{
	l
	@{\hspace{4\tabcolsep}}
	r
...
Zwei Belege für \norm{sie bėide} liegen \citet[\pno~21ra,25--31 und 22ra,34--22rb,6]{kc:VB} vor; dazu \citet[4255--4261]{schroeder1895} und
...
% 	Nychea und Aquila. \\
% 	Die warn dez peide vil vro}
% 	\begin{taggedline}{\parencite[\pno~7vc,30]{kc:B1}}
% 	\trans `Da kamen auch Niceta und Aquila zu St.~Petrus, die waren darüber
% 	beide sehr froh.'
% 	\end{taggedline}
% \end{exe}
...
% 		& $\Tsg.\MascM + \Tsg.\MascM$
% 		& \autocites[\pno~16vb,29]{kc:A1}[3938]{schroeder1895}
% 		\\

% 	%
% 		& die herrren bæde
% 		& $\Tsg.\MascM + \Tsg.\MascM$
% 		& \autocites[\pno~18ra,41]{kc:A1}[4250]{schroeder1895}
% 		\\
	
% 	%
% 		& di gotes boten bede
% 		& $\Tsg.\MascM + \Tsg.\MascM$
% 		& \autocites[\pno~33vb,42]{kc:A1}[7845]{schroeder1895}
% 		\medskip\\
	
% 	b.
% 		& bædiv gvͦten vn̄ ubelen\footnote{Hier mit nachgestellten attributiven Adjektiven mit Bezug auf \lit{von den bæſten. un̄ uon den chunigen} `von den Päpsten und von den Königen' \autocites[\pno~1ra,17--18]{kc:A1}[19]{schroeder1895}.}
% 		& $\Tpl.\MascM + \Tpl.\MascM$
% 		& \autocites[\pno~1ra,18]{kc:A1}[20]{schroeder1895}
% 		\\

% 	%
% 		& bediu wip unt man
% 		& $\Tsg.\FemF + \Tsg.\MascM$
% 		& \autocites[\pno~3va,1]{kc:A1}[19]{schroeder1895}
% 		\\

% 	%
% 		& bediv kint vnt wip
% 		& $\Tsg.\NeutA + \Tsg.\NeutF$
% 		& \autocites[\pno~4rb,33--34]{kc:A1}[861]{schroeder1895}
% 		\\

% 	%
% 		& bedíu kunſt unt uernunft
% 		& $\Tsg.\FemI + \Tsg.\FemI$
% 		& \autocites[\pno~10va,37--38]{kc:A1}[2435]{schroeder1895}
% 		\\

% 	%
% 		& bediv groz unde ſtarc\footnote{Hier mit prädikativen Adjektiven mit Bezug auf \lit{antwerc} `Maschine (?)' \autocites[\pno~32vb,34]{kc:A1}[7586]{schroeder1895}.}
% 		& $\Tsg.\NeutI$
% 		& \autocites[\pno~32vb,34]{kc:A1}[7587]{schroeder1895}
% 		\\
% 	\end{tabular}
% \end{exe}
...
\norm{sine} $\gets$ \norm{si ne} `sie \Neg') wurden dabei nicht gezählt.
...
%
	...
	% Einfluss in VB dar, wenn man eine durch Phonemzusammenfall
...
% 		\node [wordbox, base right=1ex of 2       ] (3) {Lāterān};
% 		\node [wordbox, base right=3ex of 3       ] (4) {wurden};
% 		\node [wordbox, base right=3ex of 4       ] (5) {im};
% 		\node [wordbox, base right=1ex of 5, thick] (6) {bėide};
...
% 		\node [word, base right=1ex of 2] (3) {Lāterān};
% 		\node [word, base right=1ex of 3] (4) {wurden};
% 		\node [word, base right=1ex of 4] (5) {im};
% 		\node [word, base right=1ex of 5] (6) {bėide};
...
K vor \cref{ex:baideuwarn3}. Die Form \lit{baidu̍} im Nebensatz
...
% \cpageref{ex:chintpeide}). Das Neutrum \lit{chint} `Kind' bezieht sich
...
% \norm{-iu} tritt in der \KC{}-Stichprobe in dieser Position ohnehin
...
% Adjektivflexion in den untersuchten \KC{}-Textzeugen zufolge ist in
% VB nachgestelltes
% % \norm{guot}
% \norm{guet} `gut' ohne Flexion die Regel in
% NPs mit Definitartikel; \norm{arm} `arm' tritt in diesem Fall dagegen
...
% \norm{wîp}
\norm{wīp} `Frau' in direkter Rede auf
sich und sein
% \norm{kindelîn}
...
%	\begin{taggedline}{\parencite[\pno~109va,28--29]{kc:VB}}
	\begin{taggedline}{\parencite[\pno~109va,29]{kc:VB}}
%	\trans `Sie hatten sehr viel an Heer, sowohl Ritter als auch Bauern.'
...
% sind sowohl in A1 als auch in VB beide Formen zu
% finden, obwohl in A1 die Verhältnisse bei der Adjektivdeklination
...
\lit{\textins{\lit{da}}rzuͦ man vn̄ wip} `dazu Männern und Frauen'
...
\ex \label{ex:kczbeidenunde_1}
	...
	\sn \gll Gebott er allen an den lyb \\
		...
		\begin{taggedline}{%
			...
			[\pno~3va,19--20]{kc:C1}% beidev man vnd weip.
			[\pno~3va,22--23]{kc:K}% [da]rzuͦ man vnd(e) wip
			[abweichend][\pno~3va,1--2]{kc:A1}% bediu wip unt man.
			[\pno~5va,16--17]{kc:M}% Baidív wip vnd man.
			[\pno~3vb,10--12]{kc:H}% Bede und(e) man.
			[\pno~3vc,51--53]{kc:B1}% Baideu weip vnde man
			[\pno~3vb,5--7]{kc:VB}% Beidiv wip vnd man
			[\pno~6ra,8--10]{kc:P}% beide wip vnd(e) man.
			% [zu]
	...
	% 	\begin{taggedline}{%
	% 		\parencites[\pno~3va,1--2]{kc:A1}[vgl.]%
	% 		[\pno~5va,16--17]{kc:M}% Baidív wip vnd man.
	% 		[\pno~3vb,10--12]{kc:H}% Bede und(e) man.
	% 		[\pno~3vc,51--53]{kc:B1}% Baideu weip vnde man
	% 		[\pno~3vb,5--7]{kc:VB}% Beidiv wip vnd man
	% 		[\pno~6ra,8--10]{kc:P}% beide wip vnd(e) man.
	% 		[619--621]{schroeder1895}}
	% 	\trans `sowohl Mann als auch Frau, jeder müsse dahin reisen, wo
	...
	\sn \gll Sol es gan an den lÿb \\
		...
		\begin{taggedline}{%
			...
			[\pno~4vb,2]{kc:C1}% baidev man vnd weip.
			[\pno~4vb,33--34]{kc:K}% Baidu̍ man vnd(e) wip
...
\ex \begin{xlist}
	...
	% 	\lit{ſi macheten ir antwerc. \\ bediv
	% 	groz unde ſtarc.}
	% 	\begin{taggedline}{\parencites[\pno~32vb,34--35]{kc:A1}[vgl.][7586--7587]{schroeder1895}}
	% 		\trans `Sie machten ihr Belagerungsgerät sowohl groß als auch kräftig.'
	% 	\end{taggedline}
...
\ex \label{ex:syntintvar3}
	...
	\lit{Beideu ſtill vnd ûber laut}
	\end{taggedline}
