...
abhängt und deren Personenmerkmale\is{Personenmerkmal} durch Kongruenz
...
direkt mit dem Großteil der Urkundenbelege\is{Urkunde} vergleichen ließen, sind
abgesehen von K (mittelalemannisch\il{Alemannisch}) nur unter den Fragmenten zu
...
Zeit und Raum möglich, da diese -- im Unterschied zu Urkunden\is{Urkunde} --
keine derartigen Selbstauskünfte\is{Vorlagenproblematik} bieten
...
Jahrhundertviertel um 1300 und stehen damit zeitlich den Urkunden\is{Urkunde}
des \CAO{} nahe.
...
liegen zwei Belege vor; zusammen mit der Kombination von \isi{Substantiv} und
...
zwei Substantive\is{Substantiv} bezieht, verteilen sich auf nur drei
verschiedene Parallelstellen\is{Paralleltext}. Die Menge an Kombinationen von
...
Substantiven\is{Substantiv} ($\MascM+\FemF$, $\FemF+\MascM$) liegen im hier
...
Substantiv und Personal\-pronomen\is{Personalpronomen}, der in
\REF{ex:mutterdu} wiedergegeben wird.
...
Referent, nämlich der \lit{chûnig} `König', der direkt angesprochen wird. Der
Beleg passt damit in das Bild, das schon die Auswertung der Urkunden ergeben
hat. Auch bei Pro\-nomina ohne \isi{Genusmarkierung} tritt aufgrund der
...
Substantiven\is{Substantiv}, sondern nur um ein einzelnes Substantiv, das im
Plural steht.
...
Kongruenzbeziehung\is{Kongruenzrelation} mit einem einzelnen \isi{Substantiv}
im Plural steht, sind auch in der \KC{} im Vergleich zu Kontexten mit
kombinierter Referenz zahlreicher vorhanden. \tabref{tab:simpnomctrla} zeigt
ihre Verteilung\is{Distribution!statistische} nach Personenmerkmalen und
Flexionstyp.
...
das Fehlen von overter Flexion, die ansonsten in der \isi{Stichprobe} zu B1 für
den starken Nom./Akk.~Pl.~M./F. belegt ist (vgl.~\tabref{tab:kcadjdeclovw}).
...
zwei Substantive\is{Substantiv} fallen nicht aus dem Rahmen bisheriger
Ergebnisse, wenn es auch die geringe Belegzahl unmöglich macht, generelle
Aussagen zu treffen. In beiden Fällen zeigte sich die Form \norm{bėide} mit
...
der NP\is{Nominalphrase} ebenfalls formale Kongruenz auf, doch liegen zwei
Belege für neutrales \norm{bėidiu} bei eindeutig maskulin-männlichem Bezug vor.
...
Pronomen der ersten oder zweiten Person als Diskurs\-anker\is{Diskursanker}
...
Geschlecht des Kindes ist unbekannt; es muss sich dem Kontext\is{Pragmatik}
nach um einen Säugling handeln (vgl.~auch
...
\isi{Stichprobe} zur Adjektiv\-flexion\is{Adjektivdeklination}, wobei
...
\lit{beideu} in \REF{ex:wipkindelin_2} passt zum einen zur Kombination von zwei
formalen Neutra, zum anderen als Resolutionsform, insofern weiblich und
...
sich um unbelebte\is{Inanimata} Maskulina. Es scheint im Kontext\is{Pragmatik}
der Stelle sinnvoller, das neutrale \lit{beideu} in Zeile~34 nicht darauf,
sondern auf \lit{ſi} `sie' -- den \lit{papſt} `Papst' und den \lit{chv̂nich}
`König' -- zu beziehen. Der Blick in die Parallelstellen\is{Paralleltext} in VB
und A1 stützt diese Interpretation, insofern es hier trotz abweichendem
...
Beim indirekten Bezug zwischen einem einzelnen \isi{Substantiv} im Plural und
\norm{bėide} als Target wie in \REF{ex:nplsibeide} und \figref{fig:nplsibeide}
liegt in der \isi{Stichprobe} zur \KC{} keine Variation vor. In allen neun
...
einzig belegte. Belege mit Relativpronomen\is{Relativpronomen} als Controller
liegen lediglich drei vor, die sich auf zwei Stellen beziehen.
...
\lit{beideu} auf das Figurenpaar dem Kontext\is{Pragmatik} nach
wahrscheinlicher ist als auf \lit{ſchatz vnd gewín} `Schatz und Gewinn', auch
...
Quantors zu rechnen ist und die Belege daher nicht in die bereinigte
\isi{Stichprobe} eingeflossen sind. In den Handschriften A1 und M, die bei
...
\isi{Substantiv} im Plural: \norm{jungere} `Jünger'. Die Belege für \lit{bede}
...
einzelne Plural-Substantive\is{Substantiv} liegen insgesamt 17 Belege zu zwölf
...
\norm{-iu}. Beachtenswert erscheinen die zwei Belege mit \norm{bėidiu} in Bezug
auf ein Maskulinum im gleichen Teilsatz, die neutrales \norm{-iu} entgegen Form
und Semantik aufweisen.
...
Innerhalb der \KC{}-\isi{Stichprobe} ist aufgrund der geringen Belegmenge kein
Vergleich möglich. Die Handschrift B1 verhält sich aber zumindest zu den
Urkunden des \CAO{} kongruent, insofern ihre Version der Stelle die formal
neutrale Form \lit{beideu} enthält \REF{ex:wipkindelin2_2}, die hier am
wahrscheinlichsten als Resolutionsform in Ermangelung klarer semantischer
Hinweise auf die vorliegende Geschlechterkombination aufgefasst werden kann.
...
Kontext\is{Pragmatik} keinen Hinweis darauf gibt, ob eine kollektive oder eine
distributive \isi{Lesart} wahrscheinlicher ist (vgl.~\sectref{sec:floatquant}
zur Semantik von Floating Quantifiers\is{Floating Quantifier}).
\citet[623]{ksw2} folgend wurden diese unsicheren Fälle als
attributiv\is{Attribut} nachgestellt und damit als sich im gleichen Satzglied
wie ihr Controller befindlich gezählt.

Bei der Abhängigkeit des Quantors von einem \isi{Personalpronomen}, das sich
auf ein einzelnes Plural-\isi{Substantiv} bezieht (indirekte Abhängigkeit vom
...
hier wesentlich kürzer sind als bei den Urkunden\is{Urkunde}, daher liegen auch
weniger Distanzklassen vor; Klassen ohne Belege wurden in der Darstellung
übersprungen. Im Durchschnitt beträgt die Distanz rund 19 Wortformen zwischen
Erstcontroller und \norm{bėide}-Target gegenüber rund 42 Wortformen bei der
Urkundenstichprobe\is{Stichprobe}.
...
`sowohl \dots\ als auch' Personenmerkmale\is{Personenmerkmal} der Konjunkte auf
die Form der Konjunktion auswirken, ist die
Verteilung\is{Distribution!statistische} der Formen bei manchen der
Handschriften in \tabref{tab:kcbeidefuncvar} auffällig. In den untersuchten
bairischen\il{Bairisch} Handschriften A1, M, B1, VB und C1 sowie in der
mittelalemannischen\il{Alemannisch} Handschrift K deutet sich trotz
...
der Tatsache, dass die \isi{Stichprobe} zu A1, VB, M und K inhaltsbedingt fast
...
Konjunktion in \REF{ex:rittergebure} mit \lit{ritter} `Ritter' und
\lit{gebovre} `Bauern' als Kombination zweier eindeutiger Maskulina als
...
Adjektivstichprobe\is{Stichprobe} teilweise ein Nebeneinander von \norm{-e} und
...
\REF{ex:konjmixbeide1} `Mann' und `Frau' kombiniert. In diesem Kontext wäre bei
pronominaler Verwendung von \norm{bėide} mit \isi{Gender Resolution} zu
...
und Verben diejenigen Substantive\is{Substantiv} oder Pronomina, auf die sie
sich beziehen, sodass \norm{bėide} zumindest theoretisch indirekt über die
Konjunkte eine Affinität zu den Personenmerkmalen\is{Personenmerkmal} der
Controller der Konjunkte zeigen könnte, wie anhand von \REF{ex:recquery} in
...
ihnen referenzierten Personenmerkmalen\is{Personenmerkmal} übereinstimmen.
...
als belebte\is{Animata}, kann nicht ausgemacht werden, da für
...
Satzes bezieht. Auch in diesem Fall ist der Bezug dem Kontext\is{Pragmatik} nach
...
hier prädikativ\is{Adjektiv!prädikativ} und stehen in einem \isi{Relativsatz},
...
mit Adverbien\is{Adverb} und Präpositionalphrasen\is{Präpositionalphrase}
(PPs), wie in (\ref{ex:syntintvar1}--\ref{ex:syntintvar4}) dargestellt. Die
zitierten Stellen umfassen alle exzerptierten Belege für diesen Kontext.
...
\isi{Stichprobe} zur \isi{Adjektivdeklination} in B1 die
Verteilung\is{Distribution!morphologische} der Suffixe klar zwischen \norm{-e}
und \norm{-iu} getrennt ist, kommen in \REF{ex:syntintvar2_1} und
\REF{ex:syntintvar2_2} beide Formen der Konjunktion, \norm{bėide} und
...
syntaktischen Kontext Personenmerkmale\is{Personenmerkmal} keine Rolle. Da in
...
\norm{bėidiu} die bevorzugte Form darstellt, falls noch keine sporadische
...
\norm{bėidiu}. Neben den Personenmerkmalen\is{Personenmerkmal} der Konjunkte
wurde eine mögliche Abhängigkeit der Variation von der \isi{Distanz} zwischen
den kombinierten Controllern\is{Controller} und ihrem Target erwogen. Doch auch
