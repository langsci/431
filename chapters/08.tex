...
Pro\-nomina ohne \isi{Genusmarkierung} tritt aufgrund der referenzierten
...
Referenz zahlreicher vorhanden. \tabref{tab:simpnomctrla} zeigt ihre
Verteilung\is{Distribution!statistische} nach Personenmerkmalen und
Flexionstyp.
...
insofern auch hier der Quantor innerhalb der NP formale Kongruenz zeigt
\REF{ex:uozehende_2}, obwohl es sich um Körperteile\is{Körperteil} handelt und
damit um etwas, von dem auszugehen ist, dass es eine mittlere Position zwischen
den Polen belebt und unbelebt\is{Inanimata} einnimmt
(vgl.~\sectref{sec:gendsex} zur Annotation\is{Annotation} von \isi{Genus} bei
\isi{Inanimata}).
...
strikt um eine syntaktische \isi{Koordination} vom Typ \q{X \fw{und} Y} handelt
(vgl.~\sectref{sec:erwkonjbegr} zur \q{erweiterten} Koordination), werden
\lit{muoter} `Mutter' und \lit{sun} `Sohn' durch \lit{si} `sie' kombiniert
zusammengefasst. Der Quantor \lit{baide} modifiziert dieses \lit{si} und
kongruiert mit ihm. Der Quantor kongruiert damit direkt mit dem
Personalpronomen \lit{si} und indirekt mit \lit{muoter} und \lit{sun}.
...
Sowohl bei \lit{ſchatz} `Schatz' als auch bei \lit{gewín} `Gewinn' handelt es
sich um unbelebte\is{Inanimata} Maskulina. Es scheint im Kontext der Stelle
...
neuhochdeutscher\il{Neuhochdeutsch} Diphthongierung\is{Diphthongierung,
neuhochdeutsche}) als ver\-all\-gemeinerte Form des Nom./Akk.\ Pl.\ im
Bairischen\il{Bairisch} an und mutmaßen, dass diese Form in der Umgebung des
Habsburgers Albrecht~I.\ (1255--1308) aus dem Alemannischen\il{Alemannisch}
...
\norm{bėide} als Quantor und \norm{bėidiu} als \isi{Konjunktion} auf, was
...
die Form \norm{bed-} in A1 nur bis Blatt 33vb belegt ist, also nur für die
erste Hälfte des Texts. In M kommt \norm{bed-} dagegen nur als Quantor vor, und
in diesen Fällen nur mit \norm{-e} oder -Ø.
...
Vertreter von genusindifferentem\is{Genusindifferenz} \norm{siu} zumindest
denkbar.
...
Belege für kombinierten unbelebten\is{Inanimata} Bezug.
...
hinaus liegen keine Belege mit unbelebtem\is{Inanimata} Bezug vor. Ein
...
von Floating Quantifiers\is{Floating Quantifier}). \citet[623]{ksw2} folgend
...
Klasseneinteilung\is{Klassenbildung} vorgenommen. Es fällt auf, dass die Ketten
...
und \norm{bėide}-Target gegenüber rund 42 Wortformen bei der
Urkundenstichprobe.
...
sich die Frage nach dem Grad und der Zeit der \isi{Grammatikalisierung} der
...
`Mann' und \norm{wīp} `Frau' ist in solchen Fällen im Zusammen\-hang nicht
immer eindeutig,\is{Ambiguität} ob das jeweilige Konjunkt im Singular oder im
...
\norm{wīp} um ein starkes Neutrum der \fw{a}-Deklination \autocites[250,
...
von der pronominal-kataphorischen\is{Katapher} unterschieden werden
kann,\is{Ambiguität} erscheint die neutrale Form der Konjunktion in
\REF{ex:rittergebure} mit \lit{ritter} `Ritter' und \lit{gebovre} `Bauern' als
Kombination zweier eindeutiger Maskulina als unregelmäßig\is{Ausnahme}, wenn
sie überhaupt dekliniert ist. In \sectref{sec:adjdeclkc} wurde zwar berichtet,
dass in VB in der Adjektiv-Stichprobe teilweise ein Nebeneinander von \norm{-e}
...
In den Tabellenzeilen mit unbelebten\is{Inanimata} Konjunkten in
...
Nomina mit gleichem \isi{Genus}. Bei gleichem \isi{Genus} stehen unabhängig vom
Numerus insgesamt sechs Belege mit \norm{-e} sieben Belegen mit \norm{-iu}
gegenüber; bei verschiedenem \isi{Genus} beläuft sich die Zahl auf insgesamt
...
gleichmäßig. Im Fall von Konjunkten mit verschiedenem \isi{Genus} liegt in VB
...
Grund für die hohe Belegzahl für \norm{bėidiu} bei unbelebten\is{Inanimata}
Akkusativen mit verschiedenem \isi{Genus}. Im Gegensatz dazu korrespondiert die
äußerst geringe Anzahl von Dativ-Belegen mit dem auffälligen Fehlen von
Formulierungen wie \lit{beidiv in vnd den langenmænteln} `sowohl ihnen als auch
...
\REF{ex:recquery} keine Flexionsendungen aufweisen,\is{Kongruenz!prädikativ} da
es sich um koprädikative Adjektive\is{Adjektiv!prädikativ} handelt
\autocite[188]{ksw2}, verweisen beide Adjektive dennoch auf ihr Bezugswort
\lit{antwerc} `Belagerungsgerät'. Wenn die Konjunktion \lit{bediv} Kongruenz
...
unbelebte\is{Inanimata} Targets tendenziell anders verhalten als
belebte\is{Animata}, kann nicht ausgemacht werden, da für
unbelebte\is{Inanimata} \norm{bėide}-Targets nur ein einziger Beleg vorhanden
ist. Dass mit belebtem\is{Animata} Bezug \norm{bėidiu} in sechs Fällen
...
Hinweise auf \isi{Kovarianz} mit den Personenmerkmalen der Konjunkte, so
vorhanden. Auch wenn man annimmt, dass pronominal-kataphorisches\is{Katapher}
\norm{bėide} in den Belegen zu koordinierten Nominalen\is{Koordination}
enthalten ist, fallen diese nicht durch eine Konzentration auf \norm{bėide} bei
gleichem und \norm{bėidiu} bei ungleichem belebten\is{Animata} Geschlecht
beziehungsweise bei unbelebtem\is{Inanimata} Bezug auf.
...
über\-einstim\-mendem oder verschiedenem \isi{Genus} oder Sexus auftreten. Bei
VB können die Belege zwar so interpretiert werden, dass bei
belebten\is{Animata} Konjunkten eine leichte Tendenz zu \norm{bėide} und bei
unbelebten\is{Inanimata} zu \norm{bėidiu} vorliegt. Dies deckt sich jedoch
nicht mit der wesentlich älteren Handschrift A1, in der keine derartige Tendenz
auszumachen ist -- hier scheint generell \norm{bėidiu} bevorzugt zu werden.
...
\isi{Konjunktion} bemerkbar. Für diese Teilauswertung wurden Belege aus den
Handschriften A1, B1 und VB verwendet. In vergleichbaren Kontexten herrscht
dort ein Nebeneinander von \norm{bėide} und
...
werden, die keine Personenmerkmale aufweisen, das heißt, Adverbien\is{Adverb}
und Adverbiale\is{Adverbial}. Mit Kongruenz dürfte in diesem Kontext also nicht
zu rechnen sein. Auch in diesem Fall herrscht Variation zwischen \norm{bėide}
