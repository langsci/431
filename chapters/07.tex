...
Verteilung der verschiedenen Kongruenz\-formen von \norm{bėide} in Abhängigkeit
von den Personenmerkmalen des jeweiligen Controllers.
...
Erstcontrollers\is{Erstcontroller} gibt \tabref{tab:caosimprefctrl2} an. Wie
auch in \tabref{tab:simpnomctrl}, dort mit direktem Bezug auf ein
\isi{Substantiv} im Plural, lösen maskuline und feminine Referenz beim
indirekten Bezug über ein Pronomen die Kongruenzform \norm{bėide} aus, auf
neutrale Referenz folgt dagegen eine Form vom Typ \norm{bėidiu}.
...
aus \tabref{tab:combnomctrl} und \ref{tab:caosimprefctrl} noch einmal
...
(vgl.~\sectref{sec:floatquant} zu Floating Quantifiers\is{Floating
Quantifier}). In allen Fällen folgt das Target seinen Controllern.
...
Satzglied wie ihr Controller stehen, und größtenteils in unmittelbarer Nähe
...
nach Belebtheit und gleichem oder verschiedenem Geschlecht beziehungsweise
\isi{Genus}.
...
Kriterien verhalten als in der \isi{Kontaktstellung} (\q{gleiches Satzglied}).
Und zwar steht bei neun von zehn Belegen mit gemischtgeschlechtlichem Bezug
eine Form vom Typ \norm{bėidiu}, während in \isi{Kontaktstellung} nur etwa
70\,\% der Belege auf diese Form entfallen. Beim unbelebten\is{Inanimata} Bezug
dagegen ist kein entsprechender Unterschied zu beobachten.
...
bei den unbelebten\is{Inanimata} Targets unabhängig vom \isi{Genus} der
Erstcontroller \norm{bėidiu}.
