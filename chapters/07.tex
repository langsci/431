...
unbelebten Controllern mit gleichem Genus ebenfalls ein Beleg für \norm{-iu}
hinzu.
...
mit unterschiedlichem Genus die morphologisch neutrale Kongruenzform mit
\norm{-iu} auftritt.
...
\norm{bėidiu} aufgenommen werden können. Dies ist der Fall bei dem in
...
Beleg nur noch \lit{endru guͦt} `andere Güter (\textsc{nom.pl.\NeutI})'
\autocites(Nr.~2915)[213.27]{cao4} auf. Auch wenn \lit{bedu Jungherren} `beide
...
Bei unbelebten Controllern korrespondiert maskulines Genus regulär mit
...
\REF{ex:cao_diffgend_33_beide} stammen
...
die Kombination von Mann und Frau als Erstcontroller entfallen. Dies ist als
...
Quantifier (\sectref{sec:floatquant}) einnimmt. Es wird angenommen, dass
...
Erstcontroller mit gleichem Genus ergibt sich in \tabref{tab:caosimprefctrl}
ein Spiegelbild zum belebten\is{Animata} Gegenstück. Neben 23 Fällen von
\norm{bėidiu} mit neutralem Bezug ist die neutrale Form auch in vier von sechs
Fällen mit kombiniertem maskulinem Bezug zu finden, wie in
...
verhalten sich noch regelmäßiger als ihr belebtes\is{Animata} Gegenstück. Das
Beispiel in \REF{ex:cao_diffgend_inan} illustriert den hier diskutierten
Kongruenzkontext.
...
Erstcontroller. Ihre Verteilung\is{Distribution!statistische} nach den
Personenmerkmalen des Erstcontrollers gibt \tabref{tab:caosimprefctrl2} an. Wie
auch in \tabref{tab:simpnomctrl}, dort mit direktem Bezug auf ein Substantiv im
Plural, lösen maskuline und feminine Referenz beim indirekten Bezug über ein
Pronomen die Kongruenzform \norm{bėide} aus, auf neutrale Referenz folgt
dagegen eine Form vom Typ \norm{bėidiu}. \isi{Belebtheit} scheint auch hier
keine Rolle zu spielen, wenn sich die drei Belege für unbelebte Feminina
regelmäßig verhalten.
...
flektiert würde. Er schließt dies aus der Häufigkeit der Belege für
\norm{die/diu bėide} beziehungsweise \norm{si bėide/bėidiu} in seiner Stichprobe
aus der \citetitle{maroldschroeder1969}-Edition von \citet{maroldschroeder1969}
und der Edition des \citetitle{lachmannhartl1952} von
\citet{lachmannhartl1952}. \tabref{tab:asksiebeidekombis} gibt die
...
zeigt die im \CAO{}-Material belegten Kombinationen und ihre Häufigkeit.
...
insofern einen neutralisierenden Effekt auf die Form von \norm{bėide} haben,
als danach immer entweder \norm{bėide} oder \norm{bėidiu} stehen würde.
...
Belege für den direkten Bezug auf kombinierte \isi{Controller} wurden hier die
in \tabref{tab:combnomctrl} grau gedruckten Belege mitgezählt, da sie sich
...
\norm{bėide} auf zwei Erstcontroller mittels eines Pronomens (PRO\tsub{i+j}).
Hier liegen für den indirekten Bezug auf zwei belebte\is{Animata}
Erstcontroller von gleichem Geschlecht ausschließlich Belege für \norm{bėide}
vor. Beim indirekten Bezug auf zwei belebte\is{Animata} Substantive von
...
Genus-Kombinationen vorliegen. Bei weitem nicht alle Felder können mit den
Belegen aus dem Material des \CAO{} gefüllt werden.\is{Desiderat} Aufgrund der
Lücken lassen sich mit dieser Tabelle allein keine generellen Aussagen zur
...
Markierung von Genus an einem der beiden Glieder grammatikalisiert ist, macht
dies aber höchst fragwürdig an Einzelbelegen fest. Das umfangreichere Material
des \CAO{} weist darüber hinaus mehr Variation auf als die sehr kleine
Stichprobe von \citeauthor{askedal1973}. Die Häufigkeit von
...
halber vereinfacht im Rahmen des Feldermodells des deutschen\il{Neuhochdeutsch}
Satzes \autocites{drach1963}{woellstein2010}[vgl.\ auch][44--49]{mueller2023}
wiederholt. Die Controller \norm{ich} und \norm{hūsvrouwe} `Ehefrau' stehen im
Vorfeld (\textsc{VF}), während das Target \norm{bėidiu} `beide' nach der linken
Satzklammer (\textsc{LSK}) im Mittelfeld (\textsc{MF}) steht. Damit steht das
Target in einem anderen Satzteil als seine Controller, dennoch aber im gleichen
Teilsatz, in diesem Fall dem Hauptsatz (\norm{verjęhen} `aussagen, bekennen'
nimmt in der Regel einen Nebensatz als Komplement). Da die linke Satzklammer
mit einem Vollverb besetzt ist, ist die rechte Satzklammer (\textsc{RSK}) leer.
Der Wortformenabstand\is{Distanz!lineare} steigt tendenziell mit zunehmendem
...
(vgl.~\sectref{sec:floatquant} zu Floating Quantifiers). In allen Fällen folgt
das Target seinen Controllern.
...
Bezug häufig auch unab\-hängig vom Genus der Referenten \norm{bėidiu} steht.
Der betreffende Beleg wird in
...
Abhängigkeit zwischen den \q{Erstcontrollern} und dem \norm{bėide}-Target.
Zunächst erfolgt in \tabref{tab:caoanadist} ein Blick auf die direkte
Abhängigkeit zwischen anaphorischem Controller und Target.
...
gleiches beziehungsweise verschiedenes Geschlecht beziehungsweise Genus sind,
werden in der Tabelle die einzelnen Targets nicht mehr nach ihren
Personenmerkmale aufgelistet, sondern nach \isi{Belebtheit} und gleichem oder
verschiedenem Geschlecht beziehungsweise Genus.
...
\isi{Domäne} \q{gleicher Teilsatz} nicht anders als diejenigen in der
...
Genus aufgeteilt. Die Einteilung nach Vielfachen von Fünferpotenzen mit
...
Ebenso regelmäßig steht bei den unbelebten Targets unabhängig vom Genus der
Erstcontroller \norm{bėidiu}.
...
\autocite[vgl.\ auch][625--626]{ksw2}. Der \isi{Abstand} zwischen
...
Wortformenabstand\is{Distanz!lineare} zwischen \q{Erstcontrollern} und
\norm{bėide}-Targets ändert sich daran prinzipiell nichts. Damit gelten die
Schlüsse, die in \sectref{sec:caotargpers} bezüglich des Einflusses der
Kombination von Personenmerkmalen auf die Form von \norm{bėide} gezogen wurden,
auch unter Berücksichtigung von Wortformenabstand\is{Distanz!lineare} und
syntaktischer Distanz.

Auffällig ist allerdings, dass einerseits \norm{bėide} mit Bezug auf die
Kombination zweier belebter\is{Animata} Erstcontroller vom gleichen Geschlecht
($\SM+\SM{}$ und $\SF+\SF$) und andererseits \norm{bėidiu} mit Bezug auf die
Kombination zweier belebter\is{Animata} Erstcontroller mit unterschiedlichem
Geschlecht ($\SM+\SF$, $\SF+\SM$) unabhängig von der \isi{Distanz} dominant ist.
Dasselbe gilt für \norm{bėidiu} in Bezug auf die Kombination unbelebter
\isi{Controller} unabhängig von deren Genus. Die Vermutung, dass mit wachsendem
\isi{Abstand} die spezifische Information \q{Menschen von unterschiedlichem
Geschlecht}, die Gender Resolution bei Targets hin zum Neutrum auslöst, von der
unspezifischeren Information \q{Menschen} verdrängt wird, hat sich im
untersuchten Material des \CAO{} nicht bestätigt. Es ließ sich also nicht
beobachten, dass \norm{bėide} `beide (\textsc{m+f})' mit wachsender
\isi{Distanz} zwischen \isi{Controller} und Target gegenüber \norm{bėidiu}
`beide (\textsc{n})' dominiert.

Es lässt sich trotzdem feststellen, dass im Nahbereich zwischen
durchschnittlich sechs bis zehn Wortformen Abstand\is{Distanz!lineare}
zwischen Erstcontrollern und dem jeweils untersuchten Target und wieder im
Fernbereich der Klasse 10 (Abstand zwischen 126 und 250 Wortformen) die Form
\norm{bėide} verstärkt in Bezug auf belebte\is{Animata} Erstcontroller mit
verschiedenem Geschlecht auftritt. In allen diesen Fällen ist \norm{bėide}
...
Konjunkten enthalten. Sollte dies als Evidenz für Gender Resolution dienen?
\citet[187]{gjelsten1980} räumt in ihrer Kritik an \posscite{askedal1974}
Arbeit zu konjunktionalem \norm{bėide} ein, dass bei substantivischen
Konjunkten nicht klar zwischen pronominalem Gebrauch (\norm{bėide}, X
\norm{unde}~Y) und konjunktionalem (\norm{bėide}~X \norm{unde}~Y) unterschieden
...
unbelebten Genus. Bei verschiedenem unbelebten Genus entfällt der Großteil der
Belege auf \norm{bėidiu}. Daneben liegen auch Belege für \norm{bėide} und
\norm{bėidiu} im Dativ und Genitiv vor. Wenn \norm{bėide} entsprechend der
formalen Merkmale der jeweiligen NP flektieren würde, dürften diese Formen
nicht auftreten. Abgesehen von je einem Beleg für \norm{bėide} bei
verschiedenem unbelebten Genus steht in den vereinzelten belegten Fällen
\norm{bėidiu}.

Insgesamt ist keine deutliche Häufung auf \norm{bėide} bei gleichem
belebtem\is{Animata} Geschlecht sowie auf \norm{bėidiu} bei unbelebtem Bezug
unabhängig vom Genus auszumachen, wie dies bei der quantifizierenden Verwendung
der Fall war. Angesichts der vereinzelten Belege in den einzelnen Kasusfeldern
kann wohl auch für belebte\is{Animata} Konjunkte mit verschiedenem Geschlecht
keine Regel für das Auftreten von \norm{bėidiu} abgeleitet werden. Wenn sich
Belege mit pronominalem \norm{bėide} eingeschlichen haben sollten, treten diese
nicht klar aus der Auszählung hervor.
...
\tabref{tab:caokoordtarg} zeigt. Für Konjunkte mit gleichen Genus- und
...
dient, die die Zweiheit der Optionen betont \autocites(siehe auch
\sectref{sec:ovwbeideconj})[425--428]{johannessen2005}. Im exzerpierten Material
kommt \norm{bėide} in diesem Zusammenhang mit Temporaladverbien\is{Adverb}
\REF{ex:caokoordsyn_1}, Lokaladverbien\is{Adverb} \REF{ex:caokoordsyn_2} und
...
koordinierten Controllern\isi{Controller} noch mit koordinierten Targets wird
eine eindeutige Abhängigkeit der Form der Konjunktion von den Personenmerkmalen
der Konjunkte deutlich. Dass der Aspekt von \norm{bėide} als Fokuspartikel
\autocites(siehe auch \sectref{sec:ovwbeideconj})[425--428]{johannessen2005} in
...
angewendet werden kann (\sectref{sec:beideconj}).
