...
von Personenmerkmalen\is{Personenmerkmal} in unterschiedlichen syntaktischen
Kontexten. Daran schließt sich eine Untersuchung nach \isi{Distanz} zwischen
...
koordinierten\is{Koordination} nominalen Ausdrücken
(Substantive\is{Substantiv}, Pronomen) wie zum Beispie \norm{her Rvͦdiger vnd
ſin hovſfrowe} `Herr Rüdiger und seine Ehefrau' in \REF{ex:beid2coordncao1} und
dem Schema in \figref{fig:beid2coordncao1_1} näher betrachtet. Dabei werden
Kombinationen von \isi{Personalpronomen} und Eigenname wie \norm{ich unde
Ulrich} an dieser Stelle mitgezählt, da auch Personalpronomina eine nominale
Kategorie darstellen, insofern sie sich im Kontext dieser Untersuchung
regelmäßig auf ein substantivisches \isi{Antezedens} beziehen und dessen
...
bei unbelebten\is{Inanimata} Controllern mit gleichem \isi{Genus} ebenfalls ein
Beleg für \norm{-iu} hinzu.
...
besonders im Zusammenhang mit zwei belebten\is{Animata}
Nominalphrasen\is{Nominalphrase} (NPs) mit unterschiedlichem \isi{Genus} die
morphologisch neutrale Kongruenzform mit \norm{-iu} auftritt.
...
Personen in der Zeugenliste bilden sollte, wäre die neutrale Form \lit{bedu} in
Bezug auf maskulin-männliches \lit{Jungherren} immer noch auffällig.
...
in denen sich \norm{bėide} direkt auf ein einzelnes Substantiv\is{Substantiv}
im Plural bezieht. Für diesen Kontext gibt es weit mehr Belege, als dass jeder
einzelne zitiert werden könnte. Das Beispiel in \REF{ex:beid2snglncao} und das
...
Vergleichsort in der Adjektivstichprobe\is{Stichprobe} ist München
...
Plural-\isi{Substantiv} als Controller kommen bei den belebten\is{Animata}
...
der \isi{Stichprobe} zu Zürich (\sectref{par:adjzuerich}) ein Unterschied
...
\isi{Distanzstellung} als Floating Quantifier\is{Floating Quantifier}
...
`wir beide' als Einheit im Kontext\is{Pragmatik} der Urkunde ausgeschlossen
werden, da drei Aussteller genannt werden: die Brüder \lit{wezel vn̄ hainrich
wezel vnde Cvͦnrat der Bodemer} \autocites(Nrn.~1201~AB)[472,7]{cao2}.
...
\isi{Substantiv} im Plural bezieht. Ein Beispiel für diesen syntaktischen
...
direktem Bezug auf ein \isi{Substantiv} im Plural, lösen maskuline und feminine
...
Belege für unbelebte\is{Inanimata} Feminina regelmäßig verhalten.
...
\norm{kint} jeweils handelt, geht aus dem Kontext\is{Pragmatik} der jeweiligen
Urkunden hervor.
...
\isi{Stichprobe} aus der \citetitle{maroldschroeder1969}-Edition von
...
Frauen, im zweiten auf Paare aus Mann und Frau oder unbelebte\is{Inanimata}
Dinge (\sectref{subsubsec:beid2p2coordncao}).
...
bei der Kongruenzbeziehung\is{Kongruenzrelation} zwischen zwei
...
Bezug auf kombinierte \isi{Controller} wurden hier die in
\tabref{tab:combnomctrl} grau gedruckten Belege mitgezählt, da sie sich nicht
anders verhalten als ihre regulär gezählten Gegenstücke. Aussagen über das
Verhalten des Targets beim direkten Bezug von \norm{bėide} auf zwei
unbelebte\is{Inanimata} Substantive können keine gemacht werden, da dieser
Kontext in der \isi{Stichprobe} nicht belegt ist.
...
Substantive\is{Substantiv} von unterschiedlichem Geschlecht tritt dagegen
...
Bezugs auf zwei unbelebte\is{Inanimata} Substantive\is{Substantiv} von
...
als die sehr kleine \isi{Stichprobe} von \citeauthor{askedal1973}. Die
Häufigkeit\is{Frequenz} von \norm{si} `sie' ist zumindest im \CAO{} der
Vereinfachung des pronominalen Paradigmas\is{Paradigma} hin zu einer
genusübergreifenden\is{Genusindifferenz} Pluralform
zuzuschreiben\is{Genusdistinktion, Abbau der}, die nach \citet[391--392]{ksw2}
im Oberdeutschen\il{Oberdeutsch} der zweiten Hälfte des 13.~Jahrhunderts den
...
\norm{bėidiu} `beide' nach der linken \isi{Satzklammer} (\textsc{LSK}) im
\isi{Mittelfeld} (\textsc{MF}) steht. Damit steht das Target in einem anderen
Satzteil als seine Controller, dennoch aber im gleichen Teilsatz, in diesem
Fall dem Hauptsatz (\norm{verjęhen} `aussagen, bekennen' nimmt in der Regel
einen Nebensatz als \isi{Komplement}). Da die linke Satzklammer mit einem
Vollverb besetzt ist, ist die rechte Satzklammer (\textsc{RSK}) leer. Der
...
vor. Hierbei ist keine Variation in der \isi{Lokalität} zwischen beiden
Controllern zu beobachten. Es liegen also keine Fälle vor, in denen beide
Controller in unterschiedlichen (Teil-)Sätzen stehen. Aufgrund der geringen
Belegmenge sind auch hier keine starken Konzentrationen von Belegen zu
beobachten. Die unter Vorbehalt aufgenommenen Belege sind grau gedruckt; bei
ihnen handelt es sich in allen Fällen zufällig um Targets in
\isi{Distanzstellung} zu ihrem Controller (vgl.~\sectref{sec:floatquant} zu
Floating Quantifiers\is{Floating Quantifier}). In allen Fällen folgt das Target
seinen Controllern.
...
Verteilung\is{Distribution!morphologische} vermuten, dass
Personenmerkmale\is{Personenmerkmal} eine wichtigere Rolle spielen als die
syntaktische Distanz\is{Distanz!syntaktische} oder die Distanz nach
Wortformen\is{Distanz!lineare}. Einzig der Beleg für zwei
unbelebte\is{Inanimata} Maskulina mit Abstand zwanzig und drei
Wortformen\is{Distanz!lineare} fällt durch die unerwartete \norm{bėidiu}-Form
auf. In \sectref{subsubsec:beid2p2coordncao} bezüglich des indirekten Bezugs
auf kombinierte Controller konnte jedoch beobachtet werden, dass bei
unbelebtem\is{Inanimata} Bezug häufig auch unab\-hängig vom \isi{Genus} der
...
unmittelbar von einer einfachen NP\is{Nominalphrase} im Plural abhängen. In
allen Fällen verteilen sich die Formen von \norm{bėide} in
...
nach syntaktischem Abstand\is{Distanz!syntaktische} der
Kongruenzbeziehung\is{Kongruenzrelation} aus. Es ist deutlich sichtbar, dass
...
\sectref{sec:caotargpers} zur Untersuchung der Form des Quantors \norm{bėide}
anhand der referenzierten Personenmerkmale\is{Personenmerkmal} bereits deutlich
wurde, dass die ausschlaggebenden Merkmale für die Variation zwischen
\norm{bėide} und \norm{bėidiu} \isi{Belebtheit} und gleiches beziehungsweise
verschiedenes Geschlecht beziehungsweise \isi{Genus} sind, werden in der
Tabelle die einzelnen Targets nicht mehr nach ihren Personenmerkmale
aufgelistet, sondern nach \isi{Belebtheit} und gleichem oder verschiedenem
Geschlecht beziehungsweise \isi{Genus}.
...
Merkmal einer Konrad-typischen Corroboratio gibt er als Beispiel: \lit{darvmbe
han ich im geben diſen brief / verſigelt vnde gefeſtent mit minem Jnſigel vnde
mit der Stet Jnſigel zu auſpurch div baidiv dran hangent} \autocites(Nr.~N~272,
Augsburg, 1285)[120--121]{haacke1964}[vgl.~dazu][216,1--2]{cao5}. Dies
entspricht im Grunde der Formulierung in \REF{ex:insigel}.
...
\isi{Kontaktstellung} (\q{gleiches Satzglied}). Und zwar steht bei neun von zehn
Belegen mit gemischtgeschlechtlichem Bezug eine Form vom Typ \norm{bėidiu},
während in \isi{Kontaktstellung} nur etwa 70\,\% der Belege auf diese Form
entfallen. Beim unbelebten\is{Inanimata} Bezug dagegen ist kein entsprechender
Unterschied zu beobachten.

Betreffend den indirekten Bezug auf einzelne Substantive\is{Substantiv} im
Plural als \isi{Erstcontroller} gilt dasselbe. Auch hier verteilen sich die
fünf Targets in der \isi{Domäne} \q{gleicher Teilsatz} nicht anders als
diejenigen in der \isi{Domäne} \q{gleicher Satzteil}. An den bisherigen
Beobachtungen zum Einfluss der Personenmerkmale\is{Personenmerkmal} auf die
Form von \norm{bėide} in \sectref{subsubsec:beid2p2snglncao} ändert sich also
ebenfalls nichts.
...
Kongruenzbeziehung\is{Kongruenzrelation} -- einen Effekt auf die Wahl der
...
Erstcontroller arbeiten zu müssen, wurde zur \isi{Komplexitätsreduktion} bei
der \isi{Klassenbildung} jeweils das arithmetische Mittel des
Wortformenabstands für jedes Controllerpaar gewertet, vergleiche das fiktive
Beispiel in \tabref{tab:worddistarith}.
...
Ebenso regelmäßig steht bei den unbelebten\is{Inanimata} Targets unabhängig vom
\isi{Genus} der Erstcontroller \norm{bėidiu}.
...
Frau und steht direkt hinter oder kurz nach einem \isi{Personalpronomen} in
...
Personenmerkmale\is{Personenmerkmal} ihres Controllers\is{Controller}
beziehungsweise ihrer \isi{Controller}. Auch in Hinblick auf den
...
Kombination unbelebter\is{Inanimata} \isi{Controller} unabhängig von deren
\isi{Genus}. Die Vermutung, dass mit wachsendem Abstand\is{Distanz!lineare} die
spezifische Information \q{Menschen von unterschiedlichem Geschlecht}, die
\isi{Gender Resolution} bei Targets hin zum Neutrum auslöst, von der
...
Auch wenn Urkunden mündlich\is{Mündlichkeit} durchgeführte Verhandlungen
dokumentieren und sich vermuten lässt, dass sie gemäß der verbreiteten Formel
\norm{hȫren lęsen} `lesen hören' darauf ausgelegt sind,
mündlich\is{Mündlichkeit} verlesen zu werden
\autocites[595]{schmidtwiegand1998b}[31]{schulze2011}, könnte man den Grund
dafür in konzeptueller \isi{Schriftlichkeit} suchen.
...
durchgängig die Form \lit{bede} belegt. Der Aufstellung in \citet[621,
...
der Belege für \norm{bēd-} `beid-' in ihrem \isi{Korpus}.
...
Anwendungsfall der Konstruktion \norm{bėide \dots\ unde} `sowohl \dots\ als
...
\REF{ex:caokonj2ctrl_1}, Pronomina und Namen \REF{ex:caokonj2ctrl_2}, komplexe
Nominalphasen \REF{ex:caokonj2ctrl_3} oder auch freie
Relativsätze\is{Relativsatz} \REF{ex:caokonj2ctrl_4}. Gemeinsam ist den hier
...
\norm{bėidiu} in Abhängigkeit der Personenmerkmale\is{Personenmerkmal} der
...
Aus der Belegverteilung im Urkundenmaterial ist jedoch nicht \fw{per se} zu
...
übereinstimmenden \isi{Genus}-\is{Genusmerkmal} und Sexus\-merkmalen ($\MascM{}
+ \MascM{}$ und $\FemF{} + \FemF$). Anders als bisher beobachtet, steht in
54\,\% der Fälle die ansonsten neutrale Form \norm{bėidiu}. Im dritten
Abschnitt, der mindestens ein Element mit irrlevantem Sexus enthält, das formal
jedoch maskulin ist (\MascA, zum Beispiel \norm{lǖte} `Leute', \norm{burgǟre}
...
bei \norm{bėid(e)}, das 65\,\% der für diesen Kontext belegten Formen ausmacht.
Im letzten Abschnitt der Tabelle, der unbelebte\is{Inanimata} Elemente mit
unterschiedlichem \isi{Genus} enthält, wird anders als zuvor \norm{bėidiu}
nicht klar favorisiert: Mit 58\,\% für \norm{bėidiu} sind die Verhältnisse
nahezu ausgeglichen. Insgesamt ergibt sich keine klare, von
...
mit unspezifischem Geschlecht sowie belebten\is{Animata} mit
unbelebten\is{Inanimata} Konjunkten enthalten. Sollte dies als Evidenz für
\isi{Gender Resolution} dienen? \citet[187]{gjelsten1980} räumt in ihrer Kritik
an \posscite{askedal1974} Arbeit zu konjunktionalem \norm{bėide} ein, dass bei
...
unbelebten\is{Inanimata} Konjunkten wie in \norm{lǖte unde guet} `Leute
...
unbelebten\is{Inanimata} \isi{Genus}. Bei verschiedenem unbelebten \isi{Genus}
entfällt der Großteil der Belege auf \norm{bėidiu}. Daneben liegen auch Belege
für \norm{bėide} und \norm{bėidiu} im Dativ und Genitiv vor. Wenn \norm{bėide}
entsprechend der formalen Merkmale der jeweiligen NP\is{Nominalphrase}
flektieren würde, dürften diese Formen nicht auftreten. Abgesehen von je einem
Beleg für \norm{bėide} bei verschiedenem unbelebten\is{Inanimata} \isi{Genus}
steht in den vereinzelten belegten Fällen \norm{bėidiu}.
...
Belege in den einzelnen Kasusfeldern kann wohl auch für belebte\is{Animata}
...
indirekten Bezug auf kombinierte nominale \isi{Controller} lassen sich auch für
...
\isi{Stichprobe} sind dies konkret verschiedene Arten von Adjektiven, die in
...
attributive\is{Adjektiv!attributiv} \REF{ex:caobeidkoordtarg_2} und prädikative
...
\isi{Genus}- und Sexusmerkmalen liegt nur ein einziger Beleg vor; bei der
Konjunktion von maskulinen Targets mit beliebigem Sexus sind die Verhältnisse
nahezu ausgeglichen, insofern in 57\,\% der Fälle \norm{bėide} belegt ist. Bei
den unbelebten\is{Inanimata} Targets mit unterschiedlichem \isi{Genus} steht in
64\,\% der Belege \norm{bėidiu}.
...
Im Urkundenmaterial kommt neben der Konjunktion von Controllern\is{Controller}
und Targets, die Personenmerkmale\is{Personenmerkmal} definieren
beziehungsweise widerspiegeln, auch die Konjunktion solcher Elemente vor, die
keinerlei Personenmerkmale definieren. Die Konstruktion \norm{bėide \dots\
unde} `sowohl \dots\ als auch' hat hier insofern funktionalen Charakter, als
...
mit koordinierten Controllern\is{Controller} noch mit koordinierten Targets
wird eine eindeutige Abhängigkeit der Form der \isi{Konjunktion} von den
Personenmerkmalen\is{Personenmerkmal} der Konjunkte deutlich.\is{Koordination}
