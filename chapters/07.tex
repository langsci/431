...
schließt sich eine Untersuchung nach Distanz zwischen Controller und Target an.
Am Schluss des Kapitels steht der Vergleich zwischen \norm{bėide} als
...
alemannischen\il{Alemannisch} Sprachraum (Kloster St.~Urban,
Kt.~Luzern; hochalemannisch\il{Alemannisch}). Mit
...
eines Fotos der Original\-urkunde bestätigen (\figref{fig:1843}). Ein
\norm{er}-Haken oder ein Nasalstrich, der die Flexion zu \norm{bėider}
beziehungsweise \norm{bėiden} ändern würde, sind auch im Original nicht
vorhanden. Regelmäßiger \fw{n}-Schwund am Wortende
...
mitteldeutschen\il{Mitteldeutsch} Sprachraum \REF{ex:remgenbeide}, der in
dieser Studie ansonsten ausgeklammert wurde. Beispiele für \norm{bėidiu} im
Gen.\ Pl.\ konnten nicht gefunden werden, das heißt, \lit{-í} in \lit{beidí}
ist in \REF{ex:remgenbeide_1} nicht als Variante von \norm{-iu} zu werten,
sondern als ostmitteldeutsche\il{Ostmitteldeutsch} Schreibweise für \norm{-e}
...
bairischen\il{Bairisch} Sprachraum (Rothenburg ob der Tauber, Kloster Stams)
und einer aus dem schwäbischen\il{Schwäbisch} (Kloster Heiligkreuztal); bei
\norm{siu bėide} fanden sich alle fünf Belege in alemannischen\il{Alemannisch}
Urkunden (Kloster Kirchberg, Überlingen, Zürich). Da in keiner der Urkunden,
die diese Belege enthalten, ein systematischer Unterschied zwischen \norm{si},
\norm{sie} und \norm{siu} festgestellt werden konnte, müssen sie als zum
...
wären, da auch auf Distanz Variation in der Genusmarkierung von \norm{bėide}
als indirektes Target zweier NPs beobachtet werden kann. Es konnte nicht
festgestellt werden, dass Pronomen, die nicht nach Genus flektieren, insofern
einen neutralisierenden Effekt auf die Form von \norm{bėide} haben, als danach
...
vgl.~\sectref{sec:ctrltarg}). Die Schemata in \figref{fig:caodirdist_words} und
\ref{fig:caodirdist_syn} verdeutlichen die jeweilige Zählweise.
...
In \figref{fig:caodirdist_words} wird der Abstand zwischen den beiden
Controllern \norm{ich} und \norm{(mīn) hūsvrouwe} `(meine) Ehefrau' zu ihrem
Target \norm{bėidiu} `beide' nach Wortformen gezählt. Der erste Sprung zur
jeweils nächsten Wortform wird mit null angesetzt, da zwischen \norm{ich} und
...
\figref{fig:caodirdist_syn} wird das Beispiel der Anschaulichkeit halber
vereinfacht im Rahmen des Feldermodells des deutschen\il{Neuhochdeutsch} Satzes
\autocites{drach1963}{woellstein2010}[vgl.\ auch][44--49]{mueller2023}
...
Der Wortformenabstand steigt tendenziell mit zunehmendem Abstand zwischen
...
Für die syntaktische Distanz liegt die in \tabref{tab:caocodistp} gezeigte
...
ihrem Controller (vgl.~\sectref{sec:floatquant} zu Floating Quantifiers). In allen
...
\norm{bėidiu}-Form auf. In \sectref{subsubsec:beid2p2coordncao} bezüglich des
...
\norm{bėidiu} steht. Der betreffende Beleg wird in \REF{ex:n241_hofzehnt}
zitiert.
...
\begin{table}
\centering
\caption{Form nach Distanz vom einfachen nominalen
Controller}
\begin{tabular}{
	l
	c >{\scshape}l
...
\tabref{tab:caoanadist} ein Blick auf die direkte Abhängigkeit zwischen
anaphorischem Controller und Target.

\tabref{tab:caoanadist} weist die Belegverteilung nach syntaktischem Abstand der
...
Geschlecht beziehungsweise Genus sind, werden in der Tabelle die einzelnen
Targets nicht mehr nach ihren Personenmerkmale aufgelistet, sondern nach
\isi{Belebtheit} und gleichem oder verschiedenem Geschlecht beziehungsweise
Genus.

\begin{sidewaystable}
\caption{Form nach Distanz vom anaphorischen
...
Distanzstellung von Targets die belebten\is{Animata} Belege wesentlich
...
als auch aus dem hoch- und niederalemannischen\il{Alemannisch} Sprachraum.
...
\isi{Controller} steht: direkt dahinter und damit in der Regel im gleichen
Satzglied \autocite[vgl.\ auch][625--626]{ksw2}. Der Abstand zwischen
...
\isi{Controller}. Auch in Hinblick auf den Wortformenabstand zwischen
\q{Erstcontrollern} und \norm{bėide}-Targets ändert sich daran prinzipiell
nichts. Damit gelten die Schlüsse, die in \sectref{sec:caotargpers} bezüglich
des Einflusses der Kombination von Personenmerkmalen auf die Form von
...
Geschlecht ($\SM+\SF$, $\SF+\SM$) unabhängig von der Distanz dominant ist.
...
Abstand die spezifische Information \q{Menschen von unterschiedlichem
...
beobachten, dass \norm{bėide} `beide (\textsc{m+f})' mit wachsender Distanz
zwischen \isi{Controller} und Target gegenüber \norm{bėidiu} `beide
(\textsc{n})' dominiert.

Es lässt sich trotzdem feststellen, dass im Nahbereich zwischen
durchschnittlich sechs bis zehn Wortformen Abstand zwischen Erstcontrollern und
...
Bezug auf belebte\is{Animata} Erstcontroller mit verschiedenem Geschlecht
auftritt. In allen diesen Fällen ist \norm{bėide} indirekt über ein oder
mehrere anaphorische Ausdrücke\is{Anapher} (Pronomina wie \norm{ich, wir, uns})
mit der letzten vollständigen Nennung seiner \isi{Controller} verbunden.
...
Sprachraum immer eine Form vom Typ \norm{bėide} steht. Analog zur Auswertung in
\sectref{sec:adjdeclcao} zur \isi{Adjektivdeklination} im \CAO{} gibt
\tabref{tab:caobeidefuncvar} die Belegmengen in der Region ausgewählter
Ausstellungs\-orte nach Funktion von \norm{bėide} getrennt wieder. Beim Quantor
wurden zum Vergleich nur Kontexte mit zwei Controllern\is{Controller} (direkter
und indirekter Bezug) berücksichtigt.
...
und \ref{fig:cao_beide_geofunctypes_conj} die geografische Verteilung der
\norm{bėide}- und \norm{bėidiu}-Typen nach Verwendungskontext grafisch für alle
in der Stichprobe vorhandenen Belegorte dar. Auffällig ist, dass besonders im
Nieder\-alemannischen\il{Alemannisch} bei der Konjunktion
...
\caption{Geografische Verteilung der Formentypen bei \norm{bėide} als
	Konjunktion (\norm{bėide \emph{X} unde \emph{Y}}; Hintergrundkarte:
...
stammen aus dem ostfränkischen\il{Ostfränkisch} Sprachraum und illustrieren das
dortige Neben\-einander beider Formen von konjunktionalem \norm{bėide}, wie aus
der Karte in \figref{fig:cao_beide_geofunctypes_conj} deutlich wird.
...
\norm{bėide} sämtlich aus dem alemannischen\il{Alemannisch} Sprachraum. Die
Belege für \norm{bėidiu} stammen umgekehrt hauptsächlich aus dem
bairischen\il{Bairisch} Sprachraum.
...
schwäbischen\il{Schwäbisch} Sprachraum steht dagegen hauptsächlich
\norm{bėidiu}. Ein Unterschied zwischen \norm{-e} und \norm{-iu} anhand der
syntaktischen Funktion von \norm{bėide} macht sich nur insofern bemerkbar, als
die Form bei konjunktionalem Gebrauch in den Belegen zu Straßburg, Konstanz,
Salzburg und Wien deutlich erstarrt erscheint, während beim Quantor zumindest
an den drei zuletzt genannten Orten beide Flexionsformen auftreten.
