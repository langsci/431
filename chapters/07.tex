...
Personenmerkmale reflektieren. Zwei Belege werden in \REF{ex:beid2coordncao1}
angeführt. Das Schema in \REF{ex:beid2coordncao1_1} verdeutlicht den
...
Daneben liegen noch die drei Belege in \REF{ex:beid2coordncao2} vor, die
zunächst getrennt von der regulären Auswertung behandelt wurden, da bei ihnen
die Flexion des Targets vor einem Wort steht, das mit einem Vokal beginnt, also
in einer Hiatusposition steht \autocites[vgl.][90--91]{askedal1973}[191--193,
...
Die niedrige Belegzahl in \tabref{tab:combnomctrl} erweckt den Eindruck, dass die
...
Zusammenhang.%
%
	\footnote{Während für das \q{klassische} Mittelhochdeutsche von einem
...
\REF{ex:cao_diffgend_33_beide} stammen \citet{cao-online} zufolge beide aus
Wien (\sectref{par:adjwien}).
...
An der Belegverteilung in \tabref{tab:caosimprefctrl} ist zu erkennen, dass
unter den Belegen mit belebter Referenz 80\,\% der Belege auf die Kombination
von Mann und Frau als Erstcontroller entfallen. Dies ist als Merkmal der
...
Belegzahl mit 32 zu 9 allerdings sehr stark zugunsten von \norm{bėidiu} aus.
...
als eindeutige Tendenz zu werten. Zwei Belege mit \lit{bede} `beide
(\textsc{m+f}?)' und einer mit \norm{bedi} `beide (\textsc{n}?)' stammen ferner
aus Straßburg und sind mit Vorsicht zu werten (\sectref{par:adjstrassburg}).
Das Beispiel in \REF{ex:cao_diffgend_11_beide} illustriert den Kontext.
...
Daneben stehen die zwei Belege in \REF{ex:vvfinctrl2}, die zunächst aus formalen
Gründen getrennt von den regulären Belegen gezählt wurden, dennoch aber das
Phänomen illustrieren. Auch hier steht \norm{bėide} vor einem Wort, das mit
Vokal beginnt.
...
\REF{ex:1843_kinde}. In Urkunde Nr.~1843 \autocite[146,11--47]{cao3} bezeugt
...
vorhanden, auch gibt es keinen Hinweis auf einen Schreibfehler. Regelmäßiger
...
\ex\label{ex:682_insigel}
	...
	\gll Tougen wir abt vollant / vnd Bertholt der prior / vnd der Conuent von
		...
		tun wir Abt Volland[\textsc{nom.sg.\MascM}] {} und
			...
			Konvent[\textsc{nom.sg.\MascM}] von Hirsau kund {} Darum heben
		...
		\trans `machen wir, Abt Volland und Berthold der Prior und der
			...
			Urkunde unsere beiden Siegel%
			% und Graf Albrechts von Hohenberg
			'
			\parencites(Nr.~682, Kl.~Hirsau, Kr.~Calw, 1284)[96,3+11--12]{cao2}
		%
...
mitteldeutschen\il{Mitteldeutsch} Sprachraum, der bei der vorliegenden
Untersuchung ausgeklammert wurde. Die Belege werden in \REF{ex:remgenbeide}
zitiert. Die Annotation ist aus dem \REM{} übernommen und in
\REF{ex:remgenbeide_1} unklar. Belege für \norm{bėidiu} im Gen.\ Pl.\ konnten
nicht gefunden werden, das heißt, \lit{-í} in \lit{beidí} ist in
\REF{ex:remgenbeide_1} nicht als Variante von \norm{-iu} zu werten, sondern als
ostmitteldeutsche\il{Ostmitteldeutsch} Schreibweise für \norm{-e}
...
\begin{xlist}
	...
	\ex \label{ex:remgenbeide_2}
		...
		\trans `durch den Tod beider Kinder oder eines von ihnen'
			(%
				Köln, Historisches Archiv der Stadt, Best.~210
...
ein Spiegelbild zum belebten Gegenstück. Neben 23 Fällen von \norm{bėidiu} mit
neutralem Bezug ist die neutrale Form auch in vier von sechs Fällen mit
kombiniertem maskulinem Bezug zu finden, wie in
...
\autocites(Nrn.~1201~AB)[472,7]{cao2}.
...
%
	\footnote{Bei den anderen beiden Stellen handelt es sich um
		...
		\tit{Baumgarten geistlicher Herzen} in München, Bayerische Staatsbibl.,
		Cgm~6247: 108v,19--21 (\cite[vgl.][M401]{rem}; mit Parallelstelle in
...
\REF{ex:cao_diffgend_inan} illustrieren den hier diskutierten
Kongruenzkontext.
...
\ex \label{ex:beid2p2snglncao}
	\gll vnd ſol ez den zwein wroͮwen gen biz an ir beider tot / ſo
		...
		und soll es den zwei Frau-\textsc{dat.pl.\FemF} übergeben bis an ihr
...
ist, möchte ich die Belege kurz charakterisieren. Die Stellen für ersteren
Kontext werden in \REF{ex:cao_beidiu_neutfem} zitiert.
...
\ex \label{ex:cao_beidiu_neutfem}
	...
	\ex \label{ex:cao_beidiu_neutfem_3}
		...
		\trans `Schwester Gertraut und Schwester Diemut, Herrn Wernhers
			Kindern \textelp{} Und wenn der vorgenannten Kinder eines stirbt
...
oder vielleicht auch spezifischer `Klosterangehörige'
(\ref{ex:cao_beidiu_neutfem_2}--c) aufzufassen ist. Die \norm{kint} sind dabei
im Kontext der Rechtssprache nicht zwangsläufig minderjährig, insofern
Erwachsene unabhängig von ihrem Alter Kinder ihrer Eltern sind
\autocites[vgl.][1736]{schwab2012}[siehe
...
hervor. Bei \REF{ex:cao_beidiu_neutfem_2} und \REF{ex:cao_beidiu_neutfem_3}
...
\autocite[Kr.~Nürnberger Land; vgl.][619]{caor}. Die Kongruenz zwischen
\lit{ſi} und \lit{chinde} `Kinder' wurde hier jeweils aufgrund der Nähe
zwischen pronominalem Target und nominalem Controller angenommen. Im ganzen
unter\-suchten Material ließen sich keine Belege für direkte Kongruenz zwischen
\norm{bėidiu} und zwei weiblichen Controllern finden. Eine Recherche
diesbezüglich im \REM{} lieferte ebenfalls keine Ergeb\-nisse.
...
\ex \label{ex:cao_beidiu_neutunkn}
	...
	\trans `mit zwei Kindern \textelp{} das sollen sie beide besitzen
		bis an ihren Tod \textelp{} Und wenn sie beide nicht \textins{mehr}
		sind \textelp{}'
...
weitere Untersuchungen für diesen syntaktischen Zusammenhang nicht relevant
...
festgestellt werden, dass Pronomen, die nicht nach Genus flektieren, einen
neutralisierenden Effekt auf die Form von \norm{bėide} haben, insofern dass
danach immer entweder \norm{bėide} oder \norm{bėidiu} stehen würde.
...
\begin{table}
\centering
\caption{Form von \norm{bėide} nach syntaktischem Kontext (kombinierter
	Bezug)}
...
mittels eines Pronomens (PRO\tsub{i+j}). Hier liegen für den indirekten Bezug
auf zwei belebte Erstcontroller von gleichem Geschlecht ausschließlich Belege
...
\begin{table}
\centering
\caption{Form von \norm{bėide} nach syntaktischem Kontext (einfacher
	Bezug)}
\begin{tabular}{
	l l
	c
...
\begin{table}
\centering
\caption{Form von \norm{bėide} nach Distanz vom einfachen nominalen
Controller}
\begin{tabular}{
	l
	c >{\scshape}l
...
\begin{sidewaystable}
\caption{Form von \norm{bėide} nach Distanz vom anaphorischen
...
Die \tabref{tab:caodist} stellt die Verteilung der Belege nach Distanzklasse mit
...
von verschiedenem Geschlecht in der Distanzklasse 2 (durchschnittlicher Abstand
zwischen 6 und 10 Wortformen). Nahezu drei Viertel der Belege für diesen
Kontext zeigen die Form \norm{bėide}. Im Vergleich zu demselben Kontext in
anderen Distanzklassen liegt also eine umgekehrte Verteilung der Belege vor.
...
Geschlecht der Erstcontroller ist auch in der Distanzklasse 10
...
\norm{bėide} und \norm{bėidiu} besonders bei eher nahen Targets mit 6 bis 10
Wort\-formen Abstand herrscht, wobei \norm{bėidiu} ansonsten durchweg die
Hauptform darstellt. Der plötzliche Anstieg der \norm{bėide}-Form bei Targets
mit verschiedenem belebten Geschlecht im oberen Abstandsbereich wird als Zufall
gewertet.
...
\norm{bėide} `beide (\textsc{m+f})' mit wachsender Distanz zwischen Controller
...
\sectref{sec:adjdeclcao} zur Adjektivdeklination im \CAO{} gibt
...
\caption{Geografische Verteilung von \norm{e}- und \norm{iu}-Flexionstypen bei
...
dortige Neben\-einander beider Formen.
...
\ex \label{ex:intraurkvar1}
	...
	\ex \label{ex:intraurkvar1_1}
		...
		\glll A: vnd daz wir deſte baz beide der ſtat zu Wirceburg vnd deme
				lande den Vride gevurdern mugen \\
			B: Vnd daz wir deſte baz beidu der ſtat zu Wirceburg/ Vnd dem lande
				den fride gevurdern mugen \\
			{} und dass wir desto besser beide der Stadt zu Würzburg und dem
				Land den Frieden fördern können \\
...
(\norm{bėide}~X \norm{unde}~Y) unterschieden werden kann. Angesichts der recht
homogenen Verteilung der Formen \norm{bėide} und \norm{bėidiu} ist aber nicht
...
\REF{ex:caobeidkoordtarg_1}, nachgestellt attributive
\REF{ex:caobeidkoordtarg_2} und prädikative \REF{ex:caobeidkoordtarg_3},
...
\begin{table}
\centering
\caption{Form von \norm{bėide} nach Personenmerkmalen adjektivischer Konjunkte}
\begin{tabular}{>{\scshape}l >{\scshape}l r r r}
\lsptoprule
\normalfont Konjunkt 1
	& \normalfont Konjunkt 2
...
kommt \norm{bėide} in diesem Zusammenhang mit Temporaladverbien
\REF{ex:caokoordsyn_1}, Lokaladverbien \REF{ex:caokoordsyn_2} und
...
\begin{table}
\centering
\caption{Form von \norm{bėide} nach der Wortart adverbialer und präpositionaler Konjunkte}
