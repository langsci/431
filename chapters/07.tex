...
weiteren solchen Fällen gewissermaßen als
...
\REF{ex:beid2coordncao1} näher betrachtet. Dabei werden Kombinationen von
...
Zwei Belege werden in \REF{ex:beid2coordncao1} angeführt. Das Schema in
\REF{ex:beid2coordncao1_1} verdeutlicht den syntaktischen Kontext.
...
Daneben liegen noch die drei Belege in \REF{ex:beid2coordncao2} vor, die aus
...
(vgl.~\sectref{par:adjmuenchen}). In anderen Urkunden aus Herrenchiemsee und
...
\norm{bėid}, siehe die Beispiele in \REF{ex:cao_samegend_beide}. Was im
Vergleich zu den Belegen mit direktem Bezug auf zwei Substantive mit gleichem
Geschlecht (\sectref{subsubsec:perscombsgnp}) nicht bezeugt ist, sind Belege
...
typisch mit \norm{siu} `sie' bezeichnet. Diese Form ist nicht als neutral
...
Wien (vgl.~\sectref{par:adjwien}) aus den Jahren 1289 beziehungsweise 1291.
...
\ex \label{ex:cao_diffgend_33_beide}
	...
	\ex \label{ex:cao_diffgend_33_beide_2}
		\gll Jch Heinrich der Swab vnd min houſvrowe Chvnegunt
			...
			Kunigunde[\textsc{nom.sg.\FemF}] {} darüber geben \textsc{1pl\subMF.nom}
...
An der Belegverteilung in \tabref{tab:caosimprefctrl} ist zu erkennen, dass unter
...
\REF{ex:cao_diffgend_13_beide} stammen aus% Basel (1273%
...
\ex \label{ex:cao_diffgend_13_beide}
	...
	\ex \label{ex:cao_diffgend_13_beide_2}
		\gll Jch Rvͦdolf \textelp{} vnde Adelheit min wirtin / tvͤn
			...
			Ehefrau {} tun kund {} dass \textsc{1pl\subMF.nom}
...
aus Straßburg und sind mit Vorsicht zu werten (vgl.~\sectref{par:adjstrassburg}).
Die Beispiele in \REF{ex:cao_diffgend_11_beide}
...
\ex \label{ex:cao_diffgend_11_beide}
	...
	\ex \label{ex:cao_diffgend_11_beide_1}
		\gll wîr dú vorgenanten herre Otto / vn fro Berchte
			...
			\textsc{1pl\subMF.nom} die vorgenannten Herr Otto[\textsc{nom.sg.\MascM}] {}
				und Frau Berta[\textsc{nom.sg.\FemF}] {} \textsc{1pl\subMF.nom} haben
	...
	\ex \label{ex:cao_diffgend_11_beide_2}
		\gll Wir loben ovch paide, ich Herman von
			...
			\textsc{1pl\subMF.nom} versprechen auch beide-\textsc{nom.pl.m+f\subMF.st}
...
Quantifier (vgl. \sectref{sec:floatquant}) einnimmt. Es wird angenommen, dass
...
\begin{xlist}
	\ex \label{ex:remgenbeide_1}
		\gll von den luitín die vrí beidí gívoren ſien \\
			...
			\tsup{?}\norm{gevuore}-\textsc{nom.pl.\MascA} sind
			\\
		\trans `von den Leuten, die ihr beider Fahrtgenossen (?) sind'
			\parencites(\tit{Mühlhäuser Rechtsbuch}; Nordhausen, Stadtarchiv,
				Ms. II, Na 6%
				%; \cite[1379]{hsc}
				)[M320: 17v,21--22]{rem}
...
\REF{ex:beid2p2combrem} zitierten Belege zurück, allerdings stammen zwei davon
...
von \citet{lachmannhartl1952}. \tabref{tab:asksiebeidekombis} gibt die Ergebnisse
...
Abbildung P~26]{ksw2} verzeichnen in der zweiten Hälfte des 13.\ Jahrhunderts für
das Bairische\il{Bairisch}, den alemannisch-bairischen
...
\begin{table}
\centering
\caption{Kombinationen von \norm{si/sie/siu} und \norm{di/die/diu} mit \norm{bėide/-iu} im \tit{Corpus der altdeutschen Originalurkunden}}
\begin{tabular}{
	l
	r r
	r
...
alemannischen\il{Alemannisch} Urkunden (Kloster Kirchberg,
Überlingen, Zürich). Da in keiner der Urkunden, die diese Belege enthalten, ein
...
Controllern und \norm{bėide} ($N_i + N_j$) die Belebtheit der Substantive eine
...
\begin{table}
\centering
\caption{Form von \norm{bėide} nach syntaktischem Kontext (einf.
	Bezug)}
\begin{tabular}{
	l l
	c
...
\sectref{subsubsec:beid2p2coordncao}. Einzig aufällig ist, dass sich bei der
...
\sectref{subsubsec:beid2p2snglncao} ändert sich also nichts.
...
\caption{Geografische Verteilung von \norm{e}- und \norm{iu}-Flexionstypen bei
	\norm{bėide} als Quantor (X, Y \dots\ \norm{(si) bėide}; Hintergrundkarte:
...
(\norm{bėide}, X \norm{unde} Y). Aus der Belegverteilung im
