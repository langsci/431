...
(Substantive, Pronomina) wie zum Beispie \norm{her Rvͦdiger vnd ſin hovſfrowe}
\wdef{Herr Rüdiger und seine Ehefrau} in \REF{ex:beid2coordncao1} und dem
Schema in \figref{fig:beid2coordncao1_1} näher betrachtet. Dabei werden
...
(\textsc{pl})} beziehen sich bereits auf eine Kombination von Referenten mit
ihren jeweiligen Personenmerkmalen. Das Pronomen ist mit seinem Bezug über den
referenziellen \isi{Index} verbunden; \norm{bėide} selbst kongruiert mit dem
Pronomen. Das Beispiel in \REF{ex:beid2p2coordncao} verdeutlicht den hier
untersuchten syntaktischen Zusammenhang. Während für das \q{klassische}
Mittelhochdeutsche von einem Unterschied zwischen Maskulinum-Femininum
\norm{sie} und Neutrum \norm{siu} ausgegangen wird
\autocites[vgl.][213--214]{paul2007}[369, 390--397]{ksw2}, fehlt sowohl im
\CAO{} als auch in der \KC{} eindeutige Evidenz für diesen Genusunterschied,
siehe die Teiluntersuchungen zur Form des Pronomens in
...
(\textsc{pl})} ist zumindest im \CAO{} der Vereinfachung des pronominalen
...
(Teil-)Satz; vgl.~\sectref{sec:ctrltarg}). Die Schemata in
...
\norm{hūsvrouwe} stehen im \isi{Vorfeld} (\textsc{vf}), während
...
Vollverb besetzt ist, bleibt die rechte Satzklammer (\textsc{rsk}) leer. Der
...
Controller \norm{si} \wdef{sie (\textsc{pl})} unmittelbar, das heißt mit einem
Abstand von null Wortformen. Das Pronomen \norm{si} wiederum geht in letzter
Instanz zurück auf das Erstcontroller-Paar \norm{Konrad} und
...
\norm{hȫren lęsen} \wdef{lesen hören} darauf ausgelegt sind, mündlich verlesen
...
klar zwischen pronominalem Gebrauch (\norm{bėide,~X unde~Y}) und
konjunktionalem (\norm{bėide~X unde~Y}) unterschieden werden
...
belebtem Geschlecht liegen zu wenige Belege vor, um eine Regel aufzustellen.
Dasselbe gilt für die drei Belege für \norm{bėide} bei gleichem
unbelebtem\is{Inanimata} \isi{Genus}. Bei verschiedenem unbelebtem Genus
...
Beleg für \norm{bėide} bei verschiedenem unbelebtem Genus steht in den
