...
\citet[28]{behaghel1928} und \citet[188]{dal2014} als \textquote{von alters her}
...
der einzelnen Pluralcontroller in \sectref{ch:caoanalyse}, \sectref{ch:kcanalyse} für die
...
\REF{ex:pronindir_1} das Objekt darstellt (vgl.~\sectref{sec:floatquant} zu
...
Fällen wie in \REF{ex:pronindir}, in denen das Target indirekt von einem
...
anstelle von \norm{bėidiu} möglich wäre, wie in \sectref{sec:gendsex} ausgeführt,
...
(siehe~\sectref{subsubsec:monoflexioncao}), ist selbst bei den wenigen Fällen von
...
männlichen Bezug \REF{ex:papstkoenig5}; bei unterschiedlichem Geschlecht liegt
...
Ausgleichsregel bei Inanimata eintritt (siehe \REF{ex:gendasshier}). Die
...
den Genusmerkmalen seiner Konjunkte flektiert (vgl.~\sectref{sec:ovwbeideconj}).
Nimmt man dennoch an, dass im Zusammenspiel mit nominalen Konjunkten innerhalb
der Konstruktion Closest Conjunct Agreement auftritt (vgl.~\sectref{sec:gendres}),
...
In \REF{ex:beidquantsyncont} wird versucht, den Gebrauch von \norm{bėide} mit
adverbialen Konjunkten in \sectref{subsec:caobeidquantsyncont} und
\ref{subsec:kcbeidquantsyncont} als F-Struktur zu modellieren. Während die
Präpositionalobjekte in \REF{ex:caokoordsyn_3_2} und \REF{fig:caokoordsyn_3_2}
...
\REF{ex:syntintvar1_2} und \REF{fig:syntintvar1_2} enthalten die Konjunkte
keinerlei Genusmerkmale, nach denen \lit{Beideu} flektieren könnte. In beiden
Fällen ist eine Interpretation von \lit{Beideu} als Hinweis, dass sich der
...
\Cref{fig:cao_beideconj_geo_1,fig:cao_beideconj_geo_2} zeigen die jeweilige
...
ist, finden sich Belege für \norm{bėide} im Alemannischen, Ostfränkischen und
...
Bairische 89\,\% \norm{bėidiu} verzeichnen, für das Übergangsgebiet dazwischen
...
die zwei Belege in \REF{ex:kczbeidenundesynt1} aus der \KC{}-Handschrift Z
