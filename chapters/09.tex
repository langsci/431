...
Genusmerkmal problemlos kongruiert werden kann, was in \figref{fig:npconstit}
...
semantische Geschlechtsmerkmale (Sexus) keine Schnittmenge bilden können wie im
dritten Diagramm von \figref{fig:sexintersect}.
...
seinem Controller in den formalen Merk\-malen (\textsc{concord}): Kasus
(\textsc{case}), Genus (\textsc{gend}) und Numerus (\textsc{num}). Darüber
hinaus koindiziert (\textsc{index}) es seinen Controller mit dessen Merkmalen
Person (\textsc{pers}), Numerus (\textsc{num}) und Geschlecht (\textsc{sex}).
...
die aufgelösten Personenmerkmale enthält. Sodann bildet $g_1$ mit $g_2$ eine
Apposition in $f$, wobei das Pronomen \norm{bėiden} in $g_2$ mit $g_1$
koindiziert ist. Beide Teile zusammengenommen haben ihrerseits ein
Index-Merkmal, das hier allerdings dem Index $i$ entspricht, da $g_1$ keine
neuen Informationen hinzufügt. In der C-Struktur auf der linken Seite bilden
