...
die \term{Gender Assignment Hierarchy}\is{Genuszuweisung} in
\REF{ex:gendasshier} auf (\cites[584]{wechsler2009}[195]{wechslerzlatic2003}).
...
\REF{ex:gendassgmt2} bezieht sich das Target \lit{beide} \wdef{beide} auf den
...
Interessant ist diese Stelle, weil \lit{peidev} \wdef{beide} das
\isi{Personalpronomen} \lit{ſi} \wdef{sie (\textsc{pl})}
modifiziert\is{Modifikator}, sodass aufgrund der Wahrscheinlichkeit für
...
direkten, pronominalen Controller \lit{ſi} \wdef{sie (\textsc{pl})} inklusive
semantischer Kongruenz\is{Kongruenz!semantische} und die dabei operierende
...
Ob auch der direkte Bezug auf ein einzelnes Hybrid Noun im Plural eine
Kongruenzform entsprechend der Semantik auslösen würde, lässt sich nicht
feststellen, da das ausgewertete Material keine entsprechenden Fälle enthält.
Aus der Analyse von \citet[54--55, 84]{wechslerzlatic2003} basierend auf Daten
zu prädikativen Adjektiven\is{Adjektiv!prädikativ} im
...
\REF{ex:iclgr2_3} mit den Abbildungen~\ref{fig:iclgr2_2} und \ref{fig:iclgr2_3}, kommt es
...
definiert; das Neutrum als Resolutionsgenus\is{Gender Resolution} ist eine
...
(\textsc{pl})} oder \norm{wir} \wdef{wir}. Das Personalpronomen ist in allen
untersuchten Kontexten genus\-indifferent\is{Genusindifferenz}, gibt also
...
Erklärungsansatz für \isi{Gender Resolution} von \citet{wechsler2009} und
...
\isi{Gender Resolution} zu rechnen. Aufgrund der Apposition\is{Apposition} muss
