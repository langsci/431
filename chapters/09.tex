...
Quantor \norm{al} \wdef{alle} \autocite[vgl.][606--621]{ksw2} kommt in der
\KC{} im relevanten Kontext nahezu ausschließlich mit Bezug auf einzelne
...
stützen sich ebenfalls maßgeblich auf das \CAO{}, da die \KC{} aufgrund ihres
Inhalts kaum Belege dazu hergibt.
...
Wortformenabstand\is{Distanz!lineare} zwischen Controller und Target. Bei den
Urkunden\is{Urkunde} hatte sich gezeigt, dass ein Vokal nach der Flexionsendung
...
\REF{ex:gendassgmt2} bezieht sich das Target \lit{baide} \wdef{beide} auf den
Controller \lit{richtare} \wdef{Richter}, wobei der Quantor hinter dem finiten
Verb im \isi{Mittelfeld} und damit getrennt von dem Substantiv steht, das er
...
\wdef{Frau (\NeutF)} interessant,\is{Desiderat} da dadurch Aufschluss darüber
...
in \REF{ex:gendres2} gegeben. Hier treten \lit{Rvͦdiger} \wdef{Rüdiger
(\MascM)} und \lit{ſin hovſfrowe} \wdef{seine Ehefrau (\FemF)} als kombinierte
Controller auf, die durch das Target\is{Target} \lit{bediv} (\NeutMF) gemeinsam
aufgenommen werden. Gemischt männliche und weibliche Referenz wird mit dem
Neutrum als derjenigen Option markiert, die weder männliche noch weibliche
Personen bezeichnet. Dass semantische Kriterien im Spiel sind, wird darin
sichtbar, dass die \norm{e}-Form formal gleichermaßen maskuline und feminine
Referenten bezeichnet. Dies wird deutlich in der Analyse der einzelnen
Pluralcontroller in \chapref{ch:caoanalyse} und \ref{ch:kcanalyse} für die
...
Wie in \sectref{subsubsec:beid2p2snglncao} und
\sectref{subsubsec:beid2p2snglnkc} zum indirekten Bezug von \norm{bėide} auf
...
\REF{ex:f+f_kindesibeidiu} die Form \lit{peidev} in Kongruenz mit
...
\norm{si} \wdef{sie} modifiziert\is{Modifikator}, sodass aufgrund der
Wahrscheinlichkeit für semantische Kongruenz\is{Kongruenz!semantische} bei
pronominaler Referenz auch eine Form vom Typ \norm{bėide} anstelle von
\norm{bėidiu} möglich wäre, wie in \sectref{sec:gendsex} ausgeführt, zumal die
\lit{chinde} \wdef{Kinder} im Kontext namentlich als weiblich identifiziert
werden. Dem Zusammenhang nach handelt es sich bei ihnen nicht notwendigerweise
...
\wdef{sie} oder \norm{di} \wdef{die (\textsc{rel.pl})} bei übereinstimmendem
Geschlecht im \CAO{} stets die Form \norm{bėide} steht, bei unterschiedlichem
Geschlecht dagegen Variation zwischen \norm{bėidiu} \REF{ex:m+f_si_beidiu} und
...
\wdef{beide}, wie in \REF{ex:m+f_si_beidiu_float}\is{Floating Quantifier}
...
\wdef{Ulrich oder Frau Margarete}, daher spielt formale
...
Floating Quantifier entsprechend eine Position zwischen \q{attributive} und
...
dem Kontext nach am ehesten auf \lit{zehenden} \wdef{Zehnten} und \lit{Garten}
\wdef{Garten}. Da es sich bei den Ausstellern der Urkunde um die drei Brüder
...
seiner Resolutionsregel vermelde, insofern das Maskulin-Plural-\isi{Default}
übermäßig auf solche Kontexte\is{Pragmatik} angewandt wird, die allein aus
...
zitierten Beleg an. Dort steht in Bezug auf die Abstrakta \lit{liͤbe}
...
Bezug auf ein Hybrid Noun wie \norm{wīp} \wdef{Frau (\NeutF)} als
\isi{Controller} beim \isi{Target} in gefloateter\is{Floating Quantifier}
...
bieten und nach dem \isi{Kasus} flektieren, den die Präposition regiert, steht
...
\ex \label{ex:beidquantsyncont}
	...
	\ex \label{ex:caokoordsyn_3_2}
		...
		\caption{Analyse des Satzfragments \norm{bėidiu ze dorfe unde ze velde}
...
Kartenbild bemerkbar macht. \figref{fig:cao_beideconj_geo_1} und
\ref{fig:cao_beideconj_geo_2} zeigen die jeweilige
...
\caption{Geografische Distribution der Form \norm{bėide} in
	\norm{bėide \dots\ unde} \wdef{sowohl \dots\ als auch} (Hintergrundkarte:
...
Konjunkte \norm{man} \wdef{Mann} und \norm{wīp} ihre Index-Merkmale zu einem
...
bilden \norm{man unde wīp} zusammen eine NP\is{Nominalphrase}, die wiederum
zusammen mit der QP\is{Quantorenphrase} \norm{bėiden} eine
