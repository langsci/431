...
in dem das Target \lit{pediv} `beide' mit nur einem nominalen Controller
\lit{Jnſidel} `Siegel' kongruiert. Es handelt sich dabei um ein
...
\lit{Jnſidel} `Siegel' bietet gemäß seines Lexikoneintrags das Neutrum
...
Controller \lit{rihtære} `Richter', wobei der Quantor hinter dem finiten
...
treten \lit{Rvͦdiger} `Rüdiger (\MascM)' und \lit{ſin hovſfrowe}
...
mit \lit{hof} `Hof' und \lit{zehenden} `Zehnten' zwei Maskulina
...
(\textsc{sex}). Formal wird vom Pronomen \norm{si} aber kein Genus (\textsc{gend}) festgelegt,
...
`beide', wie in \cref{ex:m+f_si_beidiu_float} illustriert
...
derselben NP wie sein Controller \lit{vlrich / oder fraw Margret} `Ulrich
...
\ex \label{ex:m+m_inan_e2}
	...
	\ex \label{ex:m+m_inan_e2_1}
		...
		\trans `und einen Zehnten in Plešivec und in Velenje einen
	...
	\ex \label{ex:m+m_inan_e2_2}
		\gll einen Weingarten \textelp{} Vnd einen Chrvͦtgarten
				\textelp{} · die geltent beide ſechſthalben ſchillinch
				\textelp{} \\
			einen Weingarten[\textsc{acc.sg.\MascI}] {} und einen
				Gemüsegarten[\textsc{acc.sg.\MascI}] {} {}
...
oder auf \lit{Jungherren} `Junker'. In jedem Fall ist die Kongruenzform
...
\lit{zehenden} `Zehnten' und \lit{Garten} `Garten'. Da es sich bei
den Ausstellern der Urkunde um die drei Brüder \lit{wezel vn̄ hainrich wezel
vnde Cvͦnrat de\textins{n} Bodemer} \autocites(Nrn.~1201~AB)[472,6--7]{cao2}
handelt, wird sich \lit{baidiv} nicht auf \lit{wier} beziehen. Vielmehr wurde
...
die Abstrakta \lit{liͤbe} `Liebe' und \lit{ſchoͤne} `Schönheit' das
Demonstrativum \lit{diſiu̍} `diese', das zumindest der Form nach als
...
(\norm{-e}) eintritt, das heißt, ob eher \textsc{gend} oder \textsc{sex} das wichtigere
...
`Mann' und \norm{wīp} `Frau' in $g_2$ mit ihren grammatischen
