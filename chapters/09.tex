...
seines Lexikoneintrags das Neutrum als \isi{Genusmerkmal}.
...
\norm{si} `sie' modifiziert\is{Modifikator}, sodass aufgrund der
...
\lit{chinde} `Kinder' im Kontext namentlich als weiblich identifiziert werden.
...
`beide', wie in \REF{ex:m+f_si_beidiu_float}\is{Floating Quantifier}
...
\lit{rihtær} `Richter' bezieht, das seinerseits männlich
...
Bezug auf ein Hybrid Noun wie \norm{wīp} `Frau (\NeutF)' als \isi{Controller}
beim \isi{Target} in gefloateter\is{Floating Quantifier} Position eher formale
...
das wichtigere Merkmal bei der Kongruenz\is{Genuskongruenz} darstellt.%
%
