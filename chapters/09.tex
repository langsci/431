...
\norm{all} `alle' \autocite[vgl.][606--621]{ksw2} kommt in der \KC{}
...
%
	\footnote{Auch im 20.~Jahrhundert finden sich noch Belege wie der folgende,
		...
		\ul{sie} \textins{\textsc{3pl\subMF.nom}} auf der Erde, zwei Ehebrecher in
	...
	\ex \label{ex:m+m_inan_e_2}
		\gll einen Weingarten \textelp{} Vnd einen Chrvͦtgarten
			...
			einen Weingarten[\textsc{acc.sg.\MascI}] {} und einen
				Gemüsegarten[\textsc{acc.sg.\MascI}] {} {} \textsc{dem.nom.pl.\MascI}
				gelten beide-\textsc{nom.pl.\MascI.st} sechsthalben Schilling {} \\
		\begin{taggedline}{\parencites%
			(Regensburg, 1296)
			[\pno~2396, 484.28--30]{cao3}}
		\trans `einen Weingarten \textelp{} und einen Gemüsegarten
...
ausgeführt, zumal die \lit{chinde} `Kinder' im Kontext namentlich als
...
\ex \label{ex:m+m_inan_e2}
	...
	\ex \label{ex:m+m_inan_e2_2}
		\gll einen Weingarten \textelp{} Vnd einen Chrvͦtgarten
				...
				ſchillinch \textelp{} \\
			einen Weingarten[\textsc{acc.sg.\MascI}] {} und einen
				Gemüsegarten[\textsc{acc.sg.\MascI}] {} {} \textsc{rel.nom.pl.\MascI}
				gelten beide-\textsc{nom.pl.\MascI.st} sechsthalben Schilling {} \\
		\begin{taggedline}{\parencites%
			(Regensburg, 1296)%
			[\pno~2396, 484.28--30]{cao3}}
...
Urkunde zu \cref{ex:m+m_inan_e2_1} enthält neben \lit{mit allev dew} `mit
all dem' und \lit{auf alle deu} `auf alle die' \autocite[\pno~2401,
...
\sn \gll hant diſu̍ beide rehten mvͦt~· \\
	...
	Gesinnung sind.' % Walther von der Vogelweide
	\end{taggedline}
	\\
\end{exe}
...
\ex \label{ex:beidquantsyncont}
	...
	\ex \label{ex:syntintvar1_2}
		\gll Beideu \textup{[\tsup{Adv}} ſpæt \textup{]} vnde
