...
% \citetitle{cao} und der \citetitle{kc} quantitativ und qualitativ untersucht.
...
In beiden Datenserien, dem \citetitle*{cao} (\citetitle{cao}) und der
\citetitle{kc} (\citet{kc}), ist der direkte Bezug zwischen der Kombination von
...
\citetitle{cao}-Material hier allein betrachtet werden muss. Der Quantor
\norm{all} \wdef{alle} \autocite[vgl.][606--621]{ksw2} kommt in der \citet{kc}
...
stützen sich ebenfalls maßgeblich auf das \citetitle{cao}, da die \citet{kc}
...
Umgekehrt enthält das Material zum \citetitle{cao} einen Beleg, bei dem in
...
\citetitle{cao}- noch im \citet{kc}-Material vorhanden. Eine Suche im
...
sowohl im \citetitle{cao} als auch in der \citet{kc} sehr klein
...
%
	\footnote{Kursorische Suchen im \citetitle{cao} sowie im \citetitle{rem}
...
zwischen Controllern und Target noch weitere vier Male im \citetitle{cao}. Die
...
\norm{-iu}. Der regelmäßige Fall mit \norm{-e} ist im \citetitle{cao} zweimal
...
(\Rel.\Pl)} bei übereinstimmendem Geschlecht im \citetitle{cao} stets die Form
...
Wie bereits bei der Diskussion der \citetitle{cao}-Belege geschildert
...
\citet{kc} gibt es zumindest einen Beleg für \norm{bėidiu} bei kombiniertem
...
Belegmaterial aus dem \citetitle{cao} zur Verfügung. Wie bei der Auswertung zur
...
% dieser Episode der \citet{kc} debattiert.
...
Stichprobe betroffen sind -- sechs im ausgewertete Material des \citetitle{cao}
und weitere drei in der Stichprobe zur \citet{kc}. Es handelt sich hier
also eher um eine Randerscheinung.
% Dabei wurden beim \citetitle{cao} alle verfügbaren Belege gewertet, auch
...
im \citetitle{cao} nur in Kontexten mit kombinierter Referenz auftritt, sowohl
...
Belege vorliegen gegenüber den sechs Belegen beim \citetitle{cao}, verhalten
sich beide Datenserien konträr zueinander.

Dadurch, dass neutrales \norm{bėidiu} mit maskuliner Referenz hauptsächlich mit
unbelebten Controllern einhergeht, gilt zumindest für das \citetitle{cao}
...
trifft auf die Beleglage im \citetitle{cao} nicht ganz zu, und gar nicht auf
...
\citetitle{cao}-Belege zum Neutrum mit Bezug auf Maskulina aus Augsburg,
...
\citetitle{cao} und der \citet{kc} wurde das in
...
%
	...
	sowohl im \citetitle{cao} als auch in den als oberdeutsch klassifizierten
...
Urkunden des \citetitle{cao} und die \citet{kc}-Handschriften
...
femininen Inanimata über. Auch in den \citetitle{cao}-Belegen sind
...
Verbreitung beider Typen aufgrund der aus dem \citetitle{cao} gewonnenen
...
die zwei Belege in \cref{ex:kczbeidenundesynt1} aus der \citet{kc}-Handschrift
