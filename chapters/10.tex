...
Personenmerkmalen\is{Personenmerkmal} auftritt, wird als \emph{\isi{Gender
...
zusammen ca.~282 Vorkommen pro 1~Millionen Wortformen, für die \KC{} ca.~510
Vorkommen pro Million. Die Texte des \CAO{} und der \KC{} wurden mit Hilfe
von regulären Ausdrücken\is{regulärer Ausdruck} durchsucht und anschließend
manuell annotiert \autocites[vgl.\ z.\,B.][33--37]{perkuhnetal2012}[zur Methode
...
diesen Kontexten \isi{Distanzstellung} vor (N\tsub{i} \norm{unde} N\tsub{j}
...
Belege aus (\isi{Kontaktstellung}: PRO\tsub{i+j} \norm{bėide}; Distanzstellung:
PRO\tsub{i+j} \dots\ \norm{bėide}). Genus\-indifferentes\is{Genusindifferenz}
\norm{si} \wdef{sie (\textsc{pl})} stellt gefolgt von \norm{wir} \wdef{wir} den
häufigsten unmittelbaren Controller von \norm{bėide} dar.
...
unbelebt\is{Inanimata}) im \CAO{} weisen in diesem Kontext \isi{Gender
...
Bei unbelebtem gemischtem Bezug erfolgt noch regelmäßiger als bei den
